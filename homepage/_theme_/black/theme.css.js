//CSS=theme.css
    
(function() {
  return function() {
    var obj;   
    
    obj = new nexacro.Style_accessibility("","true","all","","","");
    this._addCss("MainFrame", "accessibility", obj, ["normal"]);
    this._addCss("ChildFrame", "accessibility", obj, ["normal"]);
    this._addCss("TitleBarControl", "accessibility", obj, ["normal"]);
    this._addCss("TitleBarControl>#minbutton", "accessibility", obj, ["normal"]);
    this._addCss("TitleBarControl>#maxbutton", "accessibility", obj, ["normal"]);
    this._addCss("TitleBarControl>#normalbutton", "accessibility", obj, ["normal"]);
    this._addCss("TitleBarControl>#closebutton", "accessibility", obj, ["normal"]);
    this._addCss("StatusBarControl", "accessibility", obj, ["normal"]);
    this._addCss("StatusBarControl>#progressbar", "accessibility", obj, ["normal"]);
    this._addCss("StatusBarControl>#zoomcombo", "accessibility", obj, ["normal"]);
    this._addCss("StatusBarControl>#zoomcombo>#comboedit", "accessibility", obj, ["normal"]);
    this._addCss("StatusBarControl>#zoomcombo>#dropbutton", "accessibility", obj, ["normal"]);
    this._addCss("Form", "accessibility", obj, ["normal"]);
    this._addCss("VScrollBar", "accessibility", obj, ["normal"]);
    this._addCss("VScrollBar>#incbutton", "accessibility", obj, ["normal"]);
    this._addCss("VScrollBar>#decbutton", "accessibility", obj, ["normal"]);
    this._addCss("VScrollBar>#trackbar", "accessibility", obj, ["normal"]);
    this._addCss("VScrollBarControl", "accessibility", obj, ["normal"]);
    this._addCss("VScrollBarControl>#incbutton", "accessibility", obj, ["normal"]);
    this._addCss("VScrollBarControl>#decbutton", "accessibility", obj, ["normal"]);
    this._addCss("VScrollBarControl>#trackbar", "accessibility", obj, ["normal"]);
    this._addCss("HScrollBar", "accessibility", obj, ["normal"]);
    this._addCss("HScrollBar>#incbutton", "accessibility", obj, ["normal"]);
    this._addCss("HScrollBar>#decbutton", "accessibility", obj, ["normal"]);
    this._addCss("HScrollBar>#trackbar", "accessibility", obj, ["normal"]);
    this._addCss("HScrollBarControl", "accessibility", obj, ["normal"]);
    this._addCss("HScrollBarControl>#incbutton", "accessibility", obj, ["normal"]);
    this._addCss("HScrollBarControl>#decbutton", "accessibility", obj, ["normal"]);
    this._addCss("*>#resizebutton", "accessibility", obj, ["normal"]);
    this._addCss("Static", "accessibility", obj, ["normal"]);
    this._addCss("StaticControl", "accessibility", obj, ["normal"]);
    this._addCss("Edit", "accessibility", obj, ["normal", "readonly"]);
    this._addCss("EditControl", "accessibility", obj, ["normal", "readonly"]);
    this._addCss("TextArea", "accessibility", obj, ["normal", "readonly"]);
    this._addCss("TextAreaControl", "accessibility", obj, ["normal", "readonly"]);
    this._addCss("MaskEdit", "accessibility", obj, ["normal", "readonly"]);
    this._addCss("MaskEditControl", "accessibility", obj, ["normal", "readonly"]);
    this._addCss("Button", "accessibility", obj, ["normal"]);
    this._addCss("ButtonControl", "accessibility", obj, ["normal"]);
    this._addCss("Spin", "accessibility", obj, ["normal", "readonly"]);
    this._addCss("Spin>#spinedit", "accessibility", obj, ["normal"]);
    this._addCss("Spin>#spinupbutton", "accessibility", obj, ["normal"]);
    this._addCss("Spin>#spindownbutton", "accessibility", obj, ["normal"]);
    this._addCss("SpinControl", "accessibility", obj, ["normal", "readonly"]);
    this._addCss("SpinControl>#spinedit", "accessibility", obj, ["normal"]);
    this._addCss("SpinControl>#spinupbutton", "accessibility", obj, ["normal"]);
    this._addCss("SpinControl>#spindownbutton", "accessibility", obj, ["normal"]);
    this._addCss("ListBox", "accessibility", obj, ["normal"]);
    this._addCss("ListBoxControl", "accessibility", obj, ["normal"]);
    this._addCss("Combo", "accessibility", obj, ["normal", "readonly"]);
    this._addCss("Combo>#comboedit", "accessibility", obj, ["normal", "mouseover", "focused", "readonly"]);
    this._addCss("Combo>#dropbutton", "accessibility", obj, ["normal"]);
    this._addCss("ComboControl", "accessibility", obj, ["normal", "readonly"]);
    this._addCss("ComboControl>#comboedit", "accessibility", obj, ["normal", "mouseover", "focused", "readonly"]);
    this._addCss("ComboControl>#dropbutton", "accessibility", obj, ["normal"]);
    this._addCss("Radio", "accessibility", obj, ["normal"]);
    this._addCss("RadioControl", "accessibility", obj, ["normal"]);
    this._addCss("CheckBox", "accessibility", obj, ["normal"]);
    this._addCss("CheckBoxControl", "accessibility", obj, ["normal"]);
    this._addCss("Div", "accessibility", obj, ["normal"]);
    this._addCss("DivControl", "accessibility", obj, ["normal"]);
    this._addCss("PopupDiv", "accessibility", obj, ["normal"]);
    this._addCss("PopupDivControl", "accessibility", obj, ["normal"]);
    this._addCss("Step", "accessibility", obj, ["normal"]);
    this._addCss("StepControl", "accessibility", obj, ["normal"]);
    this._addCss("ImageViewer", "accessibility", obj, ["normal"]);
    this._addCss("ImageViewerControl", "accessibility", obj, ["normal"]);
    this._addCss("GroupBox", "accessibility", obj, ["normal"]);
    this._addCss("GroupBoxControl", "accessibility", obj, ["normal"]);
    this._addCss("Calendar", "accessibility", obj, ["normal"]);
    this._addCss("Calendar>#calendaredit", "accessibility", obj, ["normal"]);
    this._addCss("Calendar>#dropbutton", "accessibility", obj, ["normal", "mouseover", "pushed", "focused"]);
    this._addCss("Calendar>#spinupbutton", "accessibility", obj, ["normal"]);
    this._addCss("Calendar>#spindownbutton", "accessibility", obj, ["normal"]);
    this._addCss("CalendarControl", "accessibility", obj, ["normal"]);
    this._addCss("CalendarControl>#calendaredit", "accessibility", obj, ["normal"]);
    this._addCss("CalendarControl>#dropbutton", "accessibility", obj, ["normal", "mouseover", "pushed", "focused"]);
    this._addCss("CalendarControl>#spinupbutton", "accessibility", obj, ["normal"]);
    this._addCss("CalendarControl>#spindownbutton", "accessibility", obj, ["normal"]);
    this._addCss("Grid", "accessibility", obj, ["normal"]);
    this._addCss("Grid>#head", "accessibility", obj, ["normal"]);
    this._addCss("Grid>#body", "accessibility", obj, ["normal"]);
    this._addCss("Grid>#summ", "accessibility", obj, ["normal"]);
    this._addCss("Grid>#controledit", "accessibility", obj, ["normal"]);
    this._addCss("Grid>#controltextarea", "accessibility", obj, ["normal", "readonly"]);
    this._addCss("Grid>#controlmaskedit", "accessibility", obj, ["normal", "readonly"]);
    this._addCss("Grid>#controlbutton", "accessibility", obj, ["normal"]);
    this._addCss("Grid>#controlcombo", "accessibility", obj, ["normal", "readonly"]);
    this._addCss("Grid>#controlcombo>#comboedit", "accessibility", obj, ["normal", "mouseover", "readonly"]);
    this._addCss("Grid>#controlcombo>#dropbutton", "accessibility", obj, ["normal"]);
    this._addCss("Grid>#controlcheckbox", "accessibility", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar", "accessibility", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#calendaredit", "accessibility", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#dropbutton", "accessibility", obj, ["normal", "mouseover", "pushed", "focused"]);
    this._addCss("Grid>#controlcalendar>#spinupbutton", "accessibility", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#spindownbutton", "accessibility", obj, ["normal"]);
    this._addCss("GridControl", "accessibility", obj, ["normal"]);
    this._addCss("GridControl>#head", "accessibility", obj, ["normal"]);
    this._addCss("GridControl>#body", "accessibility", obj, ["normal"]);
    this._addCss("GridControl>#summ", "accessibility", obj, ["normal"]);
    this._addCss("GridControl>#controledit", "accessibility", obj, ["normal"]);
    this._addCss("GridControl>#controltextarea", "accessibility", obj, ["normal", "readonly"]);
    this._addCss("GridControl>#controlmaskedit", "accessibility", obj, ["normal", "readonly"]);
    this._addCss("GridControl>#controlbutton", "accessibility", obj, ["normal"]);
    this._addCss("GridControl>#controlcombo", "accessibility", obj, ["normal", "readonly"]);
    this._addCss("GridControl>#controlcombo>#comboedit", "accessibility", obj, ["normal", "mouseover", "readonly"]);
    this._addCss("GridControl>#controlcombo>#dropbutton", "accessibility", obj, ["normal"]);
    this._addCss("GridControl>#controlcheckbox", "accessibility", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar", "accessibility", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#calendaredit", "accessibility", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#dropbutton", "accessibility", obj, ["normal", "mouseover", "pushed", "focused"]);
    this._addCss("GridControl>#controlcalendar>#spinupbutton", "accessibility", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#spindownbutton", "accessibility", obj, ["normal"]);
    this._addCss("Tab", "accessibility", obj, ["normal"]);
    this._addCss("Tab>#spinupbutton", "accessibility", obj, ["normal"]);
    this._addCss("Tab>#spindownbutton", "accessibility", obj, ["normal"]);
    this._addCss("TabControl", "accessibility", obj, ["normal"]);
    this._addCss("TabControl>#spinupbutton", "accessibility", obj, ["normal"]);
    this._addCss("TabControl>#spindownbutton", "accessibility", obj, ["normal"]);
    this._addCss("Tabpage", "accessibility", obj, ["normal"]);
    this._addCss("PopupMenu", "accessibility", obj, ["normal"]);
    this._addCss("PopupMenu>#spindownbutton", "accessibility", obj, ["normal"]);
    this._addCss("PopupMenu>#spinupbutton", "accessibility", obj, ["normal"]);
    this._addCss("PopupMenuControl", "accessibility", obj, ["normal"]);
    this._addCss("PopupMenuControl>#spindownbutton", "accessibility", obj, ["normal"]);
    this._addCss("PopupMenuControl>#spinupbutton", "accessibility", obj, ["normal"]);
    this._addCss("Menu", "accessibility", obj, ["normal"]);
    this._addCss("Menu>#spindownbutton", "accessibility", obj, ["normal"]);
    this._addCss("Menu>#spinupbutton", "accessibility", obj, ["normal"]);
    this._addCss("MenuControl", "accessibility", obj, ["normal"]);
    this._addCss("MenuControl>#spindownbutton", "accessibility", obj, ["normal"]);
    this._addCss("MenuControl>#spinupbutton", "accessibility", obj, ["normal"]);
    this._addCss("ProgressBar", "accessibility", obj, ["normal"]);
    this._addCss("ProgressBarControl", "accessibility", obj, ["normal"]);
    this._addCss("FileUpload", "accessibility", obj, ["normal"]);
    this._addCss("FileUploadControl", "accessibility", obj, ["normal"]);
    this._addCss("FileDownload", "accessibility", obj, ["normal"]);
    this._addCss("FileDownloadControl", "accessibility", obj, ["normal"]);
    this._addCss("Tooltip", "accessibility", obj, ["normal"]);

    obj = new nexacro.Style_background("","","","0","0","0","0","true");
    this._addCss("MainFrame", "background", obj, ["normal"]);
    this._addCss("ChildFrame", "background", obj, ["normal"]);
    this._addCss("TitleBarControl>#minbutton", "background", obj, ["normal", "mouseover", "pushed"]);
    this._addCss("TitleBarControl>#maxbutton", "background", obj, ["normal", "mouseover", "pushed"]);
    this._addCss("TitleBarControl>#normalbutton", "background", obj, ["normal", "mouseover", "pushed"]);
    this._addCss("TitleBarControl>#closebutton", "background", obj, ["normal", "mouseover", "pushed"]);
    this._addCss("StatusBarControl>#progressbar", "background", obj, ["normal"]);
    this._addCss("StatusBarControl>#zoomcombo>#comboedit", "background", obj, ["normal"]);
    this._addCss("VScrollBar", "background", obj, ["normal"]);
    this._addCss("VScrollBarControl", "background", obj, ["normal"]);
    this._addCss("HScrollBar", "background", obj, ["normal"]);
    this._addCss("HScrollBarControl", "background", obj, ["normal"]);
    this._addCss("*>#resizebutton", "background", obj, ["normal"]);
    this._addCss("Static", "background", obj, ["normal"]);
    this._addCss("Spin>#spinedit", "background", obj, ["normal"]);
    this._addCss("SpinControl>#spinedit", "background", obj, ["normal"]);
    this._addCss("Combo>#comboedit", "background", obj, ["normal", "mouseover", "focused", "readonly"]);
    this._addCss("ComboControl>#comboedit", "background", obj, ["normal", "mouseover", "focused", "readonly"]);
    this._addCss("Radio", "background", obj, ["normal"]);
    this._addCss("RadioControl", "background", obj, ["normal"]);
    this._addCss("CheckBox", "background", obj, ["normal"]);
    this._addCss("CheckBoxControl", "background", obj, ["normal"]);
    this._addCss("Step", "background", obj, ["normal"]);
    this._addCss("StepControl", "background", obj, ["normal"]);
    this._addCss("GroupBox", "background", obj, ["normal"]);
    this._addCss("GroupBoxControl", "background", obj, ["normal"]);
    this._addCss("Calendar", "background", obj, ["normal"]);
    this._addCss("Calendar>#dropbutton", "background", obj, ["normal", "mouseover", "pushed", "focused", "disabled"]);
    this._addCss("Calendar>#popupcalendar>#yearspin>#spinedit", "background", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#monthspin>#spinedit", "background", obj, ["normal"]);
    this._addCss("CalendarControl", "background", obj, ["normal"]);
    this._addCss("CalendarControl>#dropbutton", "background", obj, ["normal", "mouseover", "pushed", "focused", "disabled"]);
    this._addCss("CalendarControl>#popupcalendar>#yearspin>#spinedit", "background", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#monthspin>#spinedit", "background", obj, ["normal"]);
    this._addCss("Grid", "background", obj, ["normal"]);
    this._addCss("Grid>#resizebutton", "background", obj, ["normal"]);
    this._addCss("Grid>#controlcombo>#comboedit", "background", obj, ["normal", "mouseover", "readonly"]);
    this._addCss("Grid>#controlcheckbox", "background", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar", "background", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#dropbutton", "background", obj, ["normal", "mouseover", "pushed", "focused", "disabled"]);
    this._addCss("GridControl", "background", obj, ["normal"]);
    this._addCss("GridControl>#resizebutton", "background", obj, ["normal"]);
    this._addCss("GridControl>#controlcombo>#comboedit", "background", obj, ["normal", "mouseover", "readonly"]);
    this._addCss("GridControl>#controlcheckbox", "background", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar", "background", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#dropbutton", "background", obj, ["normal", "mouseover", "pushed", "focused", "disabled"]);
    this._addCss("Tab", "background", obj, ["normal"]);
    this._addCss("Tab>#extrabutton", "background", obj, ["normal", "mouseover", "focused", "selected", "pushed", "disabled"]);
    this._addCss("TabControl", "background", obj, ["normal"]);
    this._addCss("TabControl>#extrabutton", "background", obj, ["normal", "mouseover", "focused", "selected", "pushed", "disabled"]);
    this._addCss("Tabpage", "background", obj, ["normal"]);
    this._addCss("FileUpload", "background", obj, ["normal"]);
    this._addCss("FileUploadControl", "background", obj, ["normal"]);

    obj = new nexacro.Style_border("0","none","","");
    this._addCss("MainFrame", "border", obj, ["normal"]);
    this._addCss("ChildFrame", "border", obj, ["normal"]);
    this._addCss("TitleBarControl>#minbutton", "border", obj, ["normal"]);
    this._addCss("StatusBarControl>#zoomcombo>#comboedit", "border", obj, ["normal"]);
    this._addCss("Form", "border", obj, ["normal"]);
    this._addCss("VScrollBar", "border", obj, ["normal"]);
    this._addCss("VScrollBarControl", "border", obj, ["normal"]);
    this._addCss("HScrollBar", "border", obj, ["normal"]);
    this._addCss("HScrollBarControl", "border", obj, ["normal"]);
    this._addCss("*>#resizebutton", "border", obj, ["normal"]);
    this._addCss("Static", "border", obj, ["normal"]);
    this._addCss("MaskEditControl", "border", obj, ["mouseover", "pushed", "focused", "disabled"]);
    this._addCss("Spin>#spinedit", "border", obj, ["normal"]);
    this._addCss("SpinControl>#spinedit", "border", obj, ["normal"]);
    this._addCss("CheckBox", "border", obj, ["normal"]);
    this._addCss("CheckBoxControl", "border", obj, ["normal"]);
    this._addCss("Div", "border", obj, ["normal"]);
    this._addCss("DivControl", "border", obj, ["normal"]);
    this._addCss("Step", "border", obj, ["normal"]);
    this._addCss("StepControl", "border", obj, ["normal"]);
    this._addCss("Calendar>#dropbutton", "border", obj, ["normal", "mouseover", "pushed", "focused", "disabled"]);
    this._addCss("Calendar>#popupcalendar>#prevbutton", "border", obj, ["mouseover", "selected", "focused", "pushed"]);
    this._addCss("Calendar>#popupcalendar>#nextbutton", "border", obj, ["mouseover", "selected", "focused", "pushed"]);
    this._addCss("Calendar>#popupcalendar>#yearspin>#spinedit", "border", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#yearspin>#spinupbutton", "border", obj, ["normal", "mouseover", "pushed"]);
    this._addCss("Calendar>#popupcalendar>#yearspin>#spindownbutton", "border", obj, ["normal", "mouseover", "pushed"]);
    this._addCss("Calendar>#popupcalendar>#monthspin>#spinedit", "border", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#monthspin>#spinupbutton", "border", obj, ["normal", "mouseover", "pushed"]);
    this._addCss("Calendar>#popupcalendar>#monthspin>#spindownbutton", "border", obj, ["normal", "mouseover", "pushed"]);
    this._addCss("CalendarControl>#dropbutton", "border", obj, ["normal", "mouseover", "pushed", "focused", "disabled"]);
    this._addCss("CalendarControl>#popupcalendar>#prevbutton", "border", obj, ["mouseover", "selected", "focused", "pushed"]);
    this._addCss("CalendarControl>#popupcalendar>#nextbutton", "border", obj, ["mouseover", "selected", "focused", "pushed"]);
    this._addCss("CalendarControl>#popupcalendar>#yearspin>#spinedit", "border", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#yearspin>#spinupbutton", "border", obj, ["normal", "mouseover", "pushed"]);
    this._addCss("CalendarControl>#popupcalendar>#yearspin>#spindownbutton", "border", obj, ["normal", "mouseover", "pushed"]);
    this._addCss("CalendarControl>#popupcalendar>#monthspin>#spinedit", "border", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#monthspin>#spinupbutton", "border", obj, ["normal", "mouseover", "pushed"]);
    this._addCss("CalendarControl>#popupcalendar>#monthspin>#spindownbutton", "border", obj, ["normal", "mouseover", "pushed"]);
    this._addCss("Grid>#summ", "border", obj, ["normal"]);
    this._addCss("Grid>#resizebutton", "border", obj, ["normal"]);
    this._addCss("Grid>#controlcheckbox", "border", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#dropbutton", "border", obj, ["normal", "mouseover", "pushed", "focused", "disabled"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#prevbutton", "border", obj, ["mouseover", "selected", "focused", "pushed"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#nextbutton", "border", obj, ["mouseover", "selected", "focused", "pushed"]);
    this._addCss("GridControl>#summ", "border", obj, ["normal"]);
    this._addCss("GridControl>#resizebutton", "border", obj, ["normal"]);
    this._addCss("GridControl>#controlcheckbox", "border", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#dropbutton", "border", obj, ["normal", "mouseover", "pushed", "focused", "disabled"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#prevbutton", "border", obj, ["mouseover", "selected", "focused", "pushed"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#nextbutton", "border", obj, ["mouseover", "selected", "focused", "pushed"]);
    this._addCss("Tab>#extrabutton", "border", obj, ["normal", "mouseover", "focused", "selected", "pushed", "disabled"]);
    this._addCss("TabControl>#extrabutton", "border", obj, ["normal", "mouseover", "focused", "selected", "pushed", "disabled"]);
    this._addCss("FileUpload", "border", obj, ["normal"]);
    this._addCss("FileUploadControl", "border", obj, ["normal"]);

    obj = new nexacro.Style_color("#999999");
    this._addCss("MainFrame", "color", obj, ["normal"]);
    this._addCss("ChildFrame", "color", obj, ["normal"]);
    this._addCss("Static", "color", obj, ["normal"]);
    this._addCss("Button", "color", obj, ["disabled"]);
    this._addCss("ButtonControl", "color", obj, ["disabled"]);
    this._addCss("Grid>#controlbutton", "color", obj, ["disabled"]);
    this._addCss("GridControl>#controlbutton", "color", obj, ["disabled"]);
    this._addCss("Tab>#spinupbutton", "color", obj, ["disabled"]);
    this._addCss("Tab>#spindownbutton", "color", obj, ["disabled"]);
    this._addCss("TabControl>#spinupbutton", "color", obj, ["disabled"]);
    this._addCss("TabControl>#spindownbutton", "color", obj, ["disabled"]);
    this._addCss("Menu", "color", obj, ["normal"]);
    this._addCss("MenuControl", "color", obj, ["normal"]);

    obj = new nexacro.Style_font("9 Dotum");
    this._addCss("MainFrame", "font", obj, ["normal"]);
    this._addCss("ChildFrame", "font", obj, ["normal"]);
    this._addCss("TitleBarControl", "font", obj, ["normal"]);
    this._addCss("Form", "font", obj, ["normal"]);
    this._addCss("Spin", "font", obj, ["normal", "readonly"]);
    this._addCss("SpinControl", "font", obj, ["normal", "readonly"]);
    this._addCss("ListBox", "font", obj, ["normal"]);
    this._addCss("ListBoxControl", "font", obj, ["normal"]);
    this._addCss("Combo", "font", obj, ["normal", "readonly"]);
    this._addCss("Combo>#comboedit", "font", obj, ["normal", "mouseover", "focused", "readonly"]);
    this._addCss("ComboControl", "font", obj, ["normal", "readonly"]);
    this._addCss("ComboControl>#comboedit", "font", obj, ["normal", "mouseover", "focused", "readonly"]);
    this._addCss("Radio", "font", obj, ["normal"]);
    this._addCss("RadioControl", "font", obj, ["normal"]);
    this._addCss("CheckBox", "font", obj, ["normal"]);
    this._addCss("CheckBoxControl", "font", obj, ["normal"]);
    this._addCss("DatePicker", "font", obj, ["normal"]);
    this._addCss("DatePickerControl", "font", obj, ["normal"]);
    this._addCss("Calendar", "font", obj, ["normal"]);
    this._addCss("Calendar>#calendaredit", "font", obj, ["normal"]);
    this._addCss("CalendarControl", "font", obj, ["normal"]);
    this._addCss("CalendarControl>#calendaredit", "font", obj, ["normal"]);
    this._addCss("Grid>#controlcombo", "font", obj, ["normal", "readonly"]);
    this._addCss("Grid>#controlcombo>#comboedit", "font", obj, ["normal", "mouseover", "readonly"]);
    this._addCss("Grid>#controlcheckbox", "font", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar", "font", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#calendaredit", "font", obj, ["normal"]);
    this._addCss("GridControl>#controlcombo", "font", obj, ["normal", "readonly"]);
    this._addCss("GridControl>#controlcombo>#comboedit", "font", obj, ["normal", "mouseover", "readonly"]);
    this._addCss("GridControl>#controlcheckbox", "font", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar", "font", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#calendaredit", "font", obj, ["normal"]);
    this._addCss("Tab", "font", obj, ["normal", "selected", "focused"]);
    this._addCss("TabControl", "font", obj, ["normal", "selected", "focused"]);
    this._addCss("PopupMenu", "font", obj, ["normal"]);
    this._addCss("PopupMenuControl", "font", obj, ["normal"]);
    this._addCss("Menu", "font", obj, ["normal"]);
    this._addCss("MenuControl", "font", obj, ["normal"]);
    this._addCss("FileUpload", "font", obj, ["normal"]);
    this._addCss("FileUploadControl", "font", obj, ["normal"]);

    obj = new nexacro.Style_gradation("");
    this._addCss("MainFrame", "gradation", obj, ["normal"]);
    this._addCss("ChildFrame", "gradation", obj, ["normal"]);
    this._addCss("TitleBarControl", "gradation", obj, ["normal"]);
    this._addCss("TitleBarControl>#minbutton", "gradation", obj, ["normal"]);
    this._addCss("TitleBarControl>#maxbutton", "gradation", obj, ["normal"]);
    this._addCss("TitleBarControl>#normalbutton", "gradation", obj, ["normal"]);
    this._addCss("TitleBarControl>#closebutton", "gradation", obj, ["normal"]);
    this._addCss("StatusBarControl", "gradation", obj, ["normal"]);
    this._addCss("StatusBarControl>#progressbar", "gradation", obj, ["normal"]);
    this._addCss("StatusBarControl>#zoomcombo", "gradation", obj, ["normal"]);
    this._addCss("StatusBarControl>#zoomcombo>#comboedit", "gradation", obj, ["normal"]);
    this._addCss("Form", "gradation", obj, ["normal"]);
    this._addCss("VScrollBar", "gradation", obj, ["normal"]);
    this._addCss("VScrollBar>#incbutton", "gradation", obj, ["normal"]);
    this._addCss("VScrollBar>#decbutton", "gradation", obj, ["normal"]);
    this._addCss("VScrollBarControl", "gradation", obj, ["normal"]);
    this._addCss("VScrollBarControl>#incbutton", "gradation", obj, ["normal"]);
    this._addCss("VScrollBarControl>#decbutton", "gradation", obj, ["normal"]);
    this._addCss("HScrollBar", "gradation", obj, ["normal"]);
    this._addCss("HScrollBar>#incbutton", "gradation", obj, ["normal"]);
    this._addCss("HScrollBar>#decbutton", "gradation", obj, ["normal"]);
    this._addCss("HScrollBarControl", "gradation", obj, ["normal"]);
    this._addCss("HScrollBarControl>#incbutton", "gradation", obj, ["normal"]);
    this._addCss("HScrollBarControl>#decbutton", "gradation", obj, ["normal"]);
    this._addCss("*>#resizebutton", "gradation", obj, ["normal"]);
    this._addCss("Static", "gradation", obj, ["normal"]);
    this._addCss("StaticControl", "gradation", obj, ["normal"]);
    this._addCss("Spin>#spinedit", "gradation", obj, ["normal"]);
    this._addCss("SpinControl>#spinedit", "gradation", obj, ["normal"]);
    this._addCss("Combo>#comboedit", "gradation", obj, ["normal", "mouseover", "focused", "readonly"]);
    this._addCss("ComboControl>#comboedit", "gradation", obj, ["normal", "mouseover", "focused", "readonly"]);
    this._addCss("Radio", "gradation", obj, ["normal"]);
    this._addCss("RadioControl", "gradation", obj, ["normal"]);
    this._addCss("CheckBox", "gradation", obj, ["normal"]);
    this._addCss("CheckBoxControl", "gradation", obj, ["normal"]);
    this._addCss("Div", "gradation", obj, ["normal"]);
    this._addCss("DivControl", "gradation", obj, ["normal"]);
    this._addCss("PopupDiv", "gradation", obj, ["normal"]);
    this._addCss("PopupDivControl", "gradation", obj, ["normal"]);
    this._addCss("Step", "gradation", obj, ["normal"]);
    this._addCss("StepControl", "gradation", obj, ["normal"]);
    this._addCss("ImageViewer", "gradation", obj, ["normal"]);
    this._addCss("ImageViewerControl", "gradation", obj, ["normal"]);
    this._addCss("GroupBox", "gradation", obj, ["normal"]);
    this._addCss("GroupBoxControl", "gradation", obj, ["normal"]);
    this._addCss("DatePicker", "gradation", obj, ["normal"]);
    this._addCss("DatePickerControl", "gradation", obj, ["normal"]);
    this._addCss("Calendar", "gradation", obj, ["normal"]);
    this._addCss("Calendar>#dropbutton", "gradation", obj, ["normal", "mouseover", "pushed", "focused"]);
    this._addCss("Calendar>#popupcalendar>#prevbutton", "gradation", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#nextbutton", "gradation", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#yearspin", "gradation", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#yearspin>#spinedit", "gradation", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#monthspin", "gradation", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#monthspin>#spinedit", "gradation", obj, ["normal"]);
    this._addCss("CalendarControl", "gradation", obj, ["normal"]);
    this._addCss("CalendarControl>#dropbutton", "gradation", obj, ["normal", "mouseover", "pushed", "focused"]);
    this._addCss("CalendarControl>#popupcalendar>#prevbutton", "gradation", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#nextbutton", "gradation", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#yearspin", "gradation", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#yearspin>#spinedit", "gradation", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#monthspin", "gradation", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#monthspin>#spinedit", "gradation", obj, ["normal"]);
    this._addCss("Grid", "gradation", obj, ["normal"]);
    this._addCss("Grid>#body", "gradation", obj, ["normal"]);
    this._addCss("Grid>#controlcombo>#comboedit", "gradation", obj, ["normal", "mouseover", "readonly"]);
    this._addCss("Grid>#controlcheckbox", "gradation", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar", "gradation", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#dropbutton", "gradation", obj, ["normal", "mouseover", "pushed", "focused"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#prevbutton", "gradation", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#nextbutton", "gradation", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#yearspin", "gradation", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#monthspin", "gradation", obj, ["normal"]);
    this._addCss("GridControl", "gradation", obj, ["normal"]);
    this._addCss("GridControl>#body", "gradation", obj, ["normal"]);
    this._addCss("GridControl>#controlcombo>#comboedit", "gradation", obj, ["normal", "mouseover", "readonly"]);
    this._addCss("GridControl>#controlcheckbox", "gradation", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar", "gradation", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#dropbutton", "gradation", obj, ["normal", "mouseover", "pushed", "focused"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#prevbutton", "gradation", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#nextbutton", "gradation", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#yearspin", "gradation", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#monthspin", "gradation", obj, ["normal"]);
    this._addCss("Tab", "gradation", obj, ["normal"]);
    this._addCss("TabControl", "gradation", obj, ["normal"]);
    this._addCss("Tabpage", "gradation", obj, ["normal"]);
    this._addCss("PopupMenu", "gradation", obj, ["normal"]);
    this._addCss("PopupMenuControl", "gradation", obj, ["normal"]);
    this._addCss("FileUpload", "gradation", obj, ["normal"]);
    this._addCss("FileUploadControl", "gradation", obj, ["normal"]);
    this._addCss("Tooltip", "gradation", obj, ["normal"]);

    obj = new nexacro.Style_value("");
    this._addCss("MainFrame", "menubarheight", obj, ["normal"]);

    obj = new nexacro.Style_value("");
    this._addCss("MainFrame", "opacity", obj, ["normal"]);
    this._addCss("ChildFrame", "opacity", obj, ["normal"]);
    this._addCss("TitleBarControl", "opacity", obj, ["normal"]);
    this._addCss("TitleBarControl>#minbutton", "opacity", obj, ["normal"]);
    this._addCss("TitleBarControl>#maxbutton", "opacity", obj, ["normal"]);
    this._addCss("TitleBarControl>#normalbutton", "opacity", obj, ["normal"]);
    this._addCss("TitleBarControl>#closebutton", "opacity", obj, ["normal"]);
    this._addCss("StatusBarControl", "opacity", obj, ["normal"]);
    this._addCss("StatusBarControl>#progressbar", "opacity", obj, ["normal"]);
    this._addCss("StatusBarControl>#zoomcombo", "opacity", obj, ["normal"]);
    this._addCss("StatusBarControl>#zoomcombo>#comboedit", "opacity", obj, ["normal"]);
    this._addCss("StatusBarControl>#zoomcombo>#dropbutton", "opacity", obj, ["normal"]);
    this._addCss("Form", "opacity", obj, ["normal"]);
    this._addCss("VScrollBar", "opacity", obj, ["normal", "disabled"]);
    this._addCss("VScrollBar>#incbutton", "opacity", obj, ["normal"]);
    this._addCss("VScrollBar>#decbutton", "opacity", obj, ["normal"]);
    this._addCss("VScrollBar>#trackbar", "opacity", obj, ["normal"]);
    this._addCss("VScrollBarControl", "opacity", obj, ["normal", "disabled"]);
    this._addCss("VScrollBarControl>#incbutton", "opacity", obj, ["normal"]);
    this._addCss("VScrollBarControl>#decbutton", "opacity", obj, ["normal"]);
    this._addCss("VScrollBarControl>#trackbar", "opacity", obj, ["normal"]);
    this._addCss("HScrollBar", "opacity", obj, ["normal", "disabled"]);
    this._addCss("HScrollBar>#incbutton", "opacity", obj, ["normal"]);
    this._addCss("HScrollBar>#decbutton", "opacity", obj, ["normal"]);
    this._addCss("HScrollBar>#trackbar", "opacity", obj, ["normal"]);
    this._addCss("HScrollBarControl", "opacity", obj, ["normal", "disabled"]);
    this._addCss("HScrollBarControl>#incbutton", "opacity", obj, ["normal"]);
    this._addCss("HScrollBarControl>#decbutton", "opacity", obj, ["normal"]);
    this._addCss("*>#resizebutton", "opacity", obj, ["normal"]);
    this._addCss("Static", "opacity", obj, ["normal"]);
    this._addCss("StaticControl", "opacity", obj, ["normal", "disabled"]);
    this._addCss("Edit", "opacity", obj, ["normal", "readonly"]);
    this._addCss("EditControl", "opacity", obj, ["normal", "readonly"]);
    this._addCss("TextArea", "opacity", obj, ["normal", "readonly"]);
    this._addCss("TextAreaControl", "opacity", obj, ["normal", "readonly"]);
    this._addCss("MaskEdit", "opacity", obj, ["normal", "readonly"]);
    this._addCss("MaskEditControl", "opacity", obj, ["normal", "readonly"]);
    this._addCss("Button", "opacity", obj, ["normal"]);
    this._addCss("ButtonControl", "opacity", obj, ["normal"]);
    this._addCss("Spin", "opacity", obj, ["normal", "readonly"]);
    this._addCss("Spin>#spinedit", "opacity", obj, ["normal"]);
    this._addCss("Spin>#spinupbutton", "opacity", obj, ["normal"]);
    this._addCss("Spin>#spindownbutton", "opacity", obj, ["normal"]);
    this._addCss("SpinControl", "opacity", obj, ["normal", "readonly"]);
    this._addCss("SpinControl>#spinedit", "opacity", obj, ["normal"]);
    this._addCss("SpinControl>#spinupbutton", "opacity", obj, ["normal"]);
    this._addCss("SpinControl>#spindownbutton", "opacity", obj, ["normal"]);
    this._addCss("ListBox", "opacity", obj, ["normal"]);
    this._addCss("ListBoxControl", "opacity", obj, ["normal"]);
    this._addCss("Combo", "opacity", obj, ["normal", "readonly"]);
    this._addCss("Combo>#comboedit", "opacity", obj, ["normal", "mouseover", "focused", "readonly"]);
    this._addCss("Combo>#dropbutton", "opacity", obj, ["normal"]);
    this._addCss("ComboControl", "opacity", obj, ["normal", "readonly"]);
    this._addCss("ComboControl>#comboedit", "opacity", obj, ["normal", "mouseover", "focused", "readonly"]);
    this._addCss("ComboControl>#dropbutton", "opacity", obj, ["normal"]);
    this._addCss("Radio", "opacity", obj, ["normal"]);
    this._addCss("RadioControl", "opacity", obj, ["normal"]);
    this._addCss("CheckBox", "opacity", obj, ["normal"]);
    this._addCss("CheckBoxControl", "opacity", obj, ["normal"]);
    this._addCss("Div", "opacity", obj, ["normal"]);
    this._addCss("DivControl", "opacity", obj, ["normal"]);
    this._addCss("PopupDiv", "opacity", obj, ["normal"]);
    this._addCss("PopupDivControl", "opacity", obj, ["normal"]);
    this._addCss("Step", "opacity", obj, ["normal"]);
    this._addCss("StepControl", "opacity", obj, ["normal"]);
    this._addCss("ImageViewer", "opacity", obj, ["normal"]);
    this._addCss("ImageViewerControl", "opacity", obj, ["normal"]);
    this._addCss("GroupBox", "opacity", obj, ["normal"]);
    this._addCss("GroupBoxControl", "opacity", obj, ["normal"]);
    this._addCss("DatePicker", "opacity", obj, ["normal"]);
    this._addCss("DatePickerControl", "opacity", obj, ["normal"]);
    this._addCss("Calendar", "opacity", obj, ["normal"]);
    this._addCss("Calendar>#calendaredit", "opacity", obj, ["normal"]);
    this._addCss("Calendar>#dropbutton", "opacity", obj, ["normal", "mouseover", "pushed", "focused"]);
    this._addCss("Calendar>#spinupbutton", "opacity", obj, ["normal"]);
    this._addCss("Calendar>#spindownbutton", "opacity", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#prevbutton", "opacity", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#nextbutton", "opacity", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#yearspin", "opacity", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#yearspin>#spinedit", "opacity", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#monthspin", "opacity", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#monthspin>#spinedit", "opacity", obj, ["normal"]);
    this._addCss("CalendarControl", "opacity", obj, ["normal"]);
    this._addCss("CalendarControl>#calendaredit", "opacity", obj, ["normal"]);
    this._addCss("CalendarControl>#dropbutton", "opacity", obj, ["normal", "mouseover", "pushed", "focused"]);
    this._addCss("CalendarControl>#spinupbutton", "opacity", obj, ["normal"]);
    this._addCss("CalendarControl>#spindownbutton", "opacity", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#prevbutton", "opacity", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#nextbutton", "opacity", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#yearspin", "opacity", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#yearspin>#spinedit", "opacity", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#monthspin", "opacity", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#monthspin>#spinedit", "opacity", obj, ["normal"]);
    this._addCss("Grid>#controledit", "opacity", obj, ["normal"]);
    this._addCss("Grid>#controltextarea", "opacity", obj, ["normal", "readonly"]);
    this._addCss("Grid>#controlmaskedit", "opacity", obj, ["normal", "readonly"]);
    this._addCss("Grid>#controlbutton", "opacity", obj, ["normal"]);
    this._addCss("Grid>#controlcombo", "opacity", obj, ["normal", "readonly"]);
    this._addCss("Grid>#controlcombo>#comboedit", "opacity", obj, ["normal", "mouseover", "readonly"]);
    this._addCss("Grid>#controlcombo>#dropbutton", "opacity", obj, ["normal"]);
    this._addCss("Grid>#controlcheckbox", "opacity", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar", "opacity", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#calendaredit", "opacity", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#dropbutton", "opacity", obj, ["normal", "mouseover", "pushed", "focused"]);
    this._addCss("Grid>#controlcalendar>#spinupbutton", "opacity", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#spindownbutton", "opacity", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#prevbutton", "opacity", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#nextbutton", "opacity", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#yearspin", "opacity", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#monthspin", "opacity", obj, ["normal"]);
    this._addCss("GridControl>#controledit", "opacity", obj, ["normal"]);
    this._addCss("GridControl>#controltextarea", "opacity", obj, ["normal", "readonly"]);
    this._addCss("GridControl>#controlmaskedit", "opacity", obj, ["normal", "readonly"]);
    this._addCss("GridControl>#controlbutton", "opacity", obj, ["normal"]);
    this._addCss("GridControl>#controlcombo", "opacity", obj, ["normal", "readonly"]);
    this._addCss("GridControl>#controlcombo>#comboedit", "opacity", obj, ["normal", "mouseover", "readonly"]);
    this._addCss("GridControl>#controlcombo>#dropbutton", "opacity", obj, ["normal"]);
    this._addCss("GridControl>#controlcheckbox", "opacity", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar", "opacity", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#calendaredit", "opacity", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#dropbutton", "opacity", obj, ["normal", "mouseover", "pushed", "focused"]);
    this._addCss("GridControl>#controlcalendar>#spinupbutton", "opacity", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#spindownbutton", "opacity", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#prevbutton", "opacity", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#nextbutton", "opacity", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#yearspin", "opacity", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#monthspin", "opacity", obj, ["normal"]);
    this._addCss("Tab", "opacity", obj, ["normal"]);
    this._addCss("Tab>#spinupbutton", "opacity", obj, ["normal"]);
    this._addCss("Tab>#spindownbutton", "opacity", obj, ["normal"]);
    this._addCss("TabControl", "opacity", obj, ["normal"]);
    this._addCss("TabControl>#spinupbutton", "opacity", obj, ["normal"]);
    this._addCss("TabControl>#spindownbutton", "opacity", obj, ["normal"]);
    this._addCss("Tabpage", "opacity", obj, ["normal"]);
    this._addCss("PopupMenu", "opacity", obj, ["normal"]);
    this._addCss("PopupMenu>#spindownbutton", "opacity", obj, ["normal"]);
    this._addCss("PopupMenu>#spinupbutton", "opacity", obj, ["normal"]);
    this._addCss("PopupMenuControl", "opacity", obj, ["normal"]);
    this._addCss("PopupMenuControl>#spindownbutton", "opacity", obj, ["normal"]);
    this._addCss("PopupMenuControl>#spinupbutton", "opacity", obj, ["normal"]);
    this._addCss("Menu", "opacity", obj, ["normal"]);
    this._addCss("Menu>#spindownbutton", "opacity", obj, ["normal"]);
    this._addCss("Menu>#spinupbutton", "opacity", obj, ["normal"]);
    this._addCss("MenuControl", "opacity", obj, ["normal"]);
    this._addCss("MenuControl>#spindownbutton", "opacity", obj, ["normal"]);
    this._addCss("MenuControl>#spinupbutton", "opacity", obj, ["normal"]);
    this._addCss("ProgressBar", "opacity", obj, ["normal"]);
    this._addCss("ProgressBarControl", "opacity", obj, ["normal"]);
    this._addCss("FileUpload", "opacity", obj, ["normal"]);
    this._addCss("FileUploadControl", "opacity", obj, ["normal"]);
    this._addCss("FileDownload", "opacity", obj, ["normal"]);
    this._addCss("FileDownloadControl", "opacity", obj, ["normal"]);
    this._addCss("Tooltip", "opacity", obj, ["normal"]);

    obj = new nexacro.Style_value("");
    this._addCss("MainFrame", "openstatuseffect", obj, ["normal"]);

    obj = new nexacro.Style_value("26");
    this._addCss("MainFrame", "statusbarheight", obj, ["normal"]);

    obj = new nexacro.Style_value("24");
    this._addCss("MainFrame", "titlebarheight", obj, ["normal"]);

    obj = new nexacro.Style_border("");
    this._addCss("MainFrame", "border", obj, ["deactivate"]);
    this._addCss("TitleBarControl>#maxbutton", "border", obj, ["normal"]);
    this._addCss("VScrollBar>#incbutton", "border", obj, ["normal"]);
    this._addCss("VScrollBar>#decbutton", "border", obj, ["normal"]);
    this._addCss("VScrollBarControl>#incbutton", "border", obj, ["normal"]);
    this._addCss("VScrollBarControl>#decbutton", "border", obj, ["normal"]);
    this._addCss("HScrollBar>#incbutton", "border", obj, ["normal"]);
    this._addCss("HScrollBar>#decbutton", "border", obj, ["normal"]);
    this._addCss("HScrollBar>#trackbar", "border", obj, ["mouseover"]);
    this._addCss("HScrollBarControl>#incbutton", "border", obj, ["normal"]);
    this._addCss("HScrollBarControl>#decbutton", "border", obj, ["normal"]);
    this._addCss("StaticControl", "border", obj, ["normal"]);
    this._addCss("ImageViewer", "border", obj, ["mouseover", "focused"]);
    this._addCss("ImageViewerControl", "border", obj, ["mouseover", "focused"]);

    obj = new nexacro.Style_align("left middle");
    this._addCss("ChildFrame", "align", obj, ["normal"]);
    this._addCss("TitleBarControl", "align", obj, ["normal"]);
    this._addCss("StatusBarControl", "align", obj, ["normal"]);
    this._addCss("StatusBarControl>#zoomcombo", "align", obj, ["normal"]);
    this._addCss("StatusBarControl>#zoomcombo>#comboedit", "align", obj, ["normal"]);
    this._addCss("StatusBarControl>#zoomcombo>#combolist", "align", obj, ["normal"]);
    this._addCss("Static", "align", obj, ["normal"]);
    this._addCss("Edit", "align", obj, ["normal", "readonly"]);
    this._addCss("EditControl", "align", obj, ["normal", "readonly"]);
    this._addCss("Spin", "align", obj, ["normal", "readonly"]);
    this._addCss("Spin>#spinedit", "align", obj, ["normal"]);
    this._addCss("SpinControl", "align", obj, ["normal", "readonly"]);
    this._addCss("SpinControl>#spinedit", "align", obj, ["normal"]);
    this._addCss("ListBox", "align", obj, ["normal"]);
    this._addCss("ListBoxControl", "align", obj, ["normal"]);
    this._addCss("Combo", "align", obj, ["normal", "readonly"]);
    this._addCss("Combo>#comboedit", "align", obj, ["normal", "mouseover", "focused", "readonly"]);
    this._addCss("Combo>#combolist", "align", obj, ["normal"]);
    this._addCss("ComboControl", "align", obj, ["normal", "readonly"]);
    this._addCss("ComboControl>#comboedit", "align", obj, ["normal", "mouseover", "focused", "readonly"]);
    this._addCss("ComboControl>#combolist", "align", obj, ["normal"]);
    this._addCss("Radio", "align", obj, ["normal"]);
    this._addCss("RadioControl", "align", obj, ["normal"]);
    this._addCss("CheckBox", "align", obj, ["normal"]);
    this._addCss("CheckBoxControl", "align", obj, ["normal"]);
    this._addCss("Calendar", "align", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#yearspin", "align", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#yearspin>#spinedit", "align", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#monthspin", "align", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#monthspin>#spinedit", "align", obj, ["normal"]);
    this._addCss("CalendarControl", "align", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#yearspin", "align", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#yearspin>#spinedit", "align", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#monthspin", "align", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#monthspin>#spinedit", "align", obj, ["normal"]);
    this._addCss("Grid>#controledit", "align", obj, ["normal"]);
    this._addCss("Grid>#controlcombo", "align", obj, ["normal", "readonly"]);
    this._addCss("Grid>#controlcombo>#comboedit", "align", obj, ["normal", "mouseover", "readonly"]);
    this._addCss("Grid>#controlcombo>#combolist", "align", obj, ["normal"]);
    this._addCss("Grid>#controlcheckbox", "align", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar", "align", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#calendaredit", "align", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#yearspin", "align", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#monthspin", "align", obj, ["normal"]);
    this._addCss("GridControl>#controledit", "align", obj, ["normal"]);
    this._addCss("GridControl>#controlcombo", "align", obj, ["normal", "readonly"]);
    this._addCss("GridControl>#controlcombo>#comboedit", "align", obj, ["normal", "mouseover", "readonly"]);
    this._addCss("GridControl>#controlcombo>#combolist", "align", obj, ["normal"]);
    this._addCss("GridControl>#controlcheckbox", "align", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar", "align", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#yearspin", "align", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#monthspin", "align", obj, ["normal"]);
    this._addCss("FileUpload", "align", obj, ["normal"]);
    this._addCss("FileUploadControl", "align", obj, ["normal"]);

    obj = new nexacro.Style_bordertype("","0","0","true","true","true","true");
    this._addCss("ChildFrame", "bordertype", obj, ["normal"]);
    this._addCss("StatusBarControl>#progressbar", "bordertype", obj, ["normal"]);
    this._addCss("StatusBarControl>#zoomcombo", "bordertype", obj, ["normal"]);
    this._addCss("StatusBarControl>#zoomcombo>#comboedit", "bordertype", obj, ["normal"]);
    this._addCss("StatusBarControl>#zoomcombo>#dropbutton", "bordertype", obj, ["normal"]);
    this._addCss("Form", "bordertype", obj, ["normal"]);
    this._addCss("VScrollBar>#incbutton", "bordertype", obj, ["normal"]);
    this._addCss("VScrollBar>#decbutton", "bordertype", obj, ["normal"]);
    this._addCss("VScrollBarControl>#incbutton", "bordertype", obj, ["normal"]);
    this._addCss("VScrollBarControl>#decbutton", "bordertype", obj, ["normal"]);
    this._addCss("HScrollBar>#incbutton", "bordertype", obj, ["normal"]);
    this._addCss("HScrollBar>#decbutton", "bordertype", obj, ["normal"]);
    this._addCss("HScrollBarControl>#incbutton", "bordertype", obj, ["normal"]);
    this._addCss("HScrollBarControl>#decbutton", "bordertype", obj, ["normal"]);
    this._addCss("*>#resizebutton", "bordertype", obj, ["normal"]);
    this._addCss("Static", "bordertype", obj, ["normal"]);
    this._addCss("StaticControl", "bordertype", obj, ["normal"]);
    this._addCss("Spin>#spinedit", "bordertype", obj, ["normal"]);
    this._addCss("Spin>#spinupbutton", "bordertype", obj, ["normal"]);
    this._addCss("Spin>#spindownbutton", "bordertype", obj, ["normal"]);
    this._addCss("SpinControl>#spinedit", "bordertype", obj, ["normal"]);
    this._addCss("SpinControl>#spinupbutton", "bordertype", obj, ["normal"]);
    this._addCss("SpinControl>#spindownbutton", "bordertype", obj, ["normal"]);
    this._addCss("Combo>#dropbutton", "bordertype", obj, ["normal"]);
    this._addCss("ComboControl>#dropbutton", "bordertype", obj, ["normal"]);
    this._addCss("Radio", "bordertype", obj, ["normal"]);
    this._addCss("RadioControl", "bordertype", obj, ["normal"]);
    this._addCss("CheckBox", "bordertype", obj, ["normal"]);
    this._addCss("CheckBoxControl", "bordertype", obj, ["normal"]);
    this._addCss("Div", "bordertype", obj, ["normal"]);
    this._addCss("DivControl", "bordertype", obj, ["normal"]);
    this._addCss("ImageViewer", "bordertype", obj, ["normal"]);
    this._addCss("ImageViewerControl", "bordertype", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#prevbutton", "bordertype", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#nextbutton", "bordertype", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#yearspin>#spinedit", "bordertype", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#monthspin>#spinedit", "bordertype", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#prevbutton", "bordertype", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#nextbutton", "bordertype", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#yearspin>#spinedit", "bordertype", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#monthspin>#spinedit", "bordertype", obj, ["normal"]);
    this._addCss("Grid>#head", "bordertype", obj, ["normal"]);
    this._addCss("Grid>#body", "bordertype", obj, ["normal"]);
    this._addCss("Grid>#summ", "bordertype", obj, ["normal"]);
    this._addCss("Grid>#controlcombo>#dropbutton", "bordertype", obj, ["normal"]);
    this._addCss("Grid>#controlcheckbox", "bordertype", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#prevbutton", "bordertype", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#nextbutton", "bordertype", obj, ["normal"]);
    this._addCss("GridControl>#head", "bordertype", obj, ["normal"]);
    this._addCss("GridControl>#body", "bordertype", obj, ["normal"]);
    this._addCss("GridControl>#summ", "bordertype", obj, ["normal"]);
    this._addCss("GridControl>#controlcombo>#dropbutton", "bordertype", obj, ["normal"]);
    this._addCss("GridControl>#controlcheckbox", "bordertype", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#prevbutton", "bordertype", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#nextbutton", "bordertype", obj, ["normal"]);
    this._addCss("Menu", "bordertype", obj, ["normal"]);
    this._addCss("MenuControl", "bordertype", obj, ["normal"]);
    this._addCss("Tooltip", "bordertype", obj, ["normal"]);

    obj = new nexacro.Style_value("");
    this._addCss("ChildFrame", "icon", obj, ["normal"]);

    obj = new nexacro.Style_padding("");
    this._addCss("ChildFrame", "padding", obj, ["normal"]);
    this._addCss("TitleBarControl", "padding", obj, ["normal"]);
    this._addCss("TitleBarControl>#minbutton", "padding", obj, ["normal"]);
    this._addCss("TitleBarControl>#maxbutton", "padding", obj, ["normal"]);
    this._addCss("TitleBarControl>#normalbutton", "padding", obj, ["normal"]);
    this._addCss("TitleBarControl>#closebutton", "padding", obj, ["normal"]);
    this._addCss("StatusBarControl>#zoomcombo>#dropbutton", "padding", obj, ["normal"]);
    this._addCss("Form", "padding", obj, ["normal"]);
    this._addCss("VScrollBar", "padding", obj, ["normal"]);
    this._addCss("VScrollBar>#incbutton", "padding", obj, ["normal"]);
    this._addCss("VScrollBar>#decbutton", "padding", obj, ["normal"]);
    this._addCss("VScrollBar>#trackbar", "padding", obj, ["normal"]);
    this._addCss("VScrollBarControl", "padding", obj, ["normal"]);
    this._addCss("VScrollBarControl>#incbutton", "padding", obj, ["normal"]);
    this._addCss("VScrollBarControl>#decbutton", "padding", obj, ["normal"]);
    this._addCss("VScrollBarControl>#trackbar", "padding", obj, ["normal"]);
    this._addCss("HScrollBar", "padding", obj, ["normal"]);
    this._addCss("HScrollBar>#incbutton", "padding", obj, ["normal"]);
    this._addCss("HScrollBar>#decbutton", "padding", obj, ["normal"]);
    this._addCss("HScrollBar>#trackbar", "padding", obj, ["normal"]);
    this._addCss("HScrollBarControl", "padding", obj, ["normal"]);
    this._addCss("HScrollBarControl>#incbutton", "padding", obj, ["normal"]);
    this._addCss("HScrollBarControl>#decbutton", "padding", obj, ["normal"]);
    this._addCss("HScrollBarControl>#trackbar", "padding", obj, ["normal"]);
    this._addCss("*>#resizebutton", "padding", obj, ["normal"]);
    this._addCss("Static", "padding", obj, ["normal"]);
    this._addCss("StaticControl", "padding", obj, ["normal"]);
    this._addCss("TextArea", "padding", obj, ["normal", "readonly"]);
    this._addCss("TextAreaControl", "padding", obj, ["normal", "readonly"]);
    this._addCss("Button", "padding", obj, ["normal"]);
    this._addCss("ButtonControl", "padding", obj, ["normal"]);
    this._addCss("Spin", "padding", obj, ["normal", "readonly"]);
    this._addCss("Spin>#spinupbutton", "padding", obj, ["normal"]);
    this._addCss("Spin>#spindownbutton", "padding", obj, ["normal"]);
    this._addCss("SpinControl", "padding", obj, ["normal", "readonly"]);
    this._addCss("SpinControl>#spinupbutton", "padding", obj, ["normal"]);
    this._addCss("SpinControl>#spindownbutton", "padding", obj, ["normal"]);
    this._addCss("ListBox", "padding", obj, ["normal"]);
    this._addCss("ListBoxControl", "padding", obj, ["normal"]);
    this._addCss("Combo", "padding", obj, ["normal", "readonly"]);
    this._addCss("Combo>#dropbutton", "padding", obj, ["normal"]);
    this._addCss("ComboControl", "padding", obj, ["normal", "readonly"]);
    this._addCss("ComboControl>#dropbutton", "padding", obj, ["normal"]);
    this._addCss("Div", "padding", obj, ["normal"]);
    this._addCss("DivControl", "padding", obj, ["normal"]);
    this._addCss("ImageViewer", "padding", obj, ["normal"]);
    this._addCss("ImageViewerControl", "padding", obj, ["normal"]);
    this._addCss("Calendar", "padding", obj, ["normal"]);
    this._addCss("Calendar>#dropbutton", "padding", obj, ["normal", "mouseover", "pushed", "focused"]);
    this._addCss("Calendar>#spinupbutton", "padding", obj, ["normal"]);
    this._addCss("Calendar>#spindownbutton", "padding", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#prevbutton", "padding", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#nextbutton", "padding", obj, ["normal"]);
    this._addCss("CalendarControl", "padding", obj, ["normal"]);
    this._addCss("CalendarControl>#dropbutton", "padding", obj, ["normal", "mouseover", "pushed", "focused"]);
    this._addCss("CalendarControl>#spinupbutton", "padding", obj, ["normal"]);
    this._addCss("CalendarControl>#spindownbutton", "padding", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#prevbutton", "padding", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#nextbutton", "padding", obj, ["normal"]);
    this._addCss("Grid", "padding", obj, ["normal"]);
    this._addCss("Grid>#controltextarea", "padding", obj, ["normal", "readonly"]);
    this._addCss("Grid>#controlbutton", "padding", obj, ["normal"]);
    this._addCss("Grid>#controlcombo", "padding", obj, ["normal", "readonly"]);
    this._addCss("Grid>#controlcombo>#dropbutton", "padding", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar", "padding", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#dropbutton", "padding", obj, ["normal", "mouseover", "pushed", "focused"]);
    this._addCss("Grid>#controlcalendar>#spinupbutton", "padding", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#spindownbutton", "padding", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#prevbutton", "padding", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#nextbutton", "padding", obj, ["normal"]);
    this._addCss("GridControl", "padding", obj, ["normal"]);
    this._addCss("GridControl>#controltextarea", "padding", obj, ["normal", "readonly"]);
    this._addCss("GridControl>#controlbutton", "padding", obj, ["normal"]);
    this._addCss("GridControl>#controlcombo", "padding", obj, ["normal", "readonly"]);
    this._addCss("GridControl>#controlcombo>#dropbutton", "padding", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar", "padding", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#dropbutton", "padding", obj, ["normal", "mouseover", "pushed", "focused"]);
    this._addCss("GridControl>#controlcalendar>#spinupbutton", "padding", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#spindownbutton", "padding", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#prevbutton", "padding", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#nextbutton", "padding", obj, ["normal"]);
    this._addCss("Tab", "padding", obj, ["normal"]);
    this._addCss("Tab>#spinupbutton", "padding", obj, ["normal"]);
    this._addCss("Tab>#spindownbutton", "padding", obj, ["normal"]);
    this._addCss("TabControl", "padding", obj, ["normal"]);
    this._addCss("TabControl>#spinupbutton", "padding", obj, ["normal"]);
    this._addCss("TabControl>#spindownbutton", "padding", obj, ["normal"]);
    this._addCss("Tabpage", "padding", obj, ["normal"]);
    this._addCss("PopupMenu", "padding", obj, ["normal"]);
    this._addCss("PopupMenuControl", "padding", obj, ["normal"]);
    this._addCss("Menu", "padding", obj, ["normal"]);
    this._addCss("MenuControl", "padding", obj, ["normal"]);
    this._addCss("FileUpload", "padding", obj, ["normal"]);
    this._addCss("FileUploadControl", "padding", obj, ["normal"]);
    this._addCss("FileDownload", "padding", obj, ["normal"]);
    this._addCss("FileDownloadControl", "padding", obj, ["normal"]);

    obj = new nexacro.Style_value("");
    this._addCss("ChildFrame", "statusbarheight", obj, ["normal"]);

    obj = new nexacro.Style_value("");
    this._addCss("ChildFrame", "titlebarheight", obj, ["normal"]);

    obj = new nexacro.Style_background("","theme://images/B_Titlebar_bg.png","repeat-x","0","0","0","0","true");
    this._addCss("TitleBarControl", "background", obj, ["normal"]);

    obj = new nexacro.Style_border("0","none","#808080ff","");
    this._addCss("TitleBarControl", "border", obj, ["normal"]);
    this._addCss("TitleBarControl>#minbutton", "border", obj, ["mouseover", "pushed"]);
    this._addCss("TitleBarControl>#maxbutton", "border", obj, ["mouseover", "pushed"]);
    this._addCss("TitleBarControl>#normalbutton", "border", obj, ["normal", "mouseover", "pushed"]);
    this._addCss("TitleBarControl>#closebutton", "border", obj, ["normal", "mouseover", "pushed"]);
    this._addCss("Radio", "border", obj, ["normal"]);
    this._addCss("RadioControl", "border", obj, ["normal"]);
    this._addCss("Calendar", "border", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#prevbutton", "border", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#nextbutton", "border", obj, ["normal"]);
    this._addCss("CalendarControl", "border", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#prevbutton", "border", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#nextbutton", "border", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar", "border", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#prevbutton", "border", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#nextbutton", "border", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar", "border", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#prevbutton", "border", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#nextbutton", "border", obj, ["normal"]);
    this._addCss("Menu", "border", obj, ["normal"]);
    this._addCss("MenuControl", "border", obj, ["normal"]);

    obj = new nexacro.Style_bordertype("normal","0","0","true","true","true","true");
    this._addCss("TitleBarControl", "bordertype", obj, ["normal"]);
    this._addCss("TitleBarControl>#minbutton", "bordertype", obj, ["normal"]);
    this._addCss("TitleBarControl>#maxbutton", "bordertype", obj, ["normal"]);
    this._addCss("TitleBarControl>#normalbutton", "bordertype", obj, ["normal"]);
    this._addCss("TitleBarControl>#closebutton", "bordertype", obj, ["normal"]);
    this._addCss("Step", "bordertype", obj, ["normal"]);
    this._addCss("StepControl", "bordertype", obj, ["normal"]);
    this._addCss("DatePicker", "bordertype", obj, ["normal"]);
    this._addCss("DatePickerControl", "bordertype", obj, ["normal"]);
    this._addCss("Calendar", "bordertype", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#yearspin", "bordertype", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#monthspin", "bordertype", obj, ["normal"]);
    this._addCss("CalendarControl", "bordertype", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#yearspin", "bordertype", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#monthspin", "bordertype", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar", "bordertype", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#yearspin", "bordertype", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#monthspin", "bordertype", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar", "bordertype", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#yearspin", "bordertype", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#monthspin", "bordertype", obj, ["normal"]);
    this._addCss("Tab>#extrabutton", "bordertype", obj, ["normal", "mouseover", "focused", "selected", "pushed", "disabled"]);
    this._addCss("TabControl>#extrabutton", "bordertype", obj, ["normal", "mouseover", "focused", "selected", "pushed", "disabled"]);
    this._addCss("PopupMenu", "bordertype", obj, ["normal"]);
    this._addCss("PopupMenu>#spindownbutton", "bordertype", obj, ["normal"]);
    this._addCss("PopupMenu>#spinupbutton", "bordertype", obj, ["normal"]);
    this._addCss("PopupMenuControl", "bordertype", obj, ["normal"]);
    this._addCss("PopupMenuControl>#spindownbutton", "bordertype", obj, ["normal"]);
    this._addCss("PopupMenuControl>#spinupbutton", "bordertype", obj, ["normal"]);
    this._addCss("Menu>#spindownbutton", "bordertype", obj, ["normal"]);
    this._addCss("Menu>#spinupbutton", "bordertype", obj, ["normal"]);
    this._addCss("MenuControl>#spindownbutton", "bordertype", obj, ["normal"]);
    this._addCss("MenuControl>#spinupbutton", "bordertype", obj, ["normal"]);
    this._addCss("FileUpload", "bordertype", obj, ["normal"]);
    this._addCss("FileUploadControl", "bordertype", obj, ["normal"]);

    obj = new nexacro.Style_color("#c7c7c7");
    this._addCss("TitleBarControl", "color", obj, ["normal"]);

    obj = new nexacro.Style_value("");
    this._addCss("TitleBarControl", "cursor", obj, ["normal"]);
    this._addCss("TitleBarControl>#minbutton", "cursor", obj, ["normal"]);
    this._addCss("TitleBarControl>#maxbutton", "cursor", obj, ["normal"]);
    this._addCss("TitleBarControl>#normalbutton", "cursor", obj, ["normal"]);
    this._addCss("TitleBarControl>#closebutton", "cursor", obj, ["normal"]);
    this._addCss("StatusBarControl", "cursor", obj, ["normal"]);
    this._addCss("StatusBarControl>#progressbar", "cursor", obj, ["normal"]);
    this._addCss("StatusBarControl>#zoomcombo", "cursor", obj, ["normal"]);
    this._addCss("StatusBarControl>#zoomcombo>#comboedit", "cursor", obj, ["normal"]);
    this._addCss("StatusBarControl>#zoomcombo>#dropbutton", "cursor", obj, ["normal"]);
    this._addCss("Form", "cursor", obj, ["normal"]);
    this._addCss("VScrollBar", "cursor", obj, ["normal"]);
    this._addCss("VScrollBar>#incbutton", "cursor", obj, ["normal"]);
    this._addCss("VScrollBar>#decbutton", "cursor", obj, ["normal"]);
    this._addCss("VScrollBar>#trackbar", "cursor", obj, ["normal"]);
    this._addCss("VScrollBarControl", "cursor", obj, ["normal"]);
    this._addCss("VScrollBarControl>#incbutton", "cursor", obj, ["normal"]);
    this._addCss("VScrollBarControl>#decbutton", "cursor", obj, ["normal"]);
    this._addCss("VScrollBarControl>#trackbar", "cursor", obj, ["normal"]);
    this._addCss("HScrollBar", "cursor", obj, ["normal"]);
    this._addCss("HScrollBar>#incbutton", "cursor", obj, ["normal"]);
    this._addCss("HScrollBar>#decbutton", "cursor", obj, ["normal"]);
    this._addCss("HScrollBar>#trackbar", "cursor", obj, ["normal"]);
    this._addCss("HScrollBarControl", "cursor", obj, ["normal"]);
    this._addCss("HScrollBarControl>#incbutton", "cursor", obj, ["normal"]);
    this._addCss("HScrollBarControl>#decbutton", "cursor", obj, ["normal"]);
    this._addCss("*>#resizebutton", "cursor", obj, ["normal"]);
    this._addCss("Static", "cursor", obj, ["normal"]);
    this._addCss("StaticControl", "cursor", obj, ["normal"]);
    this._addCss("Edit", "cursor", obj, ["normal", "readonly"]);
    this._addCss("EditControl", "cursor", obj, ["normal", "readonly"]);
    this._addCss("TextArea", "cursor", obj, ["normal", "readonly"]);
    this._addCss("TextAreaControl", "cursor", obj, ["normal", "readonly"]);
    this._addCss("MaskEdit", "cursor", obj, ["normal", "readonly"]);
    this._addCss("MaskEditControl", "cursor", obj, ["normal", "readonly"]);
    this._addCss("Spin", "cursor", obj, ["normal", "readonly"]);
    this._addCss("Spin>#spinedit", "cursor", obj, ["normal"]);
    this._addCss("Spin>#spinupbutton", "cursor", obj, ["normal"]);
    this._addCss("Spin>#spindownbutton", "cursor", obj, ["normal"]);
    this._addCss("SpinControl", "cursor", obj, ["normal", "readonly"]);
    this._addCss("SpinControl>#spinedit", "cursor", obj, ["normal"]);
    this._addCss("SpinControl>#spinupbutton", "cursor", obj, ["normal"]);
    this._addCss("SpinControl>#spindownbutton", "cursor", obj, ["normal"]);
    this._addCss("ListBox", "cursor", obj, ["normal"]);
    this._addCss("ListBoxControl", "cursor", obj, ["normal"]);
    this._addCss("Combo", "cursor", obj, ["normal", "readonly"]);
    this._addCss("Combo>#comboedit", "cursor", obj, ["normal", "mouseover", "focused", "readonly"]);
    this._addCss("Combo>#dropbutton", "cursor", obj, ["normal"]);
    this._addCss("ComboControl", "cursor", obj, ["normal", "readonly"]);
    this._addCss("ComboControl>#comboedit", "cursor", obj, ["normal", "mouseover", "focused", "readonly"]);
    this._addCss("ComboControl>#dropbutton", "cursor", obj, ["normal"]);
    this._addCss("Radio", "cursor", obj, ["normal"]);
    this._addCss("RadioControl", "cursor", obj, ["normal"]);
    this._addCss("CheckBox", "cursor", obj, ["normal"]);
    this._addCss("CheckBoxControl", "cursor", obj, ["normal"]);
    this._addCss("Div", "cursor", obj, ["normal"]);
    this._addCss("DivControl", "cursor", obj, ["normal"]);
    this._addCss("PopupDiv", "cursor", obj, ["normal"]);
    this._addCss("PopupDivControl", "cursor", obj, ["normal"]);
    this._addCss("ImageViewer", "cursor", obj, ["normal"]);
    this._addCss("ImageViewerControl", "cursor", obj, ["normal"]);
    this._addCss("GroupBox", "cursor", obj, ["normal"]);
    this._addCss("GroupBoxControl", "cursor", obj, ["normal"]);
    this._addCss("DatePicker", "cursor", obj, ["normal"]);
    this._addCss("DatePickerControl", "cursor", obj, ["normal"]);
    this._addCss("Calendar", "cursor", obj, ["normal"]);
    this._addCss("Calendar>#calendaredit", "cursor", obj, ["normal"]);
    this._addCss("Calendar>#dropbutton", "cursor", obj, ["normal", "mouseover", "pushed", "focused"]);
    this._addCss("Calendar>#spinupbutton", "cursor", obj, ["normal"]);
    this._addCss("Calendar>#spindownbutton", "cursor", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#prevbutton", "cursor", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#nextbutton", "cursor", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#yearspin", "cursor", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#yearspin>#spinedit", "cursor", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#monthspin", "cursor", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#monthspin>#spinedit", "cursor", obj, ["normal"]);
    this._addCss("CalendarControl", "cursor", obj, ["normal"]);
    this._addCss("CalendarControl>#calendaredit", "cursor", obj, ["normal"]);
    this._addCss("CalendarControl>#dropbutton", "cursor", obj, ["normal", "mouseover", "pushed", "focused"]);
    this._addCss("CalendarControl>#spinupbutton", "cursor", obj, ["normal"]);
    this._addCss("CalendarControl>#spindownbutton", "cursor", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#prevbutton", "cursor", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#nextbutton", "cursor", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#yearspin", "cursor", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#yearspin>#spinedit", "cursor", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#monthspin", "cursor", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#monthspin>#spinedit", "cursor", obj, ["normal"]);
    this._addCss("Grid", "cursor", obj, ["normal"]);
    this._addCss("Grid>#controledit", "cursor", obj, ["normal"]);
    this._addCss("Grid>#controltextarea", "cursor", obj, ["normal", "readonly"]);
    this._addCss("Grid>#controlmaskedit", "cursor", obj, ["normal", "readonly"]);
    this._addCss("Grid>#controlcombo", "cursor", obj, ["normal", "readonly"]);
    this._addCss("Grid>#controlcombo>#comboedit", "cursor", obj, ["normal", "mouseover", "readonly"]);
    this._addCss("Grid>#controlcombo>#dropbutton", "cursor", obj, ["normal"]);
    this._addCss("Grid>#controlcheckbox", "cursor", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar", "cursor", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#calendaredit", "cursor", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#dropbutton", "cursor", obj, ["normal", "mouseover", "pushed", "focused"]);
    this._addCss("Grid>#controlcalendar>#spinupbutton", "cursor", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#spindownbutton", "cursor", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#prevbutton", "cursor", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#nextbutton", "cursor", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#yearspin", "cursor", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#monthspin", "cursor", obj, ["normal"]);
    this._addCss("GridControl", "cursor", obj, ["normal"]);
    this._addCss("GridControl>#controledit", "cursor", obj, ["normal"]);
    this._addCss("GridControl>#controltextarea", "cursor", obj, ["normal", "readonly"]);
    this._addCss("GridControl>#controlmaskedit", "cursor", obj, ["normal", "readonly"]);
    this._addCss("GridControl>#controlcombo", "cursor", obj, ["normal", "readonly"]);
    this._addCss("GridControl>#controlcombo>#comboedit", "cursor", obj, ["normal", "mouseover", "readonly"]);
    this._addCss("GridControl>#controlcombo>#dropbutton", "cursor", obj, ["normal"]);
    this._addCss("GridControl>#controlcheckbox", "cursor", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar", "cursor", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#calendaredit", "cursor", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#dropbutton", "cursor", obj, ["normal", "mouseover", "pushed", "focused"]);
    this._addCss("GridControl>#controlcalendar>#spinupbutton", "cursor", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#spindownbutton", "cursor", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#prevbutton", "cursor", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#nextbutton", "cursor", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#yearspin", "cursor", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#monthspin", "cursor", obj, ["normal"]);
    this._addCss("Tabpage", "cursor", obj, ["normal"]);
    this._addCss("PopupMenu", "cursor", obj, ["normal"]);
    this._addCss("PopupMenu>#spindownbutton", "cursor", obj, ["normal"]);
    this._addCss("PopupMenu>#spinupbutton", "cursor", obj, ["normal"]);
    this._addCss("PopupMenuControl", "cursor", obj, ["normal"]);
    this._addCss("PopupMenuControl>#spindownbutton", "cursor", obj, ["normal"]);
    this._addCss("PopupMenuControl>#spinupbutton", "cursor", obj, ["normal"]);
    this._addCss("Menu>#spindownbutton", "cursor", obj, ["normal"]);
    this._addCss("Menu>#spinupbutton", "cursor", obj, ["normal"]);
    this._addCss("MenuControl>#spindownbutton", "cursor", obj, ["normal"]);
    this._addCss("MenuControl>#spinupbutton", "cursor", obj, ["normal"]);
    this._addCss("ProgressBar", "cursor", obj, ["normal"]);
    this._addCss("ProgressBarControl", "cursor", obj, ["normal"]);
    this._addCss("FileUpload", "cursor", obj, ["normal"]);
    this._addCss("FileUploadControl", "cursor", obj, ["normal"]);
    this._addCss("FileDownload", "cursor", obj, ["normal"]);
    this._addCss("FileDownloadControl", "cursor", obj, ["normal"]);
    this._addCss("Tooltip", "cursor", obj, ["normal"]);

    obj = new nexacro.Style_value("theme://images/V13_logo.png");
    this._addCss("TitleBarControl", "icon", obj, ["normal"]);

    obj = new nexacro.Style_shadow("");
    this._addCss("TitleBarControl", "shadow", obj, ["normal"]);
    this._addCss("TitleBarControl>#minbutton", "shadow", obj, ["normal"]);
    this._addCss("TitleBarControl>#maxbutton", "shadow", obj, ["normal"]);
    this._addCss("TitleBarControl>#normalbutton", "shadow", obj, ["normal"]);
    this._addCss("TitleBarControl>#closebutton", "shadow", obj, ["normal"]);
    this._addCss("StatusBarControl", "shadow", obj, ["normal"]);
    this._addCss("StatusBarControl>#progressbar", "shadow", obj, ["normal"]);
    this._addCss("StatusBarControl>#zoomcombo", "shadow", obj, ["normal"]);
    this._addCss("StatusBarControl>#zoomcombo>#comboedit", "shadow", obj, ["normal"]);
    this._addCss("StatusBarControl>#zoomcombo>#dropbutton", "shadow", obj, ["normal"]);
    this._addCss("Form", "shadow", obj, ["normal"]);
    this._addCss("VScrollBar", "shadow", obj, ["normal"]);
    this._addCss("VScrollBar>#incbutton", "shadow", obj, ["normal"]);
    this._addCss("VScrollBar>#decbutton", "shadow", obj, ["normal"]);
    this._addCss("VScrollBarControl", "shadow", obj, ["normal"]);
    this._addCss("VScrollBarControl>#incbutton", "shadow", obj, ["normal"]);
    this._addCss("VScrollBarControl>#decbutton", "shadow", obj, ["normal"]);
    this._addCss("HScrollBar", "shadow", obj, ["normal"]);
    this._addCss("HScrollBar>#incbutton", "shadow", obj, ["normal"]);
    this._addCss("HScrollBar>#decbutton", "shadow", obj, ["normal"]);
    this._addCss("HScrollBar>#trackbar", "shadow", obj, ["normal"]);
    this._addCss("HScrollBarControl", "shadow", obj, ["normal"]);
    this._addCss("HScrollBarControl>#incbutton", "shadow", obj, ["normal"]);
    this._addCss("HScrollBarControl>#decbutton", "shadow", obj, ["normal"]);
    this._addCss("*>#resizebutton", "shadow", obj, ["normal"]);
    this._addCss("Static", "shadow", obj, ["normal"]);
    this._addCss("StaticControl", "shadow", obj, ["normal"]);
    this._addCss("Edit", "shadow", obj, ["normal", "readonly"]);
    this._addCss("EditControl", "shadow", obj, ["normal", "readonly"]);
    this._addCss("TextArea", "shadow", obj, ["normal", "readonly"]);
    this._addCss("TextAreaControl", "shadow", obj, ["normal", "readonly"]);
    this._addCss("MaskEdit", "shadow", obj, ["normal", "readonly"]);
    this._addCss("MaskEditControl", "shadow", obj, ["normal", "readonly"]);
    this._addCss("Spin", "shadow", obj, ["normal", "readonly"]);
    this._addCss("Spin>#spinedit", "shadow", obj, ["normal"]);
    this._addCss("Spin>#spinupbutton", "shadow", obj, ["normal"]);
    this._addCss("Spin>#spindownbutton", "shadow", obj, ["normal"]);
    this._addCss("SpinControl", "shadow", obj, ["normal", "readonly"]);
    this._addCss("SpinControl>#spinedit", "shadow", obj, ["normal"]);
    this._addCss("SpinControl>#spinupbutton", "shadow", obj, ["normal"]);
    this._addCss("SpinControl>#spindownbutton", "shadow", obj, ["normal"]);
    this._addCss("Combo", "shadow", obj, ["normal", "readonly"]);
    this._addCss("Combo>#comboedit", "shadow", obj, ["normal", "mouseover", "focused", "readonly"]);
    this._addCss("Combo>#dropbutton", "shadow", obj, ["normal"]);
    this._addCss("ComboControl", "shadow", obj, ["normal", "readonly"]);
    this._addCss("ComboControl>#comboedit", "shadow", obj, ["normal", "mouseover", "focused", "readonly"]);
    this._addCss("ComboControl>#dropbutton", "shadow", obj, ["normal"]);
    this._addCss("Radio", "shadow", obj, ["normal"]);
    this._addCss("RadioControl", "shadow", obj, ["normal"]);
    this._addCss("CheckBox", "shadow", obj, ["normal"]);
    this._addCss("CheckBoxControl", "shadow", obj, ["normal"]);
    this._addCss("Div", "shadow", obj, ["normal"]);
    this._addCss("DivControl", "shadow", obj, ["normal"]);
    this._addCss("Step", "shadow", obj, ["normal"]);
    this._addCss("StepControl", "shadow", obj, ["normal"]);
    this._addCss("ImageViewer", "shadow", obj, ["normal"]);
    this._addCss("ImageViewerControl", "shadow", obj, ["normal"]);
    this._addCss("GroupBox", "shadow", obj, ["normal"]);
    this._addCss("GroupBoxControl", "shadow", obj, ["normal"]);
    this._addCss("DatePicker", "shadow", obj, ["normal"]);
    this._addCss("DatePickerControl", "shadow", obj, ["normal"]);
    this._addCss("Calendar", "shadow", obj, ["normal"]);
    this._addCss("Calendar>#calendaredit", "shadow", obj, ["normal"]);
    this._addCss("Calendar>#dropbutton", "shadow", obj, ["normal", "mouseover", "pushed", "focused"]);
    this._addCss("Calendar>#spinupbutton", "shadow", obj, ["normal"]);
    this._addCss("Calendar>#spindownbutton", "shadow", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#prevbutton", "shadow", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#nextbutton", "shadow", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#yearspin", "shadow", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#yearspin>#spinedit", "shadow", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#monthspin", "shadow", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#monthspin>#spinedit", "shadow", obj, ["normal"]);
    this._addCss("CalendarControl", "shadow", obj, ["normal"]);
    this._addCss("CalendarControl>#calendaredit", "shadow", obj, ["normal"]);
    this._addCss("CalendarControl>#dropbutton", "shadow", obj, ["normal", "mouseover", "pushed", "focused"]);
    this._addCss("CalendarControl>#spinupbutton", "shadow", obj, ["normal"]);
    this._addCss("CalendarControl>#spindownbutton", "shadow", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#prevbutton", "shadow", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#nextbutton", "shadow", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#yearspin", "shadow", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#yearspin>#spinedit", "shadow", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#monthspin", "shadow", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#monthspin>#spinedit", "shadow", obj, ["normal"]);
    this._addCss("Grid", "shadow", obj, ["normal"]);
    this._addCss("Grid>#controledit", "shadow", obj, ["normal"]);
    this._addCss("Grid>#controltextarea", "shadow", obj, ["normal", "readonly"]);
    this._addCss("Grid>#controlmaskedit", "shadow", obj, ["normal", "readonly"]);
    this._addCss("Grid>#controlcombo", "shadow", obj, ["normal", "readonly"]);
    this._addCss("Grid>#controlcombo>#comboedit", "shadow", obj, ["normal", "mouseover", "readonly"]);
    this._addCss("Grid>#controlcombo>#dropbutton", "shadow", obj, ["normal"]);
    this._addCss("Grid>#controlcheckbox", "shadow", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar", "shadow", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#calendaredit", "shadow", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#dropbutton", "shadow", obj, ["normal", "mouseover", "pushed", "focused"]);
    this._addCss("Grid>#controlcalendar>#spinupbutton", "shadow", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#spindownbutton", "shadow", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#prevbutton", "shadow", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#nextbutton", "shadow", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#yearspin", "shadow", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#monthspin", "shadow", obj, ["normal"]);
    this._addCss("GridControl", "shadow", obj, ["normal"]);
    this._addCss("GridControl>#controledit", "shadow", obj, ["normal"]);
    this._addCss("GridControl>#controltextarea", "shadow", obj, ["normal", "readonly"]);
    this._addCss("GridControl>#controlmaskedit", "shadow", obj, ["normal", "readonly"]);
    this._addCss("GridControl>#controlcombo", "shadow", obj, ["normal", "readonly"]);
    this._addCss("GridControl>#controlcombo>#comboedit", "shadow", obj, ["normal", "mouseover", "readonly"]);
    this._addCss("GridControl>#controlcombo>#dropbutton", "shadow", obj, ["normal"]);
    this._addCss("GridControl>#controlcheckbox", "shadow", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar", "shadow", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#calendaredit", "shadow", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#dropbutton", "shadow", obj, ["normal", "mouseover", "pushed", "focused"]);
    this._addCss("GridControl>#controlcalendar>#spinupbutton", "shadow", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#spindownbutton", "shadow", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#prevbutton", "shadow", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#nextbutton", "shadow", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#yearspin", "shadow", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#monthspin", "shadow", obj, ["normal"]);
    this._addCss("Tab", "shadow", obj, ["normal"]);
    this._addCss("TabControl", "shadow", obj, ["normal"]);
    this._addCss("PopupMenu", "shadow", obj, ["normal"]);
    this._addCss("PopupMenu>#spindownbutton", "shadow", obj, ["normal"]);
    this._addCss("PopupMenu>#spinupbutton", "shadow", obj, ["normal"]);
    this._addCss("PopupMenuControl", "shadow", obj, ["normal"]);
    this._addCss("PopupMenuControl>#spindownbutton", "shadow", obj, ["normal"]);
    this._addCss("PopupMenuControl>#spinupbutton", "shadow", obj, ["normal"]);
    this._addCss("Menu", "shadow", obj, ["normal"]);
    this._addCss("Menu>#spindownbutton", "shadow", obj, ["normal"]);
    this._addCss("Menu>#spinupbutton", "shadow", obj, ["normal"]);
    this._addCss("MenuControl", "shadow", obj, ["normal"]);
    this._addCss("MenuControl>#spindownbutton", "shadow", obj, ["normal"]);
    this._addCss("MenuControl>#spinupbutton", "shadow", obj, ["normal"]);
    this._addCss("ProgressBar", "shadow", obj, ["normal"]);
    this._addCss("ProgressBarControl", "shadow", obj, ["normal"]);
    this._addCss("FileUpload", "shadow", obj, ["normal"]);
    this._addCss("FileUploadControl", "shadow", obj, ["normal"]);
    this._addCss("FileDownload", "shadow", obj, ["normal"]);
    this._addCss("FileDownloadControl", "shadow", obj, ["normal"]);
    this._addCss("Tooltip", "shadow", obj, ["normal"]);

    obj = new nexacro.Style_align("");
    this._addCss("TitleBarControl", "stepalign", obj, ["normal"]);
    this._addCss("StatusBarControl", "stepalign", obj, ["normal"]);
    this._addCss("Form", "stepalign", obj, ["normal"]);
    this._addCss("Div", "stepalign", obj, ["normal"]);
    this._addCss("DivControl", "stepalign", obj, ["normal"]);
    this._addCss("PopupDiv", "stepalign", obj, ["normal"]);
    this._addCss("PopupDivControl", "stepalign", obj, ["normal"]);
    this._addCss("Tab", "stepalign", obj, ["normal"]);
    this._addCss("TabControl", "stepalign", obj, ["normal"]);
    this._addCss("Tabpage", "stepalign", obj, ["normal"]);

    obj = new nexacro.Style_value("");
    this._addCss("TitleBarControl", "stepshowtype", obj, ["normal"]);
    this._addCss("StatusBarControl", "stepshowtype", obj, ["normal"]);
    this._addCss("Form", "stepshowtype", obj, ["normal"]);
    this._addCss("Div", "stepshowtype", obj, ["normal"]);
    this._addCss("DivControl", "stepshowtype", obj, ["normal"]);
    this._addCss("PopupDiv", "stepshowtype", obj, ["normal"]);
    this._addCss("PopupDivControl", "stepshowtype", obj, ["normal"]);
    this._addCss("Tab", "stepshowtype", obj, ["normal"]);
    this._addCss("TabControl", "stepshowtype", obj, ["normal"]);
    this._addCss("Tabpage", "stepshowtype", obj, ["normal"]);

    obj = new nexacro.Style_align("");
    this._addCss("TitleBarControl>#minbutton", "align", obj, ["normal"]);
    this._addCss("TitleBarControl>#maxbutton", "align", obj, ["normal"]);
    this._addCss("TitleBarControl>#normalbutton", "align", obj, ["normal"]);
    this._addCss("TitleBarControl>#closebutton", "align", obj, ["normal"]);
    this._addCss("StatusBarControl>#zoomcombo>#dropbutton", "align", obj, ["normal"]);
    this._addCss("Form", "align", obj, ["normal"]);
    this._addCss("VScrollBar", "align", obj, ["normal"]);
    this._addCss("VScrollBar>#incbutton", "align", obj, ["normal"]);
    this._addCss("VScrollBar>#decbutton", "align", obj, ["normal"]);
    this._addCss("VScrollBar>#trackbar", "align", obj, ["normal"]);
    this._addCss("VScrollBarControl", "align", obj, ["normal"]);
    this._addCss("VScrollBarControl>#incbutton", "align", obj, ["normal"]);
    this._addCss("VScrollBarControl>#decbutton", "align", obj, ["normal"]);
    this._addCss("VScrollBarControl>#trackbar", "align", obj, ["normal"]);
    this._addCss("HScrollBar", "align", obj, ["normal"]);
    this._addCss("HScrollBar>#incbutton", "align", obj, ["normal"]);
    this._addCss("HScrollBar>#decbutton", "align", obj, ["normal"]);
    this._addCss("HScrollBar>#trackbar", "align", obj, ["normal"]);
    this._addCss("HScrollBarControl", "align", obj, ["normal"]);
    this._addCss("HScrollBarControl>#incbutton", "align", obj, ["normal"]);
    this._addCss("HScrollBarControl>#decbutton", "align", obj, ["normal"]);
    this._addCss("HScrollBarControl>#trackbar", "align", obj, ["normal"]);
    this._addCss("*>#resizebutton", "align", obj, ["normal"]);
    this._addCss("StaticControl", "align", obj, ["normal"]);
    this._addCss("Spin>#spinupbutton", "align", obj, ["normal"]);
    this._addCss("Spin>#spindownbutton", "align", obj, ["normal"]);
    this._addCss("SpinControl>#spinupbutton", "align", obj, ["normal"]);
    this._addCss("SpinControl>#spindownbutton", "align", obj, ["normal"]);
    this._addCss("Combo>#dropbutton", "align", obj, ["normal"]);
    this._addCss("ComboControl>#dropbutton", "align", obj, ["normal"]);
    this._addCss("PopupDiv", "align", obj, ["normal"]);
    this._addCss("PopupDivControl", "align", obj, ["normal"]);
    this._addCss("ImageViewer", "align", obj, ["normal"]);
    this._addCss("ImageViewerControl", "align", obj, ["normal"]);
    this._addCss("Calendar>#calendaredit", "align", obj, ["normal"]);
    this._addCss("Calendar>#dropbutton", "align", obj, ["normal", "mouseover", "pushed", "focused"]);
    this._addCss("Calendar>#spinupbutton", "align", obj, ["normal"]);
    this._addCss("Calendar>#spindownbutton", "align", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#prevbutton", "align", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#nextbutton", "align", obj, ["normal"]);
    this._addCss("CalendarControl>#calendaredit", "align", obj, ["normal"]);
    this._addCss("CalendarControl>#dropbutton", "align", obj, ["normal", "mouseover", "pushed", "focused"]);
    this._addCss("CalendarControl>#spinupbutton", "align", obj, ["normal"]);
    this._addCss("CalendarControl>#spindownbutton", "align", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#prevbutton", "align", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#nextbutton", "align", obj, ["normal"]);
    this._addCss("Grid", "align", obj, ["normal"]);
    this._addCss("Grid>#controlcombo>#dropbutton", "align", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#dropbutton", "align", obj, ["normal", "mouseover", "pushed", "focused"]);
    this._addCss("Grid>#controlcalendar>#spinupbutton", "align", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#spindownbutton", "align", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#prevbutton", "align", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#nextbutton", "align", obj, ["normal"]);
    this._addCss("GridControl", "align", obj, ["normal"]);
    this._addCss("GridControl>#controlcombo>#dropbutton", "align", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#calendaredit", "align", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#dropbutton", "align", obj, ["normal", "mouseover", "pushed", "focused"]);
    this._addCss("GridControl>#controlcalendar>#spinupbutton", "align", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#spindownbutton", "align", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#prevbutton", "align", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#nextbutton", "align", obj, ["normal"]);
    this._addCss("Tooltip", "align", obj, ["normal"]);

    obj = new nexacro.Style_color("");
    this._addCss("TitleBarControl>#minbutton", "color", obj, ["normal"]);
    this._addCss("TitleBarControl>#maxbutton", "color", obj, ["normal"]);
    this._addCss("TitleBarControl>#normalbutton", "color", obj, ["normal"]);
    this._addCss("TitleBarControl>#closebutton", "color", obj, ["normal"]);
    this._addCss("StatusBarControl>#zoomcombo>#dropbutton", "color", obj, ["normal"]);
    this._addCss("Form", "color", obj, ["normal"]);
    this._addCss("VScrollBar", "color", obj, ["normal"]);
    this._addCss("VScrollBar>#incbutton", "color", obj, ["normal"]);
    this._addCss("VScrollBar>#decbutton", "color", obj, ["normal"]);
    this._addCss("VScrollBar>#trackbar", "color", obj, ["normal"]);
    this._addCss("VScrollBarControl", "color", obj, ["normal"]);
    this._addCss("VScrollBarControl>#incbutton", "color", obj, ["normal"]);
    this._addCss("VScrollBarControl>#decbutton", "color", obj, ["normal"]);
    this._addCss("VScrollBarControl>#trackbar", "color", obj, ["normal"]);
    this._addCss("HScrollBar", "color", obj, ["normal"]);
    this._addCss("HScrollBar>#incbutton", "color", obj, ["normal"]);
    this._addCss("HScrollBar>#decbutton", "color", obj, ["normal"]);
    this._addCss("HScrollBar>#trackbar", "color", obj, ["normal"]);
    this._addCss("HScrollBarControl", "color", obj, ["normal"]);
    this._addCss("HScrollBarControl>#incbutton", "color", obj, ["normal"]);
    this._addCss("HScrollBarControl>#decbutton", "color", obj, ["normal"]);
    this._addCss("*>#resizebutton", "color", obj, ["normal"]);
    this._addCss("StaticControl", "color", obj, ["normal"]);
    this._addCss("Spin>#spinupbutton", "color", obj, ["normal"]);
    this._addCss("Spin>#spindownbutton", "color", obj, ["normal"]);
    this._addCss("SpinControl>#spinupbutton", "color", obj, ["normal"]);
    this._addCss("SpinControl>#spindownbutton", "color", obj, ["normal"]);
    this._addCss("Combo>#dropbutton", "color", obj, ["normal"]);
    this._addCss("ComboControl>#dropbutton", "color", obj, ["normal"]);
    this._addCss("PopupDiv", "color", obj, ["normal"]);
    this._addCss("PopupDivControl", "color", obj, ["normal"]);
    this._addCss("ImageViewer", "color", obj, ["normal"]);
    this._addCss("ImageViewerControl", "color", obj, ["normal"]);
    this._addCss("Calendar>#dropbutton", "color", obj, ["normal", "mouseover", "pushed", "focused"]);
    this._addCss("Calendar>#spinupbutton", "color", obj, ["normal"]);
    this._addCss("Calendar>#spindownbutton", "color", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#prevbutton", "color", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#nextbutton", "color", obj, ["normal"]);
    this._addCss("CalendarControl>#dropbutton", "color", obj, ["normal", "mouseover", "pushed", "focused"]);
    this._addCss("CalendarControl>#spinupbutton", "color", obj, ["normal"]);
    this._addCss("CalendarControl>#spindownbutton", "color", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#prevbutton", "color", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#nextbutton", "color", obj, ["normal"]);
    this._addCss("Grid>#controlcombo>#dropbutton", "color", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#dropbutton", "color", obj, ["normal", "mouseover", "pushed", "focused"]);
    this._addCss("Grid>#controlcalendar>#spinupbutton", "color", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#spindownbutton", "color", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#prevbutton", "color", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#nextbutton", "color", obj, ["normal"]);
    this._addCss("GridControl>#controlcombo>#dropbutton", "color", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#dropbutton", "color", obj, ["normal", "mouseover", "pushed", "focused"]);
    this._addCss("GridControl>#controlcalendar>#spinupbutton", "color", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#spindownbutton", "color", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#prevbutton", "color", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#nextbutton", "color", obj, ["normal"]);
    this._addCss("PopupMenu", "color", obj, ["disabled"]);
    this._addCss("PopupMenu>#spindownbutton", "color", obj, ["normal"]);
    this._addCss("PopupMenu>#spinupbutton", "color", obj, ["normal"]);
    this._addCss("PopupMenuControl", "color", obj, ["disabled"]);
    this._addCss("PopupMenuControl>#spindownbutton", "color", obj, ["normal"]);
    this._addCss("PopupMenuControl>#spinupbutton", "color", obj, ["normal"]);
    this._addCss("Menu>#spindownbutton", "color", obj, ["normal"]);
    this._addCss("Menu>#spinupbutton", "color", obj, ["normal"]);
    this._addCss("MenuControl>#spindownbutton", "color", obj, ["normal"]);
    this._addCss("MenuControl>#spinupbutton", "color", obj, ["normal"]);
    this._addCss("FileDownload", "color", obj, ["normal"]);
    this._addCss("FileDownloadControl", "color", obj, ["normal"]);

    obj = new nexacro.Style_font("");
    this._addCss("TitleBarControl>#minbutton", "font", obj, ["normal"]);
    this._addCss("TitleBarControl>#maxbutton", "font", obj, ["normal"]);
    this._addCss("TitleBarControl>#normalbutton", "font", obj, ["normal"]);
    this._addCss("TitleBarControl>#closebutton", "font", obj, ["normal"]);
    this._addCss("StatusBarControl>#zoomcombo>#dropbutton", "font", obj, ["normal"]);
    this._addCss("VScrollBar", "font", obj, ["normal"]);
    this._addCss("VScrollBar>#incbutton", "font", obj, ["normal"]);
    this._addCss("VScrollBar>#decbutton", "font", obj, ["normal"]);
    this._addCss("VScrollBar>#trackbar", "font", obj, ["normal"]);
    this._addCss("VScrollBarControl", "font", obj, ["normal"]);
    this._addCss("VScrollBarControl>#incbutton", "font", obj, ["normal"]);
    this._addCss("VScrollBarControl>#decbutton", "font", obj, ["normal"]);
    this._addCss("VScrollBarControl>#trackbar", "font", obj, ["normal"]);
    this._addCss("HScrollBar", "font", obj, ["normal"]);
    this._addCss("HScrollBar>#incbutton", "font", obj, ["normal"]);
    this._addCss("HScrollBar>#decbutton", "font", obj, ["normal"]);
    this._addCss("HScrollBar>#trackbar", "font", obj, ["normal"]);
    this._addCss("HScrollBarControl", "font", obj, ["normal"]);
    this._addCss("HScrollBarControl>#incbutton", "font", obj, ["normal"]);
    this._addCss("HScrollBarControl>#decbutton", "font", obj, ["normal"]);
    this._addCss("*>#resizebutton", "font", obj, ["normal"]);
    this._addCss("Static", "font", obj, ["normal"]);
    this._addCss("StaticControl", "font", obj, ["normal"]);
    this._addCss("Edit", "font", obj, ["normal", "readonly"]);
    this._addCss("EditControl", "font", obj, ["normal", "readonly"]);
    this._addCss("TextArea", "font", obj, ["normal", "readonly"]);
    this._addCss("TextAreaControl", "font", obj, ["normal", "readonly"]);
    this._addCss("MaskEdit", "font", obj, ["normal", "readonly"]);
    this._addCss("MaskEditControl", "font", obj, ["normal", "readonly"]);
    this._addCss("Button", "font", obj, ["normal"]);
    this._addCss("ButtonControl", "font", obj, ["normal"]);
    this._addCss("Spin>#spinedit", "font", obj, ["normal"]);
    this._addCss("Spin>#spinupbutton", "font", obj, ["normal"]);
    this._addCss("Spin>#spindownbutton", "font", obj, ["normal"]);
    this._addCss("SpinControl>#spinedit", "font", obj, ["normal"]);
    this._addCss("SpinControl>#spinupbutton", "font", obj, ["normal"]);
    this._addCss("SpinControl>#spindownbutton", "font", obj, ["normal"]);
    this._addCss("Combo>#dropbutton", "font", obj, ["normal"]);
    this._addCss("ComboControl>#dropbutton", "font", obj, ["normal"]);
    this._addCss("Div", "font", obj, ["normal"]);
    this._addCss("DivControl", "font", obj, ["normal"]);
    this._addCss("PopupDiv", "font", obj, ["normal"]);
    this._addCss("PopupDivControl", "font", obj, ["normal"]);
    this._addCss("Step", "font", obj, ["normal"]);
    this._addCss("StepControl", "font", obj, ["normal"]);
    this._addCss("ImageViewer", "font", obj, ["normal"]);
    this._addCss("ImageViewerControl", "font", obj, ["normal"]);
    this._addCss("GroupBox", "font", obj, ["normal"]);
    this._addCss("GroupBoxControl", "font", obj, ["normal"]);
    this._addCss("Calendar>#dropbutton", "font", obj, ["normal", "mouseover", "pushed", "focused"]);
    this._addCss("Calendar>#spinupbutton", "font", obj, ["normal"]);
    this._addCss("Calendar>#spindownbutton", "font", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#prevbutton", "font", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#nextbutton", "font", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#yearspin", "font", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#yearspin>#spinedit", "font", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#monthspin", "font", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#monthspin>#spinedit", "font", obj, ["normal"]);
    this._addCss("CalendarControl>#dropbutton", "font", obj, ["normal", "mouseover", "pushed", "focused"]);
    this._addCss("CalendarControl>#spinupbutton", "font", obj, ["normal"]);
    this._addCss("CalendarControl>#spindownbutton", "font", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#prevbutton", "font", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#nextbutton", "font", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#yearspin", "font", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#yearspin>#spinedit", "font", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#monthspin", "font", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#monthspin>#spinedit", "font", obj, ["normal"]);
    this._addCss("Grid>#controledit", "font", obj, ["normal"]);
    this._addCss("Grid>#controltextarea", "font", obj, ["normal", "readonly"]);
    this._addCss("Grid>#controlmaskedit", "font", obj, ["normal", "readonly"]);
    this._addCss("Grid>#controlbutton", "font", obj, ["normal"]);
    this._addCss("Grid>#controlcombo>#dropbutton", "font", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#dropbutton", "font", obj, ["normal", "mouseover", "pushed", "focused"]);
    this._addCss("Grid>#controlcalendar>#spinupbutton", "font", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#spindownbutton", "font", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#prevbutton", "font", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#nextbutton", "font", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#yearspin", "font", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#monthspin", "font", obj, ["normal"]);
    this._addCss("GridControl>#controledit", "font", obj, ["normal"]);
    this._addCss("GridControl>#controltextarea", "font", obj, ["normal", "readonly"]);
    this._addCss("GridControl>#controlmaskedit", "font", obj, ["normal", "readonly"]);
    this._addCss("GridControl>#controlbutton", "font", obj, ["normal"]);
    this._addCss("GridControl>#controlcombo>#dropbutton", "font", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#dropbutton", "font", obj, ["normal", "mouseover", "pushed", "focused"]);
    this._addCss("GridControl>#controlcalendar>#spinupbutton", "font", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#spindownbutton", "font", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#prevbutton", "font", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#nextbutton", "font", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#yearspin", "font", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#monthspin", "font", obj, ["normal"]);
    this._addCss("Tab>#spinupbutton", "font", obj, ["normal"]);
    this._addCss("Tab>#spindownbutton", "font", obj, ["normal"]);
    this._addCss("TabControl>#spinupbutton", "font", obj, ["normal"]);
    this._addCss("TabControl>#spindownbutton", "font", obj, ["normal"]);
    this._addCss("PopupMenu>#spindownbutton", "font", obj, ["normal"]);
    this._addCss("PopupMenu>#spinupbutton", "font", obj, ["normal"]);
    this._addCss("PopupMenuControl>#spindownbutton", "font", obj, ["normal"]);
    this._addCss("PopupMenuControl>#spinupbutton", "font", obj, ["normal"]);
    this._addCss("Menu>#spindownbutton", "font", obj, ["normal"]);
    this._addCss("Menu>#spinupbutton", "font", obj, ["normal"]);
    this._addCss("MenuControl>#spindownbutton", "font", obj, ["normal"]);
    this._addCss("MenuControl>#spinupbutton", "font", obj, ["normal"]);
    this._addCss("ProgressBar", "font", obj, ["normal"]);
    this._addCss("ProgressBarControl", "font", obj, ["normal"]);
    this._addCss("FileDownload", "font", obj, ["normal"]);
    this._addCss("FileDownloadControl", "font", obj, ["normal"]);
    this._addCss("Tooltip", "font", obj, ["normal"]);

    obj = new nexacro.Style_value("theme://images/B_btn_Titlebar_min.png");
    this._addCss("TitleBarControl>#minbutton", "image", obj, ["normal"]);

    obj = new nexacro.Style_align("");
    this._addCss("TitleBarControl>#minbutton", "imagealign", obj, ["normal"]);
    this._addCss("TitleBarControl>#maxbutton", "imagealign", obj, ["normal"]);
    this._addCss("TitleBarControl>#normalbutton", "imagealign", obj, ["normal"]);
    this._addCss("TitleBarControl>#closebutton", "imagealign", obj, ["normal"]);
    this._addCss("VScrollBar>#incbutton", "imagealign", obj, ["normal"]);
    this._addCss("VScrollBar>#decbutton", "imagealign", obj, ["normal"]);
    this._addCss("VScrollBarControl>#incbutton", "imagealign", obj, ["normal"]);
    this._addCss("VScrollBarControl>#decbutton", "imagealign", obj, ["normal"]);
    this._addCss("HScrollBar>#incbutton", "imagealign", obj, ["normal"]);
    this._addCss("HScrollBar>#decbutton", "imagealign", obj, ["normal"]);
    this._addCss("HScrollBarControl>#incbutton", "imagealign", obj, ["normal"]);
    this._addCss("HScrollBarControl>#decbutton", "imagealign", obj, ["normal"]);
    this._addCss("*>#resizebutton", "imagealign", obj, ["normal"]);
    this._addCss("Button", "imagealign", obj, ["normal"]);
    this._addCss("ButtonControl", "imagealign", obj, ["normal"]);
    this._addCss("Combo>#dropbutton", "imagealign", obj, ["normal"]);
    this._addCss("ComboControl>#dropbutton", "imagealign", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#prevbutton", "imagealign", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#nextbutton", "imagealign", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#prevbutton", "imagealign", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#nextbutton", "imagealign", obj, ["normal"]);
    this._addCss("Grid>#controlbutton", "imagealign", obj, ["normal"]);
    this._addCss("Grid>#controlcombo>#dropbutton", "imagealign", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#prevbutton", "imagealign", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#nextbutton", "imagealign", obj, ["normal"]);
    this._addCss("GridControl>#controlbutton", "imagealign", obj, ["normal"]);
    this._addCss("GridControl>#controlcombo>#dropbutton", "imagealign", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#prevbutton", "imagealign", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#nextbutton", "imagealign", obj, ["normal"]);

    obj = new nexacro.Style_value("theme://images/B_btn_Titlebar_min_O.png");
    this._addCss("TitleBarControl>#minbutton", "image", obj, ["mouseover"]);

    obj = new nexacro.Style_value("theme://images/B_btn_Titlebar_min_P.png");
    this._addCss("TitleBarControl>#minbutton", "image", obj, ["pushed"]);

    obj = new nexacro.Style_value("theme://images/B_btn_Titlebar_max.png");
    this._addCss("TitleBarControl>#maxbutton", "image", obj, ["normal"]);

    obj = new nexacro.Style_value("theme://images/B_btn_Titlebar_max_O.png");
    this._addCss("TitleBarControl>#maxbutton", "image", obj, ["mouseover"]);

    obj = new nexacro.Style_value("theme://images/B_btn_Titlebar_max_P.png");
    this._addCss("TitleBarControl>#maxbutton", "image", obj, ["pushed"]);

    obj = new nexacro.Style_value("theme://images/B_btn_Titlebar_normal.png");
    this._addCss("TitleBarControl>#normalbutton", "image", obj, ["normal"]);

    obj = new nexacro.Style_value("theme://images/B_btn_Titlebar_normal_O.png");
    this._addCss("TitleBarControl>#normalbutton", "image", obj, ["mouseover"]);

    obj = new nexacro.Style_value("theme://images/B_btn_Titlebar_normal_P.png");
    this._addCss("TitleBarControl>#normalbutton", "image", obj, ["pushed"]);

    obj = new nexacro.Style_value("theme://images/B_btn_Titlebar_close.png");
    this._addCss("TitleBarControl>#closebutton", "image", obj, ["normal"]);

    obj = new nexacro.Style_value("theme://images/B_btn_Titlebar_close_O.png");
    this._addCss("TitleBarControl>#closebutton", "image", obj, ["mouseover"]);

    obj = new nexacro.Style_value("theme://images/B_btn_Titlebar_close_P.png");
    this._addCss("TitleBarControl>#closebutton", "image", obj, ["pushed"]);

    obj = new nexacro.Style_background("#f2f2efff","","","0","0","0","0","true");
    this._addCss("StatusBarControl", "background", obj, ["normal"]);

    obj = new nexacro.Style_border("1","solid","#a8a8a2ff","","0","none","","","0","none","","","0","none","","");
    this._addCss("StatusBarControl", "border", obj, ["normal"]);

    obj = new nexacro.Style_bordertype("round","3","3","false","false","true","true");
    this._addCss("StatusBarControl", "bordertype", obj, ["normal"]);

    obj = new nexacro.Style_color("#46463d");
    this._addCss("StatusBarControl", "color", obj, ["normal"]);

    obj = new nexacro.Style_font("Dotum,9");
    this._addCss("StatusBarControl", "font", obj, ["normal"]);
    this._addCss("StatusBarControl>#zoomcombo", "font", obj, ["normal"]);
    this._addCss("StatusBarControl>#zoomcombo>#comboedit", "font", obj, ["normal"]);
    this._addCss("StatusBarControl>#zoomcombo>#combolist", "font", obj, ["normal"]);

    obj = new nexacro.Style_value("theme://images/img_grip.png");
    this._addCss("StatusBarControl", "gripimage", obj, ["normal"]);

    obj = new nexacro.Style_padding("0 0 0 12");
    this._addCss("StatusBarControl", "padding", obj, ["normal"]);

    obj = new nexacro.Style_value("5");
    this._addCss("StatusBarControl", "progressbargap", obj, ["normal"]);

    obj = new nexacro.Style_value("21");
    this._addCss("StatusBarControl", "progressbarheight", obj, ["normal"]);

    obj = new nexacro.Style_value("260");
    this._addCss("StatusBarControl", "progressbarwidth", obj, ["normal"]);

    obj = new nexacro.Style_value("10");
    this._addCss("StatusBarControl", "zoomcombogap", obj, ["normal"]);

    obj = new nexacro.Style_value("21");
    this._addCss("StatusBarControl", "zoomcomboheight", obj, ["normal"]);

    obj = new nexacro.Style_value("80");
    this._addCss("StatusBarControl", "zoomcombowidth", obj, ["normal"]);

    obj = new nexacro.Style_align("center middle");
    this._addCss("StatusBarControl>#progressbar", "align", obj, ["normal"]);
    this._addCss("Button", "align", obj, ["normal"]);
    this._addCss("ButtonControl", "align", obj, ["normal"]);
    this._addCss("Div", "align", obj, ["normal"]);
    this._addCss("DivControl", "align", obj, ["normal"]);
    this._addCss("Step", "align", obj, ["normal"]);
    this._addCss("StepControl", "align", obj, ["normal"]);
    this._addCss("DatePicker", "align", obj, ["normal"]);
    this._addCss("DatePickerControl", "align", obj, ["normal"]);
    this._addCss("Grid>#controlbutton", "align", obj, ["normal"]);
    this._addCss("GridControl>#controlbutton", "align", obj, ["normal"]);
    this._addCss("Tab>#spinupbutton", "align", obj, ["normal"]);
    this._addCss("Tab>#spindownbutton", "align", obj, ["normal"]);
    this._addCss("Tab>#extrabutton", "align", obj, ["normal", "mouseover", "focused", "selected", "pushed", "disabled"]);
    this._addCss("TabControl>#spinupbutton", "align", obj, ["normal"]);
    this._addCss("TabControl>#spindownbutton", "align", obj, ["normal"]);
    this._addCss("TabControl>#extrabutton", "align", obj, ["normal", "mouseover", "focused", "selected", "pushed", "disabled"]);
    this._addCss("PopupMenu>#spindownbutton", "align", obj, ["normal"]);
    this._addCss("PopupMenu>#spinupbutton", "align", obj, ["normal"]);
    this._addCss("PopupMenuControl>#spindownbutton", "align", obj, ["normal"]);
    this._addCss("PopupMenuControl>#spinupbutton", "align", obj, ["normal"]);
    this._addCss("Menu>#spindownbutton", "align", obj, ["normal"]);
    this._addCss("Menu>#spinupbutton", "align", obj, ["normal"]);
    this._addCss("MenuControl>#spindownbutton", "align", obj, ["normal"]);
    this._addCss("MenuControl>#spinupbutton", "align", obj, ["normal"]);
    this._addCss("ProgressBar", "align", obj, ["normal"]);
    this._addCss("ProgressBarControl", "align", obj, ["normal"]);
    this._addCss("FileDownload", "align", obj, ["normal"]);
    this._addCss("FileDownloadControl", "align", obj, ["normal"]);

    obj = new nexacro.Style_color("#cdcdc8");
    this._addCss("StatusBarControl>#progressbar", "barcolor", obj, ["normal"]);

    obj = new nexacro.Style_gradation("");
    this._addCss("StatusBarControl>#progressbar", "bargradation", obj, ["normal"]);
    this._addCss("ProgressBar", "bargradation", obj, ["normal"]);
    this._addCss("ProgressBarControl", "bargradation", obj, ["normal"]);

    obj = new nexacro.Style_value("normal");
    this._addCss("StatusBarControl>#progressbar", "bartype", obj, ["normal"]);
    this._addCss("ProgressBar", "bartype", obj, ["normal"]);
    this._addCss("ProgressBarControl", "bartype", obj, ["normal"]);

    obj = new nexacro.Style_border("1","solid","#a8a8a2ff","");
    this._addCss("StatusBarControl>#progressbar", "border", obj, ["normal"]);
    this._addCss("StatusBarControl>#zoomcombo", "border", obj, ["normal"]);
    this._addCss("StatusBarControl>#zoomcombo>#dropbutton", "border", obj, ["normal"]);
    this._addCss("StatusBarControl>#zoomcombo>#combolist", "border", obj, ["normal"]);
    this._addCss("PopupMenu>#spindownbutton", "border", obj, ["normal"]);
    this._addCss("PopupMenu>#spinupbutton", "border", obj, ["normal"]);
    this._addCss("PopupMenuControl>#spindownbutton", "border", obj, ["normal"]);
    this._addCss("PopupMenuControl>#spinupbutton", "border", obj, ["normal"]);
    this._addCss("Menu>#spindownbutton", "border", obj, ["normal"]);
    this._addCss("Menu>#spinupbutton", "border", obj, ["normal"]);
    this._addCss("MenuControl>#spindownbutton", "border", obj, ["normal"]);
    this._addCss("MenuControl>#spinupbutton", "border", obj, ["normal"]);

    obj = new nexacro.Style_color("#444444");
    this._addCss("StatusBarControl>#progressbar", "color", obj, ["normal"]);
    this._addCss("Calendar", "color", obj, ["normal"]);
    this._addCss("CalendarControl", "color", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar", "color", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar", "color", obj, ["normal"]);

    obj = new nexacro.Style_value("forward");
    this._addCss("StatusBarControl>#progressbar", "direction", obj, ["normal"]);
    this._addCss("ProgressBar", "direction", obj, ["normal"]);
    this._addCss("ProgressBarControl", "direction", obj, ["normal"]);

    obj = new nexacro.Style_value("");
    this._addCss("StatusBarControl>#progressbar", "endimage", obj, ["normal"]);
    this._addCss("ProgressBar", "endimage", obj, ["normal"]);
    this._addCss("ProgressBarControl", "endimage", obj, ["normal"]);

    obj = new nexacro.Style_font("Dotum, 9");
    this._addCss("StatusBarControl>#progressbar", "font", obj, ["normal"]);

    obj = new nexacro.Style_padding("1 1 1 1");
    this._addCss("StatusBarControl>#progressbar", "padding", obj, ["normal"]);
    this._addCss("StatusBarControl>#zoomcombo", "padding", obj, ["normal"]);

    obj = new nexacro.Style_value("");
    this._addCss("StatusBarControl>#progressbar", "progressimage", obj, ["normal"]);
    this._addCss("ProgressBar", "progressimage", obj, ["normal"]);
    this._addCss("ProgressBarControl", "progressimage", obj, ["normal"]);

    obj = new nexacro.Style_value("true");
    this._addCss("StatusBarControl>#progressbar", "smooth", obj, ["normal"]);
    this._addCss("ProgressBar", "smooth", obj, ["normal"]);
    this._addCss("ProgressBarControl", "smooth", obj, ["normal"]);

    obj = new nexacro.Style_value("");
    this._addCss("StatusBarControl>#progressbar", "startimage", obj, ["normal"]);
    this._addCss("ProgressBar", "startimage", obj, ["normal"]);
    this._addCss("ProgressBarControl", "startimage", obj, ["normal"]);

    obj = new nexacro.Style_background("#f2f2efff","theme://images/ico_zoomcombo.png","","0","0","0","50","true");
    this._addCss("StatusBarControl>#zoomcombo", "background", obj, ["normal"]);

    obj = new nexacro.Style_value("16");
    this._addCss("StatusBarControl>#zoomcombo", "buttonsize", obj, ["normal"]);

    obj = new nexacro.Style_color("#5a280b");
    this._addCss("StatusBarControl>#zoomcombo", "color", obj, ["normal"]);
    this._addCss("StatusBarControl>#zoomcombo>#comboedit", "color", obj, ["normal"]);

    obj = new nexacro.Style_accessibility("","true","all","","","");
    this._addCss("StatusBarControl>#zoomcombo", "itemaccessibility", obj, ["normal"]);
    this._addCss("ListBox", "itemaccessibility", obj, ["normal"]);
    this._addCss("ListBoxControl", "itemaccessibility", obj, ["normal"]);
    this._addCss("Combo", "itemaccessibility", obj, ["normal", "readonly"]);
    this._addCss("ComboControl", "itemaccessibility", obj, ["normal", "readonly"]);
    this._addCss("Radio", "itemaccessibility", obj, ["normal"]);
    this._addCss("RadioControl", "itemaccessibility", obj, ["normal"]);
    this._addCss("Grid>#controlcombo", "itemaccessibility", obj, ["normal", "readonly"]);
    this._addCss("GridControl>#controlcombo", "itemaccessibility", obj, ["normal", "readonly"]);
    this._addCss("PopupMenu", "itemaccessibility", obj, ["normal"]);
    this._addCss("PopupMenuControl", "itemaccessibility", obj, ["normal"]);
    this._addCss("Menu", "itemaccessibility", obj, ["normal"]);
    this._addCss("MenuControl", "itemaccessibility", obj, ["normal"]);
    this._addCss("FileUpload", "itemaccessibility", obj, ["normal"]);
    this._addCss("FileUploadControl", "itemaccessibility", obj, ["normal"]);

    obj = new nexacro.Style_background("#ffffffff","","","0","0","0","0","true");
    this._addCss("StatusBarControl>#zoomcombo", "itembackground", obj, ["normal"]);
    this._addCss("StatusBarControl>#zoomcombo>#combolist", "itembackground", obj, ["normal"]);

    obj = new nexacro.Style_border("0","none","","");
    this._addCss("StatusBarControl>#zoomcombo", "itemborder", obj, ["normal"]);
    this._addCss("StatusBarControl>#zoomcombo>#combolist", "itemborder", obj, ["normal"]);
    this._addCss("ListBox", "itemborder", obj, ["normal"]);
    this._addCss("ListBoxControl", "itemborder", obj, ["normal"]);

    obj = new nexacro.Style_bordertype("","0","0","true","true","true","true");
    this._addCss("StatusBarControl>#zoomcombo", "itembordertype", obj, ["normal"]);
    this._addCss("Combo", "itembordertype", obj, ["normal", "readonly"]);
    this._addCss("ComboControl", "itembordertype", obj, ["normal", "readonly"]);
    this._addCss("Radio", "itembordertype", obj, ["normal"]);
    this._addCss("RadioControl", "itembordertype", obj, ["normal"]);
    this._addCss("Grid>#controlcombo", "itembordertype", obj, ["normal", "readonly"]);
    this._addCss("GridControl>#controlcombo", "itembordertype", obj, ["normal", "readonly"]);
    this._addCss("PopupMenu", "itembordertype", obj, ["normal"]);
    this._addCss("PopupMenuControl", "itembordertype", obj, ["normal"]);
    this._addCss("Menu", "itembordertype", obj, ["normal"]);
    this._addCss("MenuControl", "itembordertype", obj, ["normal"]);

    obj = new nexacro.Style_color("#6e6e63");
    this._addCss("StatusBarControl>#zoomcombo", "itemcolor", obj, ["normal"]);
    this._addCss("StatusBarControl>#zoomcombo>#combolist", "itemcolor", obj, ["normal"]);

    obj = new nexacro.Style_font("");
    this._addCss("StatusBarControl>#zoomcombo", "itemfont", obj, ["normal"]);
    this._addCss("Combo", "itemfont", obj, ["normal", "readonly"]);
    this._addCss("ComboControl", "itemfont", obj, ["normal", "readonly"]);
    this._addCss("Grid>#controlcombo", "itemfont", obj, ["normal", "readonly"]);
    this._addCss("GridControl>#controlcombo", "itemfont", obj, ["normal", "readonly"]);

    obj = new nexacro.Style_gradation("");
    this._addCss("StatusBarControl>#zoomcombo", "itemgradation", obj, ["normal"]);
    this._addCss("ListBox", "itemgradation", obj, ["normal"]);
    this._addCss("ListBoxControl", "itemgradation", obj, ["normal"]);
    this._addCss("Combo", "itemgradation", obj, ["normal", "readonly"]);
    this._addCss("ComboControl", "itemgradation", obj, ["normal", "readonly"]);
    this._addCss("Radio", "itemgradation", obj, ["normal"]);
    this._addCss("RadioControl", "itemgradation", obj, ["normal"]);
    this._addCss("Grid>#controlcombo", "itemgradation", obj, ["normal", "readonly"]);
    this._addCss("GridControl>#controlcombo", "itemgradation", obj, ["normal", "readonly"]);
    this._addCss("PopupMenu", "itemgradation", obj, ["normal"]);
    this._addCss("PopupMenuControl", "itemgradation", obj, ["normal"]);
    this._addCss("Menu", "itemgradation", obj, ["normal"]);
    this._addCss("MenuControl", "itemgradation", obj, ["normal"]);

    obj = new nexacro.Style_value("20");
    this._addCss("StatusBarControl>#zoomcombo", "itemheight", obj, ["normal"]);
    this._addCss("StatusBarControl>#zoomcombo>#combolist", "itemheight", obj, ["normal"]);

    obj = new nexacro.Style_padding("0 5 0 8");
    this._addCss("StatusBarControl>#zoomcombo", "itempadding", obj, ["normal"]);
    this._addCss("StatusBarControl>#zoomcombo>#combolist", "itempadding", obj, ["normal"]);

    obj = new nexacro.Style_background("#f2f2efff","","","0","0","0","0","true");
    this._addCss("StatusBarControl>#zoomcombo", "itembackground", obj, ["mouseover"]);

    obj = new nexacro.Style_color("#5a280b");
    this._addCss("StatusBarControl>#zoomcombo", "itemcolor", obj, ["mouseover", "selected"]);

    obj = new nexacro.Style_background("#e0e0d9ff","","","0","0","0","0","true");
    this._addCss("StatusBarControl>#zoomcombo", "itembackground", obj, ["selected"]);

    obj = new nexacro.Style_padding("0 0 0 20");
    this._addCss("StatusBarControl>#zoomcombo>#comboedit", "padding", obj, ["normal"]);

    obj = new nexacro.Style_color("#3da2df");
    this._addCss("StatusBarControl>#zoomcombo>#comboedit", "caretcolor", obj, ["mouseover", "focused"]);

    obj = new nexacro.Style_color("#ffffff");
    this._addCss("StatusBarControl>#zoomcombo>#comboedit", "compositecolor", obj, ["mouseover", "focused"]);

    obj = new nexacro.Style_value("#3da2df");
    this._addCss("StatusBarControl>#zoomcombo>#comboedit", "selectbackground", obj, ["mouseover", "focused"]);

    obj = new nexacro.Style_color("#ffffff");
    this._addCss("StatusBarControl>#zoomcombo>#comboedit", "selectcolor", obj, ["mouseover", "focused"]);
    this._addCss("Edit", "selectcolor", obj, ["normal", "readonly"]);
    this._addCss("EditControl", "selectcolor", obj, ["normal", "readonly"]);
    this._addCss("TextArea", "selectcolor", obj, ["normal", "readonly"]);
    this._addCss("TextAreaControl", "selectcolor", obj, ["normal", "readonly"]);
    this._addCss("MaskEdit", "selectcolor", obj, ["normal", "readonly"]);
    this._addCss("MaskEditControl", "selectcolor", obj, ["normal", "readonly"]);
    this._addCss("Combo", "selectcolor", obj, ["focused", "selected"]);
    this._addCss("ComboControl", "selectcolor", obj, ["focused", "selected"]);
    this._addCss("Calendar>#calendaredit", "selectcolor", obj, ["normal"]);
    this._addCss("CalendarControl>#calendaredit", "selectcolor", obj, ["normal"]);
    this._addCss("Grid>#body", "selectcolor", obj, ["normal"]);
    this._addCss("Grid>#controlcombo", "selectcolor", obj, ["focused", "selected"]);
    this._addCss("GridControl>#body", "selectcolor", obj, ["normal"]);
    this._addCss("GridControl>#controlcombo", "selectcolor", obj, ["focused", "selected"]);

    obj = new nexacro.Style_background("@gradation","theme://images/img_drop_N.png","stretch","6","6","0","0","true");
    this._addCss("StatusBarControl>#zoomcombo>#dropbutton", "background", obj, ["normal"]);

    obj = new nexacro.Style_gradation("linear 0,0 #ffffff 0,100 #e3e3dd");
    this._addCss("StatusBarControl>#zoomcombo>#dropbutton", "gradation", obj, ["normal", "pushed"]);

    obj = new nexacro.Style_value("theme://images/btn_drop_N.png");
    this._addCss("StatusBarControl>#zoomcombo>#dropbutton", "image", obj, ["normal"]);

    obj = new nexacro.Style_align("center middle");
    this._addCss("StatusBarControl>#zoomcombo>#dropbutton", "imagealign", obj, ["normal"]);
    this._addCss("VScrollBar>#trackbar", "imagealign", obj, ["normal"]);
    this._addCss("VScrollBarControl>#trackbar", "imagealign", obj, ["normal"]);
    this._addCss("HScrollBar>#trackbar", "imagealign", obj, ["normal"]);
    this._addCss("HScrollBarControl>#trackbar", "imagealign", obj, ["normal"]);
    this._addCss("Spin>#spinupbutton", "imagealign", obj, ["normal"]);
    this._addCss("Spin>#spindownbutton", "imagealign", obj, ["normal"]);
    this._addCss("SpinControl>#spinupbutton", "imagealign", obj, ["normal"]);
    this._addCss("SpinControl>#spindownbutton", "imagealign", obj, ["normal"]);
    this._addCss("Calendar>#spinupbutton", "imagealign", obj, ["normal", "mouseover", "pushed", "disabled"]);
    this._addCss("Calendar>#spindownbutton", "imagealign", obj, ["normal", "mouseover", "pushed", "disabled"]);
    this._addCss("CalendarControl>#spinupbutton", "imagealign", obj, ["normal", "mouseover", "pushed", "disabled"]);
    this._addCss("CalendarControl>#spindownbutton", "imagealign", obj, ["normal", "mouseover", "pushed", "disabled"]);
    this._addCss("Grid>#controlcalendar>#spinupbutton", "imagealign", obj, ["normal", "mouseover", "pushed", "disabled"]);
    this._addCss("Grid>#controlcalendar>#spindownbutton", "imagealign", obj, ["normal", "mouseover", "pushed", "disabled"]);
    this._addCss("GridControl>#controlcalendar>#spinupbutton", "imagealign", obj, ["normal", "mouseover", "pushed", "disabled"]);
    this._addCss("GridControl>#controlcalendar>#spindownbutton", "imagealign", obj, ["normal", "mouseover", "pushed", "disabled"]);
    this._addCss("Tab>#spinupbutton", "imagealign", obj, ["normal", "disabled"]);
    this._addCss("Tab>#spindownbutton", "imagealign", obj, ["normal", "disabled"]);
    this._addCss("Tab>#extrabutton", "imagealign", obj, ["normal", "mouseover", "focused", "selected", "pushed", "disabled"]);
    this._addCss("TabControl>#spinupbutton", "imagealign", obj, ["normal", "disabled"]);
    this._addCss("TabControl>#spindownbutton", "imagealign", obj, ["normal", "disabled"]);
    this._addCss("TabControl>#extrabutton", "imagealign", obj, ["normal", "mouseover", "focused", "selected", "pushed", "disabled"]);

    obj = new nexacro.Style_background("@gradation","theme://images/img_drop_O.png","stretch","6","6","0","0","true");
    this._addCss("StatusBarControl>#zoomcombo>#dropbutton", "background", obj, ["mouseover"]);

    obj = new nexacro.Style_gradation("linear 0,0 #ddddd6 0,100 #acaca6");
    this._addCss("StatusBarControl>#zoomcombo>#dropbutton", "gradation", obj, ["mouseover"]);

    obj = new nexacro.Style_value("theme://images/btn_drop_O.png");
    this._addCss("StatusBarControl>#zoomcombo>#dropbutton", "image", obj, ["mouseover", "pushed"]);

    obj = new nexacro.Style_background("#e1e1daff","","","0","0","0","0","true");
    this._addCss("StatusBarControl>#zoomcombo>#dropbutton", "background", obj, ["pushed"]);

    obj = new nexacro.Style_background("#ffffffff","","","0","0","0","0","true");
    this._addCss("StatusBarControl>#zoomcombo>#combolist", "background", obj, ["normal"]);
    this._addCss("PopupDiv", "background", obj, ["normal"]);
    this._addCss("PopupDivControl", "background", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#yearspin", "background", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#monthspin", "background", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#yearspin", "background", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#monthspin", "background", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#yearspin", "background", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#monthspin", "background", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#yearspin", "background", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#monthspin", "background", obj, ["normal"]);

    obj = new nexacro.Style_color("#6e6e63");
    this._addCss("StatusBarControl>#zoomcombo>#combolist", "color", obj, ["normal"]);

    obj = new nexacro.Style_background("#35363bff","","","0","0","0","0","true");
    this._addCss("Form", "background", obj, ["normal"]);
    this._addCss("VScrollBar>#trackbar", "background", obj, ["pushed"]);
    this._addCss("VScrollBarControl>#trackbar", "background", obj, ["pushed"]);
    this._addCss("HScrollBar>#trackbar", "background", obj, ["pushed"]);
    this._addCss("HScrollBarControl>#trackbar", "background", obj, ["pushed"]);
    this._addCss("Div", "background", obj, ["normal"]);
    this._addCss("DivControl", "background", obj, ["normal"]);
    this._addCss("DatePicker", "background", obj, ["normal"]);
    this._addCss("DatePickerControl", "background", obj, ["normal"]);

    obj = new nexacro.Style_value("40");
    this._addCss("VScrollBar", "barminsize", obj, ["normal"]);
    this._addCss("VScrollBarControl", "barminsize", obj, ["normal"]);
    this._addCss("HScrollBar", "barminsize", obj, ["normal"]);
    this._addCss("HScrollBarControl", "barminsize", obj, ["normal"]);

    obj = new nexacro.Style_value("");
    this._addCss("VScrollBar", "baroutsize", obj, ["normal"]);
    this._addCss("VScrollBarControl", "baroutsize", obj, ["normal"]);
    this._addCss("HScrollBar", "baroutsize", obj, ["normal"]);
    this._addCss("HScrollBarControl", "baroutsize", obj, ["normal"]);

    obj = new nexacro.Style_bordertype("round","5","5","false","true","false","false");
    this._addCss("VScrollBar", "bordertype", obj, ["normal"]);
    this._addCss("VScrollBarControl", "bordertype", obj, ["normal"]);

    obj = new nexacro.Style_value("0");
    this._addCss("VScrollBar", "decbtnsize", obj, ["normal"]);
    this._addCss("VScrollBarControl", "decbtnsize", obj, ["normal"]);
    this._addCss("HScrollBar", "decbtnsize", obj, ["normal"]);
    this._addCss("HScrollBarControl", "decbtnsize", obj, ["normal"]);

    obj = new nexacro.Style_value("");
    this._addCss("VScrollBar", "imgoutsize", obj, ["normal"]);
    this._addCss("VScrollBarControl", "imgoutsize", obj, ["normal"]);
    this._addCss("HScrollBar", "imgoutsize", obj, ["normal"]);
    this._addCss("HScrollBarControl", "imgoutsize", obj, ["normal"]);

    obj = new nexacro.Style_value("0");
    this._addCss("VScrollBar", "incbtnsize", obj, ["normal"]);
    this._addCss("VScrollBarControl", "incbtnsize", obj, ["normal"]);
    this._addCss("HScrollBar", "incbtnsize", obj, ["normal"]);
    this._addCss("HScrollBarControl", "incbtnsize", obj, ["normal"]);

    obj = new nexacro.Style_value("14");
    this._addCss("VScrollBar", "scrollbarsize", obj, ["normal"]);
    this._addCss("VScrollBarControl", "scrollbarsize", obj, ["normal"]);
    this._addCss("HScrollBar", "scrollbarsize", obj, ["normal"]);
    this._addCss("HScrollBarControl", "scrollbarsize", obj, ["normal"]);

    obj = new nexacro.Style_value("");
    this._addCss("VScrollBar", "trackbarsize", obj, ["normal"]);
    this._addCss("VScrollBarControl", "trackbarsize", obj, ["normal"]);
    this._addCss("HScrollBar", "trackbarsize", obj, ["normal"]);
    this._addCss("HScrollBarControl", "trackbarsize", obj, ["normal"]);

    obj = new nexacro.Style_background("","","","0","0","0","0","true");
    this._addCss("VScrollBar>#incbutton", "background", obj, ["normal"]);
    this._addCss("VScrollBar>#decbutton", "background", obj, ["normal"]);
    this._addCss("VScrollBarControl>#incbutton", "background", obj, ["normal"]);
    this._addCss("VScrollBarControl>#decbutton", "background", obj, ["normal"]);
    this._addCss("HScrollBar>#incbutton", "background", obj, ["normal"]);
    this._addCss("HScrollBar>#decbutton", "background", obj, ["normal"]);
    this._addCss("HScrollBar>#trackbar", "background", obj, ["mouseover"]);
    this._addCss("HScrollBarControl>#incbutton", "background", obj, ["normal"]);
    this._addCss("HScrollBarControl>#decbutton", "background", obj, ["normal"]);
    this._addCss("StaticControl", "background", obj, ["normal"]);
    this._addCss("ImageViewer", "background", obj, ["normal"]);
    this._addCss("ImageViewerControl", "background", obj, ["normal"]);

    obj = new nexacro.Style_value("");
    this._addCss("VScrollBar>#incbutton", "image", obj, ["normal"]);
    this._addCss("VScrollBar>#decbutton", "image", obj, ["normal"]);
    this._addCss("VScrollBarControl>#incbutton", "image", obj, ["normal"]);
    this._addCss("VScrollBarControl>#decbutton", "image", obj, ["normal"]);
    this._addCss("HScrollBar>#incbutton", "image", obj, ["normal"]);
    this._addCss("HScrollBar>#decbutton", "image", obj, ["normal"]);
    this._addCss("HScrollBarControl>#incbutton", "image", obj, ["normal"]);
    this._addCss("HScrollBarControl>#decbutton", "image", obj, ["normal"]);
    this._addCss("*>#resizebutton", "image", obj, ["normal"]);
    this._addCss("Button", "image", obj, ["normal"]);
    this._addCss("ButtonControl", "image", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#prevbutton", "image", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#nextbutton", "image", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#prevbutton", "image", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#nextbutton", "image", obj, ["normal"]);
    this._addCss("Grid>#controlbutton", "image", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#prevbutton", "image", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#nextbutton", "image", obj, ["normal"]);
    this._addCss("GridControl>#controlbutton", "image", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#prevbutton", "image", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#nextbutton", "image", obj, ["normal"]);

    obj = new nexacro.Style_background("@gradation","","","0","0","0","0","true");
    this._addCss("VScrollBar>#trackbar", "background", obj, ["normal"]);
    this._addCss("VScrollBarControl>#trackbar", "background", obj, ["normal"]);
    this._addCss("HScrollBar>#trackbar", "background", obj, ["normal"]);
    this._addCss("HScrollBarControl>#trackbar", "background", obj, ["normal"]);
    this._addCss("Edit", "background", obj, ["normal", "readonly"]);
    this._addCss("EditControl", "background", obj, ["normal", "readonly"]);
    this._addCss("TextArea", "background", obj, ["normal", "readonly"]);
    this._addCss("TextAreaControl", "background", obj, ["normal", "readonly"]);
    this._addCss("MaskEdit", "background", obj, ["normal", "readonly"]);
    this._addCss("MaskEditControl", "background", obj, ["normal", "readonly"]);
    this._addCss("Button", "background", obj, ["normal"]);
    this._addCss("ButtonControl", "background", obj, ["normal"]);
    this._addCss("Spin", "background", obj, ["normal", "readonly"]);
    this._addCss("Spin>#spinupbutton", "background", obj, ["normal"]);
    this._addCss("Spin>#spindownbutton", "background", obj, ["normal"]);
    this._addCss("SpinControl", "background", obj, ["normal", "readonly"]);
    this._addCss("SpinControl>#spinupbutton", "background", obj, ["normal"]);
    this._addCss("SpinControl>#spindownbutton", "background", obj, ["normal"]);
    this._addCss("ListBox", "background", obj, ["normal"]);
    this._addCss("ListBoxControl", "background", obj, ["normal"]);
    this._addCss("Combo", "background", obj, ["normal", "readonly"]);
    this._addCss("Combo>#dropbutton", "background", obj, ["normal", "mouseover", "focused", "selected", "pushed", "disabled"]);
    this._addCss("Combo>#combolist", "background", obj, ["normal"]);
    this._addCss("ComboControl", "background", obj, ["normal", "readonly"]);
    this._addCss("ComboControl>#dropbutton", "background", obj, ["normal", "mouseover", "focused", "selected", "pushed", "disabled"]);
    this._addCss("ComboControl>#combolist", "background", obj, ["normal"]);
    this._addCss("Calendar>#calendaredit", "background", obj, ["normal"]);
    this._addCss("Calendar>#spinupbutton", "background", obj, ["normal", "mouseover", "pushed", "disabled"]);
    this._addCss("Calendar>#spindownbutton", "background", obj, ["normal", "mouseover", "pushed", "disabled"]);
    this._addCss("CalendarControl>#calendaredit", "background", obj, ["normal"]);
    this._addCss("CalendarControl>#spinupbutton", "background", obj, ["normal", "mouseover", "pushed", "disabled"]);
    this._addCss("CalendarControl>#spindownbutton", "background", obj, ["normal", "mouseover", "pushed", "disabled"]);
    this._addCss("Grid>#head", "background", obj, ["normal"]);
    this._addCss("Grid>#summ", "background", obj, ["normal"]);
    this._addCss("Grid>#controledit", "background", obj, ["normal"]);
    this._addCss("Grid>#controltextarea", "background", obj, ["normal", "readonly"]);
    this._addCss("Grid>#controlmaskedit", "background", obj, ["normal", "readonly"]);
    this._addCss("Grid>#controlbutton", "background", obj, ["normal"]);
    this._addCss("Grid>#controlcombo", "background", obj, ["normal", "readonly"]);
    this._addCss("Grid>#controlcombo>#dropbutton", "background", obj, ["normal", "mouseover", "pushed", "disabled"]);
    this._addCss("Grid>#controlcombo>#combolist", "background", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#calendaredit", "background", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#spinupbutton", "background", obj, ["normal", "mouseover", "pushed", "disabled"]);
    this._addCss("Grid>#controlcalendar>#spindownbutton", "background", obj, ["normal", "mouseover", "pushed", "disabled"]);
    this._addCss("GridControl>#head", "background", obj, ["normal"]);
    this._addCss("GridControl>#summ", "background", obj, ["normal"]);
    this._addCss("GridControl>#controledit", "background", obj, ["normal"]);
    this._addCss("GridControl>#controltextarea", "background", obj, ["normal", "readonly"]);
    this._addCss("GridControl>#controlmaskedit", "background", obj, ["normal", "readonly"]);
    this._addCss("GridControl>#controlbutton", "background", obj, ["normal"]);
    this._addCss("GridControl>#controlcombo", "background", obj, ["normal", "readonly"]);
    this._addCss("GridControl>#controlcombo>#dropbutton", "background", obj, ["normal", "mouseover", "pushed", "disabled"]);
    this._addCss("GridControl>#controlcombo>#combolist", "background", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#calendaredit", "background", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#spinupbutton", "background", obj, ["normal", "mouseover", "pushed", "disabled"]);
    this._addCss("GridControl>#controlcalendar>#spindownbutton", "background", obj, ["normal", "mouseover", "pushed", "disabled"]);
    this._addCss("Tab>#spinupbutton", "background", obj, ["normal"]);
    this._addCss("Tab>#spindownbutton", "background", obj, ["normal"]);
    this._addCss("TabControl>#spinupbutton", "background", obj, ["normal"]);
    this._addCss("TabControl>#spindownbutton", "background", obj, ["normal"]);
    this._addCss("PopupMenu", "background", obj, ["normal"]);
    this._addCss("PopupMenu>#spindownbutton", "background", obj, ["normal"]);
    this._addCss("PopupMenu>#spinupbutton", "background", obj, ["normal"]);
    this._addCss("PopupMenuControl", "background", obj, ["normal"]);
    this._addCss("PopupMenuControl>#spindownbutton", "background", obj, ["normal"]);
    this._addCss("PopupMenuControl>#spinupbutton", "background", obj, ["normal"]);
    this._addCss("Menu", "background", obj, ["normal"]);
    this._addCss("Menu>#spindownbutton", "background", obj, ["normal"]);
    this._addCss("Menu>#spinupbutton", "background", obj, ["normal"]);
    this._addCss("MenuControl", "background", obj, ["normal"]);
    this._addCss("MenuControl>#spindownbutton", "background", obj, ["normal"]);
    this._addCss("MenuControl>#spinupbutton", "background", obj, ["normal"]);
    this._addCss("ProgressBar", "background", obj, ["normal"]);
    this._addCss("ProgressBarControl", "background", obj, ["normal"]);
    this._addCss("FileDownload", "background", obj, ["normal"]);
    this._addCss("FileDownloadControl", "background", obj, ["normal"]);

    obj = new nexacro.Style_border("1","solid","#202020ff","");
    this._addCss("VScrollBar>#trackbar", "border", obj, ["normal", "pushed"]);
    this._addCss("VScrollBarControl>#trackbar", "border", obj, ["normal", "pushed"]);
    this._addCss("HScrollBar>#trackbar", "border", obj, ["normal", "pushed"]);
    this._addCss("HScrollBarControl>#trackbar", "border", obj, ["normal", "pushed"]);

    obj = new nexacro.Style_bordertype("round","6","6","true","true","true","true");
    this._addCss("VScrollBar>#trackbar", "bordertype", obj, ["normal"]);
    this._addCss("VScrollBarControl>#trackbar", "bordertype", obj, ["normal"]);
    this._addCss("HScrollBar>#trackbar", "bordertype", obj, ["normal"]);
    this._addCss("HScrollBarControl>#trackbar", "bordertype", obj, ["normal"]);
    this._addCss("Grid", "bordertype", obj, ["normal"]);
    this._addCss("GridControl", "bordertype", obj, ["normal"]);

    obj = new nexacro.Style_gradation("linear 0,0 #35363bff 100,0 #58595dff [97% #45464aff]");
    this._addCss("VScrollBar>#trackbar", "gradation", obj, ["normal"]);
    this._addCss("VScrollBarControl>#trackbar", "gradation", obj, ["normal"]);

    obj = new nexacro.Style_value("theme://images/ico_vscroll_trackbar.png");
    this._addCss("VScrollBar>#trackbar", "image", obj, ["normal"]);
    this._addCss("VScrollBarControl>#trackbar", "image", obj, ["normal"]);

    obj = new nexacro.Style_shadow("drop 1,1 3 #00000033");
    this._addCss("VScrollBar>#trackbar", "shadow", obj, ["normal"]);
    this._addCss("VScrollBarControl>#trackbar", "shadow", obj, ["normal"]);

    obj = new nexacro.Style_gradation("linear 0,0 #35363bff 100,0 #58595dff [75% #45464aff]");
    this._addCss("VScrollBar>#trackbar", "gradation", obj, ["mouseover"]);
    this._addCss("VScrollBarControl>#trackbar", "gradation", obj, ["mouseover"]);

    obj = new nexacro.Style_value("theme://images/ico_vscroll_trackbarP.png");
    this._addCss("VScrollBar>#trackbar", "image", obj, ["pushed"]);
    this._addCss("VScrollBarControl>#trackbar", "image", obj, ["pushed"]);

    obj = new nexacro.Style_padding("0 1 0 1");
    this._addCss("VScrollBar>#trackbar", "imagepadding", obj, ["pushed"]);
    this._addCss("VScrollBarControl>#trackbar", "imagepadding", obj, ["pushed"]);

    obj = new nexacro.Style_bordertype("round","5","5","false","false","true","false");
    this._addCss("HScrollBar", "bordertype", obj, ["normal"]);
    this._addCss("HScrollBarControl", "bordertype", obj, ["normal"]);

    obj = new nexacro.Style_gradation("linear 0,0 #35363bff 0,100 #58595dff [97% #45464aff]");
    this._addCss("HScrollBar>#trackbar", "gradation", obj, ["normal"]);
    this._addCss("HScrollBarControl>#trackbar", "gradation", obj, ["normal"]);

    obj = new nexacro.Style_value("theme://images/ico_hscroll_trackbar.png");
    this._addCss("HScrollBar>#trackbar", "image", obj, ["normal"]);
    this._addCss("HScrollBarControl>#trackbar", "image", obj, ["normal"]);

    obj = new nexacro.Style_gradation("linear 0,0 #35363bff 0,100 #58595dff [75% #45464aff]");
    this._addCss("HScrollBar>#trackbar", "gradation", obj, ["mouseover"]);
    this._addCss("HScrollBarControl>#trackbar", "gradation", obj, ["mouseover"]);

    obj = new nexacro.Style_value("theme://images/ico_hscroll_trackbarP.png");
    this._addCss("HScrollBar>#trackbar", "image", obj, ["pushed"]);
    this._addCss("HScrollBarControl>#trackbar", "image", obj, ["pushed"]);

    obj = new nexacro.Style_padding("1 0 1 0");
    this._addCss("HScrollBar>#trackbar", "imagepadding", obj, ["pushed"]);
    this._addCss("HScrollBarControl>#trackbar", "imagepadding", obj, ["pushed"]);

    obj = new nexacro.Style_value("");
    this._addCss("Static", "linespace", obj, ["normal"]);
    this._addCss("StaticControl", "linespace", obj, ["normal"]);
    this._addCss("FileDownload", "linespace", obj, ["normal"]);
    this._addCss("FileDownloadControl", "linespace", obj, ["normal"]);

    obj = new nexacro.Style_value("50");
    this._addCss("Static", "opacity", obj, ["disabled"]);
    this._addCss("Combo>#comboedit", "opacity", obj, ["disabled"]);
    this._addCss("ComboControl>#comboedit", "opacity", obj, ["disabled"]);
    this._addCss("Step", "opacity", obj, ["disabled"]);
    this._addCss("StepControl", "opacity", obj, ["disabled"]);
    this._addCss("Calendar>#dropbutton", "opacity", obj, ["disabled"]);
    this._addCss("CalendarControl>#dropbutton", "opacity", obj, ["disabled"]);
    this._addCss("Grid>#controlcombo>#comboedit", "opacity", obj, ["disabled"]);
    this._addCss("Grid>#controlcalendar>#dropbutton", "opacity", obj, ["disabled"]);
    this._addCss("GridControl>#controlcombo>#comboedit", "opacity", obj, ["disabled"]);
    this._addCss("GridControl>#controlcalendar>#dropbutton", "opacity", obj, ["disabled"]);
    this._addCss("Tab", "opacity", obj, ["disabled"]);
    this._addCss("TabControl", "opacity", obj, ["disabled"]);
    this._addCss("Menu", "opacity", obj, ["disabled"]);
    this._addCss("MenuControl", "opacity", obj, ["disabled"]);
    this._addCss("FileUpload", "opacity", obj, ["disabled"]);
    this._addCss("FileUploadControl", "opacity", obj, ["disabled"]);
    this._addCss("FileDownload", "opacity", obj, ["disabled"]);
    this._addCss("FileDownloadControl", "opacity", obj, ["disabled"]);

    obj = new nexacro.Style_border("1","solid","#ffffffff","");
    this._addCss("Edit", "border", obj, ["normal", "readonly"]);
    this._addCss("EditControl", "border", obj, ["normal", "readonly"]);
    this._addCss("TextArea", "border", obj, ["normal", "readonly"]);
    this._addCss("TextAreaControl", "border", obj, ["normal", "readonly"]);
    this._addCss("MaskEdit", "border", obj, ["normal", "readonly"]);
    this._addCss("MaskEditControl", "border", obj, ["normal", "readonly"]);
    this._addCss("Spin", "border", obj, ["normal", "readonly"]);
    this._addCss("SpinControl", "border", obj, ["normal", "readonly"]);
    this._addCss("ListBox", "border", obj, ["normal"]);
    this._addCss("ListBoxControl", "border", obj, ["normal"]);
    this._addCss("Combo", "border", obj, ["normal", "readonly"]);
    this._addCss("ComboControl", "border", obj, ["normal", "readonly"]);
    this._addCss("Calendar>#calendaredit", "border", obj, ["normal"]);
    this._addCss("CalendarControl>#calendaredit", "border", obj, ["normal"]);
    this._addCss("Grid>#controledit", "border", obj, ["normal"]);
    this._addCss("Grid>#controltextarea", "border", obj, ["normal", "readonly"]);
    this._addCss("Grid>#controlmaskedit", "border", obj, ["normal", "readonly"]);
    this._addCss("Grid>#controlcombo", "border", obj, ["normal", "readonly"]);
    this._addCss("Grid>#controlcalendar>#calendaredit", "border", obj, ["normal"]);
    this._addCss("GridControl>#controledit", "border", obj, ["normal"]);
    this._addCss("GridControl>#controltextarea", "border", obj, ["normal", "readonly"]);
    this._addCss("GridControl>#controlmaskedit", "border", obj, ["normal", "readonly"]);
    this._addCss("GridControl>#controlcombo", "border", obj, ["normal", "readonly"]);
    this._addCss("GridControl>#controlcalendar>#calendaredit", "border", obj, ["normal"]);

    obj = new nexacro.Style_bordertype("round","4","4","true","true","true","true");
    this._addCss("Edit", "bordertype", obj, ["normal", "readonly"]);
    this._addCss("EditControl", "bordertype", obj, ["normal", "readonly"]);
    this._addCss("TextArea", "bordertype", obj, ["normal", "readonly"]);
    this._addCss("TextAreaControl", "bordertype", obj, ["normal", "readonly"]);
    this._addCss("MaskEdit", "bordertype", obj, ["normal", "readonly"]);
    this._addCss("MaskEditControl", "bordertype", obj, ["normal", "readonly"]);
    this._addCss("Button", "bordertype", obj, ["normal"]);
    this._addCss("ButtonControl", "bordertype", obj, ["normal"]);
    this._addCss("Spin", "bordertype", obj, ["normal", "readonly"]);
    this._addCss("SpinControl", "bordertype", obj, ["normal", "readonly"]);
    this._addCss("Combo", "bordertype", obj, ["normal", "readonly"]);
    this._addCss("ComboControl", "bordertype", obj, ["normal", "readonly"]);
    this._addCss("PopupDiv", "bordertype", obj, ["normal"]);
    this._addCss("PopupDivControl", "bordertype", obj, ["normal"]);
    this._addCss("Calendar>#calendaredit", "bordertype", obj, ["normal"]);
    this._addCss("Calendar>#spinupbutton", "bordertype", obj, ["normal"]);
    this._addCss("Calendar>#spindownbutton", "bordertype", obj, ["normal"]);
    this._addCss("CalendarControl>#calendaredit", "bordertype", obj, ["normal"]);
    this._addCss("CalendarControl>#spinupbutton", "bordertype", obj, ["normal"]);
    this._addCss("CalendarControl>#spindownbutton", "bordertype", obj, ["normal"]);
    this._addCss("Grid>#controledit", "bordertype", obj, ["normal"]);
    this._addCss("Grid>#controltextarea", "bordertype", obj, ["normal", "readonly"]);
    this._addCss("Grid>#controlmaskedit", "bordertype", obj, ["normal", "readonly"]);
    this._addCss("Grid>#controlbutton", "bordertype", obj, ["normal"]);
    this._addCss("Grid>#controlcombo", "bordertype", obj, ["normal", "readonly"]);
    this._addCss("Grid>#controlcalendar>#calendaredit", "bordertype", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#spinupbutton", "bordertype", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#spindownbutton", "bordertype", obj, ["normal"]);
    this._addCss("GridControl>#controledit", "bordertype", obj, ["normal"]);
    this._addCss("GridControl>#controltextarea", "bordertype", obj, ["normal", "readonly"]);
    this._addCss("GridControl>#controlmaskedit", "bordertype", obj, ["normal", "readonly"]);
    this._addCss("GridControl>#controlbutton", "bordertype", obj, ["normal"]);
    this._addCss("GridControl>#controlcombo", "bordertype", obj, ["normal", "readonly"]);
    this._addCss("GridControl>#controlcalendar>#calendaredit", "bordertype", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#spinupbutton", "bordertype", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#spindownbutton", "bordertype", obj, ["normal"]);
    this._addCss("Tab>#spinupbutton", "bordertype", obj, ["normal"]);
    this._addCss("Tab>#spindownbutton", "bordertype", obj, ["normal"]);
    this._addCss("TabControl>#spinupbutton", "bordertype", obj, ["normal"]);
    this._addCss("TabControl>#spindownbutton", "bordertype", obj, ["normal"]);

    obj = new nexacro.Style_color("#333333");
    this._addCss("Edit", "color", obj, ["normal", "readonly"]);
    this._addCss("EditControl", "color", obj, ["normal", "readonly"]);
    this._addCss("TextArea", "color", obj, ["normal", "readonly"]);
    this._addCss("TextAreaControl", "color", obj, ["normal", "readonly"]);
    this._addCss("MaskEdit", "color", obj, ["normal", "readonly"]);
    this._addCss("MaskEditControl", "color", obj, ["normal", "readonly"]);
    this._addCss("Spin", "color", obj, ["normal", "readonly"]);
    this._addCss("Spin>#spinedit", "color", obj, ["normal"]);
    this._addCss("SpinControl", "color", obj, ["normal", "readonly"]);
    this._addCss("SpinControl>#spinedit", "color", obj, ["normal"]);
    this._addCss("Combo", "color", obj, ["normal", "readonly"]);
    this._addCss("Combo>#comboedit", "color", obj, ["normal", "mouseover", "focused", "readonly"]);
    this._addCss("ComboControl", "color", obj, ["normal", "readonly"]);
    this._addCss("ComboControl>#comboedit", "color", obj, ["normal", "mouseover", "focused", "readonly"]);
    this._addCss("Div", "color", obj, ["normal"]);
    this._addCss("DivControl", "color", obj, ["normal"]);
    this._addCss("Calendar>#calendaredit", "color", obj, ["normal"]);
    this._addCss("CalendarControl>#calendaredit", "color", obj, ["normal"]);
    this._addCss("Grid", "color", obj, ["normal"]);
    this._addCss("Grid>#controledit", "color", obj, ["normal"]);
    this._addCss("Grid>#controltextarea", "color", obj, ["normal", "readonly"]);
    this._addCss("Grid>#controlmaskedit", "color", obj, ["normal", "readonly", "mouseover", "focused"]);
    this._addCss("Grid>#controlcombo", "color", obj, ["normal", "readonly"]);
    this._addCss("Grid>#controlcombo>#comboedit", "color", obj, ["normal", "mouseover", "readonly"]);
    this._addCss("Grid>#controlcalendar>#calendaredit", "color", obj, ["normal"]);
    this._addCss("GridControl", "color", obj, ["normal"]);
    this._addCss("GridControl>#controledit", "color", obj, ["normal"]);
    this._addCss("GridControl>#controltextarea", "color", obj, ["normal", "readonly"]);
    this._addCss("GridControl>#controlmaskedit", "color", obj, ["normal", "readonly", "mouseover", "focused"]);
    this._addCss("GridControl>#controlcombo", "color", obj, ["normal", "readonly"]);
    this._addCss("GridControl>#controlcombo>#comboedit", "color", obj, ["normal", "mouseover", "readonly"]);
    this._addCss("GridControl>#controlcalendar>#calendaredit", "color", obj, ["normal"]);

    obj = new nexacro.Style_gradation("linear 0,0 #c2c7d0 0,100 #ffffff");
    this._addCss("Edit", "gradation", obj, ["normal", "readonly"]);
    this._addCss("EditControl", "gradation", obj, ["normal", "readonly"]);
    this._addCss("TextArea", "gradation", obj, ["normal", "readonly"]);
    this._addCss("TextAreaControl", "gradation", obj, ["normal", "readonly"]);
    this._addCss("MaskEdit", "gradation", obj, ["normal", "readonly"]);
    this._addCss("MaskEditControl", "gradation", obj, ["normal", "readonly"]);
    this._addCss("Spin", "gradation", obj, ["normal", "readonly"]);
    this._addCss("SpinControl", "gradation", obj, ["normal", "readonly"]);
    this._addCss("ListBox", "gradation", obj, ["normal"]);
    this._addCss("ListBoxControl", "gradation", obj, ["normal"]);
    this._addCss("Combo", "gradation", obj, ["normal", "readonly"]);
    this._addCss("Combo>#combolist", "gradation", obj, ["normal"]);
    this._addCss("ComboControl", "gradation", obj, ["normal", "readonly"]);
    this._addCss("ComboControl>#combolist", "gradation", obj, ["normal"]);
    this._addCss("Calendar>#calendaredit", "gradation", obj, ["normal"]);
    this._addCss("CalendarControl>#calendaredit", "gradation", obj, ["normal"]);
    this._addCss("Grid>#controledit", "gradation", obj, ["normal"]);
    this._addCss("Grid>#controltextarea", "gradation", obj, ["normal", "readonly"]);
    this._addCss("Grid>#controlmaskedit", "gradation", obj, ["normal", "readonly"]);
    this._addCss("Grid>#controlcombo", "gradation", obj, ["normal", "readonly"]);
    this._addCss("Grid>#controlcombo>#combolist", "gradation", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#calendaredit", "gradation", obj, ["normal"]);
    this._addCss("GridControl>#controledit", "gradation", obj, ["normal"]);
    this._addCss("GridControl>#controltextarea", "gradation", obj, ["normal", "readonly"]);
    this._addCss("GridControl>#controlmaskedit", "gradation", obj, ["normal", "readonly"]);
    this._addCss("GridControl>#controlcombo", "gradation", obj, ["normal", "readonly"]);
    this._addCss("GridControl>#controlcombo>#combolist", "gradation", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#calendaredit", "gradation", obj, ["normal"]);

    obj = new nexacro.Style_padding("0 7 0 7");
    this._addCss("Edit", "padding", obj, ["normal", "readonly"]);
    this._addCss("EditControl", "padding", obj, ["normal", "readonly"]);
    this._addCss("MaskEdit", "padding", obj, ["normal", "readonly"]);
    this._addCss("MaskEditControl", "padding", obj, ["normal", "readonly"]);
    this._addCss("Grid>#controledit", "padding", obj, ["normal"]);
    this._addCss("Grid>#controlmaskedit", "padding", obj, ["normal", "readonly"]);
    this._addCss("GridControl>#controledit", "padding", obj, ["normal"]);
    this._addCss("GridControl>#controlmaskedit", "padding", obj, ["normal", "readonly"]);

    obj = new nexacro.Style_value("blue");
    this._addCss("Edit", "selectbackground", obj, ["normal", "readonly"]);
    this._addCss("TextArea", "selectbackground", obj, ["normal", "readonly"]);
    this._addCss("MaskEdit", "selectbackground", obj, ["normal", "readonly"]);

    obj = new nexacro.Style_border("1","solid","#000000ff","");
    this._addCss("Edit", "border", obj, ["mouseover", "focused"]);
    this._addCss("EditControl", "border", obj, ["mouseover", "focused"]);
    this._addCss("Combo", "border", obj, ["mouseover", "focused", "selected"]);
    this._addCss("Combo>#combolist", "border", obj, ["normal"]);
    this._addCss("ComboControl", "border", obj, ["mouseover", "focused", "selected"]);
    this._addCss("ComboControl>#combolist", "border", obj, ["normal"]);
    this._addCss("Grid>#controledit", "border", obj, ["mouseover", "focused"]);
    this._addCss("Grid>#controlcombo", "border", obj, ["mouseover", "focused", "selected"]);
    this._addCss("Grid>#controlcombo>#combolist", "border", obj, ["normal"]);
    this._addCss("GridControl>#controledit", "border", obj, ["mouseover", "focused"]);
    this._addCss("GridControl>#controlcombo", "border", obj, ["mouseover", "focused", "selected"]);
    this._addCss("GridControl>#controlcombo>#combolist", "border", obj, ["normal"]);
    this._addCss("Tooltip", "border", obj, ["normal"]);

    obj = new nexacro.Style_background("#e2e2e2ff","","","0","0","0","0","true");
    this._addCss("Edit", "background", obj, ["disabled"]);
    this._addCss("EditControl", "background", obj, ["disabled"]);
    this._addCss("TextArea", "background", obj, ["disabled"]);
    this._addCss("TextAreaControl", "background", obj, ["disabled"]);
    this._addCss("MaskEdit", "background", obj, ["disabled"]);
    this._addCss("MaskEditControl", "background", obj, ["disabled"]);
    this._addCss("ListBox", "background", obj, ["disabled"]);
    this._addCss("ListBoxControl", "background", obj, ["disabled"]);
    this._addCss("Combo", "background", obj, ["disabled"]);
    this._addCss("ComboControl", "background", obj, ["disabled"]);
    this._addCss("Calendar>#calendaredit", "background", obj, ["disabled"]);
    this._addCss("CalendarControl>#calendaredit", "background", obj, ["disabled"]);
    this._addCss("Grid>#controledit", "background", obj, ["disabled"]);
    this._addCss("Grid>#controltextarea", "background", obj, ["disabled"]);
    this._addCss("Grid>#controlmaskedit", "background", obj, ["disabled"]);
    this._addCss("Grid>#controlcombo", "background", obj, ["disabled"]);
    this._addCss("Grid>#controlcalendar>#calendaredit", "background", obj, ["disabled"]);
    this._addCss("GridControl>#controledit", "background", obj, ["disabled"]);
    this._addCss("GridControl>#controltextarea", "background", obj, ["disabled"]);
    this._addCss("GridControl>#controlmaskedit", "background", obj, ["disabled"]);
    this._addCss("GridControl>#controlcombo", "background", obj, ["disabled"]);
    this._addCss("GridControl>#controlcalendar>#calendaredit", "background", obj, ["disabled"]);

    obj = new nexacro.Style_color("#888888");
    this._addCss("Edit", "color", obj, ["disabled"]);
    this._addCss("EditControl", "color", obj, ["disabled"]);
    this._addCss("TextArea", "color", obj, ["disabled"]);
    this._addCss("TextAreaControl", "color", obj, ["disabled"]);
    this._addCss("MaskEdit", "color", obj, ["disabled"]);
    this._addCss("MaskEditControl", "color", obj, ["disabled"]);
    this._addCss("Spin", "color", obj, ["disabled"]);
    this._addCss("SpinControl", "color", obj, ["disabled"]);
    this._addCss("ListBox", "color", obj, ["disabled"]);
    this._addCss("ListBoxControl", "color", obj, ["disabled"]);
    this._addCss("Combo", "color", obj, ["disabled"]);
    this._addCss("ComboControl", "color", obj, ["disabled"]);
    this._addCss("Calendar>#calendaredit", "color", obj, ["disabled"]);
    this._addCss("CalendarControl>#calendaredit", "color", obj, ["disabled"]);
    this._addCss("Grid>#controledit", "color", obj, ["disabled"]);
    this._addCss("Grid>#controltextarea", "color", obj, ["disabled"]);
    this._addCss("Grid>#controlmaskedit", "color", obj, ["disabled"]);
    this._addCss("Grid>#controlcombo", "color", obj, ["disabled"]);
    this._addCss("Grid>#controlcalendar>#calendaredit", "color", obj, ["disabled"]);
    this._addCss("GridControl>#controledit", "color", obj, ["disabled"]);
    this._addCss("GridControl>#controltextarea", "color", obj, ["disabled"]);
    this._addCss("GridControl>#controlmaskedit", "color", obj, ["disabled"]);
    this._addCss("GridControl>#controlcombo", "color", obj, ["disabled"]);
    this._addCss("GridControl>#controlcalendar>#calendaredit", "color", obj, ["disabled"]);

    obj = new nexacro.Style_border("1","solid","#b2bbc3ff","");
    this._addCss("Edit", "border", obj, ["disabled"]);
    this._addCss("EditControl", "border", obj, ["disabled"]);
    this._addCss("TextArea", "border", obj, ["disabled"]);
    this._addCss("TextAreaControl", "border", obj, ["disabled"]);
    this._addCss("MaskEdit", "border", obj, ["disabled"]);
    this._addCss("Spin", "border", obj, ["disabled"]);
    this._addCss("SpinControl", "border", obj, ["disabled"]);
    this._addCss("ListBox", "border", obj, ["disabled"]);
    this._addCss("ListBoxControl", "border", obj, ["disabled"]);
    this._addCss("Calendar>#popupcalendar>#yearspin", "border", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#monthspin", "border", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#yearspin", "border", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#monthspin", "border", obj, ["normal"]);
    this._addCss("Grid>#controledit", "border", obj, ["disabled"]);
    this._addCss("Grid>#controltextarea", "border", obj, ["disabled"]);
    this._addCss("Grid>#controlmaskedit", "border", obj, ["disabled"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#yearspin", "border", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#monthspin", "border", obj, ["normal"]);
    this._addCss("GridControl>#controledit", "border", obj, ["disabled"]);
    this._addCss("GridControl>#controltextarea", "border", obj, ["disabled"]);
    this._addCss("GridControl>#controlmaskedit", "border", obj, ["disabled"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#yearspin", "border", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#monthspin", "border", obj, ["normal"]);

    obj = new nexacro.Style_value("blue");
    this._addCss("EditControl", "selectbackground", obj, ["normal", "readonly"]);
    this._addCss("TextAreaControl", "selectbackground", obj, ["normal", "readonly"]);
    this._addCss("MaskEditControl", "selectbackground", obj, ["normal", "readonly"]);
    this._addCss("Calendar>#calendaredit", "selectbackground", obj, ["normal"]);
    this._addCss("CalendarControl>#calendaredit", "selectbackground", obj, ["normal"]);

    obj = new nexacro.Style_align("left top");
    this._addCss("TextArea", "align", obj, ["normal", "readonly"]);
    this._addCss("TextAreaControl", "align", obj, ["normal", "readonly"]);
    this._addCss("Grid>#controltextarea", "align", obj, ["normal", "readonly"]);
    this._addCss("GridControl>#controltextarea", "align", obj, ["normal", "readonly"]);

    obj = new nexacro.Style_value("5");
    this._addCss("TextArea", "linespace", obj, ["normal", "readonly"]);
    this._addCss("TextAreaControl", "linespace", obj, ["normal", "readonly"]);
    this._addCss("Grid>#controltextarea", "linespace", obj, ["normal", "readonly"]);
    this._addCss("GridControl>#controltextarea", "linespace", obj, ["normal", "readonly"]);

    obj = new nexacro.Style_border("1","solid","#333637ff","");
    this._addCss("TextArea", "border", obj, ["focused", "mouseover"]);
    this._addCss("TextAreaControl", "border", obj, ["focused", "mouseover"]);
    this._addCss("MaskEdit", "border", obj, ["mouseover", "pushed", "focused"]);
    this._addCss("Spin", "border", obj, ["mouseover", "focused", "pushed"]);
    this._addCss("SpinControl", "border", obj, ["mouseover", "focused", "pushed"]);
    this._addCss("Grid>#controltextarea", "border", obj, ["focused", "mouseover"]);
    this._addCss("Grid>#controlmaskedit", "border", obj, ["mouseover", "focused"]);
    this._addCss("GridControl>#controltextarea", "border", obj, ["focused", "mouseover"]);
    this._addCss("GridControl>#controlmaskedit", "border", obj, ["mouseover", "focused"]);

    obj = new nexacro.Style_align("right middle");
    this._addCss("MaskEdit", "align", obj, ["normal", "readonly"]);
    this._addCss("MaskEditControl", "align", obj, ["normal", "readonly"]);
    this._addCss("Grid>#controlmaskedit", "align", obj, ["normal", "readonly"]);
    this._addCss("GridControl>#controlmaskedit", "align", obj, ["normal", "readonly"]);

    obj = new nexacro.Style_border("1","solid","#101113ff","");
    this._addCss("Button", "border", obj, ["normal", "mouseover", "focused", "selected", "pushed", "disabled"]);
    this._addCss("ButtonControl", "border", obj, ["normal", "mouseover", "focused", "selected", "pushed", "disabled"]);
    this._addCss("Grid>#controlbutton", "border", obj, ["normal", "mouseover", "focused", "selected", "pushed", "disabled"]);
    this._addCss("GridControl>#controlbutton", "border", obj, ["normal", "mouseover", "focused", "selected", "pushed", "disabled"]);
    this._addCss("Tab>#spinupbutton", "border", obj, ["normal", "mouseover", "focused", "selected", "pushed", "disabled"]);
    this._addCss("Tab>#spindownbutton", "border", obj, ["normal", "mouseover", "focused", "selected", "pushed", "disabled"]);
    this._addCss("TabControl>#spinupbutton", "border", obj, ["normal", "mouseover", "focused", "selected", "pushed", "disabled"]);
    this._addCss("TabControl>#spindownbutton", "border", obj, ["normal", "mouseover", "focused", "selected", "pushed", "disabled"]);

    obj = new nexacro.Style_color("#ffffff");
    this._addCss("Button", "color", obj, ["normal", "mouseover", "focused", "selected", "pushed"]);
    this._addCss("ButtonControl", "color", obj, ["normal", "mouseover", "focused", "selected", "pushed"]);
    this._addCss("ListBox", "color", obj, ["selected"]);
    this._addCss("ListBoxControl", "color", obj, ["selected"]);
    this._addCss("Radio", "color", obj, ["normal"]);
    this._addCss("RadioControl", "color", obj, ["normal"]);
    this._addCss("CheckBox", "color", obj, ["normal"]);
    this._addCss("CheckBoxControl", "color", obj, ["normal"]);
    this._addCss("GroupBox", "color", obj, ["normal"]);
    this._addCss("GroupBoxControl", "color", obj, ["normal"]);
    this._addCss("Grid>#controlbutton", "color", obj, ["normal", "mouseover", "focused", "selected", "pushed"]);
    this._addCss("Grid>#controlcheckbox", "color", obj, ["normal"]);
    this._addCss("GridControl>#controlbutton", "color", obj, ["normal", "mouseover", "focused", "selected", "pushed"]);
    this._addCss("GridControl>#controlcheckbox", "color", obj, ["normal"]);
    this._addCss("Tab", "color", obj, ["normal"]);
    this._addCss("Tab>#spinupbutton", "color", obj, ["normal", "mouseover", "focused", "selected", "pushed"]);
    this._addCss("Tab>#spindownbutton", "color", obj, ["normal", "mouseover", "focused", "selected", "pushed"]);
    this._addCss("TabControl", "color", obj, ["normal"]);
    this._addCss("TabControl>#spinupbutton", "color", obj, ["normal", "mouseover", "focused", "selected", "pushed"]);
    this._addCss("TabControl>#spindownbutton", "color", obj, ["normal", "mouseover", "focused", "selected", "pushed"]);
    this._addCss("PopupMenu", "color", obj, ["normal", "selected", "focused", "pushed"]);
    this._addCss("PopupMenuControl", "color", obj, ["normal", "selected", "focused", "pushed"]);
    this._addCss("Menu", "color", obj, ["mouseover", "selected", "focused", "pushed"]);
    this._addCss("MenuControl", "color", obj, ["mouseover", "selected", "focused", "pushed"]);

    obj = new nexacro.Style_value("hand");
    this._addCss("Button", "cursor", obj, ["normal"]);
    this._addCss("ButtonControl", "cursor", obj, ["normal"]);
    this._addCss("Step", "cursor", obj, ["normal"]);
    this._addCss("StepControl", "cursor", obj, ["normal"]);
    this._addCss("Grid>#controlbutton", "cursor", obj, ["normal"]);
    this._addCss("GridControl>#controlbutton", "cursor", obj, ["normal"]);
    this._addCss("Tab", "cursor", obj, ["normal"]);
    this._addCss("Tab>#spinupbutton", "cursor", obj, ["normal"]);
    this._addCss("Tab>#spindownbutton", "cursor", obj, ["normal"]);
    this._addCss("TabControl", "cursor", obj, ["normal"]);
    this._addCss("TabControl>#spinupbutton", "cursor", obj, ["normal"]);
    this._addCss("TabControl>#spindownbutton", "cursor", obj, ["normal"]);
    this._addCss("Menu", "cursor", obj, ["normal"]);
    this._addCss("MenuControl", "cursor", obj, ["normal"]);

    obj = new nexacro.Style_gradation("linear 0,0 #56575b 0,100 #212326 [3% #45464a]");
    this._addCss("Button", "gradation", obj, ["normal"]);
    this._addCss("ButtonControl", "gradation", obj, ["normal"]);
    this._addCss("Grid>#controlbutton", "gradation", obj, ["normal"]);
    this._addCss("GridControl>#controlbutton", "gradation", obj, ["normal"]);
    this._addCss("Tab>#spinupbutton", "gradation", obj, ["normal"]);
    this._addCss("Tab>#spindownbutton", "gradation", obj, ["normal"]);
    this._addCss("TabControl>#spinupbutton", "gradation", obj, ["normal"]);
    this._addCss("TabControl>#spindownbutton", "gradation", obj, ["normal"]);

    obj = new nexacro.Style_shadow("drop 0,1 1 #00000024");
    this._addCss("Button", "shadow", obj, ["normal"]);
    this._addCss("ButtonControl", "shadow", obj, ["normal"]);
    this._addCss("Grid>#controlbutton", "shadow", obj, ["normal"]);
    this._addCss("GridControl>#controlbutton", "shadow", obj, ["normal"]);
    this._addCss("Tab>#spinupbutton", "shadow", obj, ["normal"]);
    this._addCss("Tab>#spindownbutton", "shadow", obj, ["normal"]);
    this._addCss("TabControl>#spinupbutton", "shadow", obj, ["normal"]);
    this._addCss("TabControl>#spindownbutton", "shadow", obj, ["normal"]);

    obj = new nexacro.Style_gradation("linear 0,0 #56575b 0,100 #212326 [35% #45464a]");
    this._addCss("Button", "gradation", obj, ["mouseover", "focused", "selected"]);
    this._addCss("ButtonControl", "gradation", obj, ["mouseover", "focused", "selected"]);
    this._addCss("Grid>#controlbutton", "gradation", obj, ["mouseover", "focused", "selected"]);
    this._addCss("GridControl>#controlbutton", "gradation", obj, ["mouseover", "focused", "selected"]);
    this._addCss("Tab>#spinupbutton", "gradation", obj, ["mouseover", "focused", "selected"]);
    this._addCss("Tab>#spindownbutton", "gradation", obj, ["mouseover", "focused", "selected"]);
    this._addCss("TabControl>#spinupbutton", "gradation", obj, ["mouseover", "focused", "selected"]);
    this._addCss("TabControl>#spindownbutton", "gradation", obj, ["mouseover", "focused", "selected"]);

    obj = new nexacro.Style_gradation("linear 0,0 #212326 0,100 #212326");
    this._addCss("Button", "gradation", obj, ["pushed"]);
    this._addCss("ButtonControl", "gradation", obj, ["pushed"]);
    this._addCss("Grid>#controlbutton", "gradation", obj, ["pushed"]);
    this._addCss("GridControl>#controlbutton", "gradation", obj, ["pushed"]);
    this._addCss("Tab>#spinupbutton", "gradation", obj, ["pushed"]);
    this._addCss("Tab>#spindownbutton", "gradation", obj, ["pushed"]);
    this._addCss("TabControl>#spinupbutton", "gradation", obj, ["pushed"]);
    this._addCss("TabControl>#spindownbutton", "gradation", obj, ["pushed"]);

    obj = new nexacro.Style_background("#56575bff","","","0","0","0","0","true");
    this._addCss("Button", "background", obj, ["disabled"]);
    this._addCss("ButtonControl", "background", obj, ["disabled"]);
    this._addCss("Grid>#controlbutton", "background", obj, ["disabled"]);
    this._addCss("GridControl>#controlbutton", "background", obj, ["disabled"]);
    this._addCss("Tab>#spinupbutton", "background", obj, ["disabled"]);
    this._addCss("Tab>#spindownbutton", "background", obj, ["disabled"]);
    this._addCss("TabControl>#spinupbutton", "background", obj, ["disabled"]);
    this._addCss("TabControl>#spindownbutton", "background", obj, ["disabled"]);

    obj = new nexacro.Style_value("");
    this._addCss("Spin", "buttonalign", obj, ["normal", "readonly"]);
    this._addCss("SpinControl", "buttonalign", obj, ["normal", "readonly"]);
    this._addCss("Calendar>#popupcalendar>#yearspin", "buttonalign", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#monthspin", "buttonalign", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#yearspin", "buttonalign", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#monthspin", "buttonalign", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#yearspin", "buttonalign", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#monthspin", "buttonalign", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#yearspin", "buttonalign", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#monthspin", "buttonalign", obj, ["normal"]);

    obj = new nexacro.Style_value("");
    this._addCss("Spin", "buttonsize", obj, ["normal", "readonly"]);
    this._addCss("SpinControl", "buttonsize", obj, ["normal", "readonly"]);
    this._addCss("Combo", "buttonsize", obj, ["normal", "readonly"]);
    this._addCss("ComboControl", "buttonsize", obj, ["normal", "readonly"]);
    this._addCss("Calendar", "buttonsize", obj, ["normal"]);
    this._addCss("CalendarControl", "buttonsize", obj, ["normal"]);
    this._addCss("Grid>#controlcombo", "buttonsize", obj, ["normal", "readonly"]);
    this._addCss("Grid>#controlcalendar", "buttonsize", obj, ["normal"]);
    this._addCss("GridControl>#controlcombo", "buttonsize", obj, ["normal", "readonly"]);
    this._addCss("GridControl>#controlcalendar", "buttonsize", obj, ["normal"]);

    obj = new nexacro.Style_background("#f2f2f2ff","","","0","0","0","0","true");
    this._addCss("Spin", "background", obj, ["disabled"]);
    this._addCss("SpinControl", "background", obj, ["disabled"]);

    obj = new nexacro.Style_padding("0 0 0 7");
    this._addCss("Spin>#spinedit", "padding", obj, ["normal"]);
    this._addCss("SpinControl>#spinedit", "padding", obj, ["normal"]);
    this._addCss("Combo>#comboedit", "padding", obj, ["normal", "mouseover", "focused", "readonly"]);
    this._addCss("ComboControl>#comboedit", "padding", obj, ["normal", "mouseover", "focused", "readonly"]);
    this._addCss("Grid>#controlcombo>#comboedit", "padding", obj, ["normal", "mouseover", "readonly"]);
    this._addCss("GridControl>#controlcombo>#comboedit", "padding", obj, ["normal", "mouseover", "readonly"]);

    obj = new nexacro.Style_border("0","none","#808080ff","","0","none","#808080ff","","1","solid","#93979bff","","1","solid","#c1c1c1ff","");
    this._addCss("Spin>#spinupbutton", "border", obj, ["normal", "mouseover", "focused", "selected", "pushed", "disabled"]);
    this._addCss("Spin>#spindownbutton", "border", obj, ["disabled"]);
    this._addCss("SpinControl>#spinupbutton", "border", obj, ["normal", "mouseover", "focused", "selected", "pushed", "disabled"]);
    this._addCss("SpinControl>#spindownbutton", "border", obj, ["disabled"]);

    obj = new nexacro.Style_gradation("linear 0,0 #ffffff 0,100 #f1f1f1");
    this._addCss("Spin>#spinupbutton", "gradation", obj, ["normal"]);
    this._addCss("SpinControl>#spinupbutton", "gradation", obj, ["normal"]);
    this._addCss("Calendar>#spinupbutton", "gradation", obj, ["normal"]);
    this._addCss("CalendarControl>#spinupbutton", "gradation", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#spinupbutton", "gradation", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#spinupbutton", "gradation", obj, ["normal"]);

    obj = new nexacro.Style_value("theme://images/btn_spinup_N.png");
    this._addCss("Spin>#spinupbutton", "image", obj, ["normal"]);
    this._addCss("SpinControl>#spinupbutton", "image", obj, ["normal"]);
    this._addCss("Calendar>#spinupbutton", "image", obj, ["normal", "disabled"]);
    this._addCss("CalendarControl>#spinupbutton", "image", obj, ["normal", "disabled"]);
    this._addCss("Grid>#controlcalendar>#spinupbutton", "image", obj, ["normal", "disabled"]);
    this._addCss("GridControl>#controlcalendar>#spinupbutton", "image", obj, ["normal", "disabled"]);

    obj = new nexacro.Style_gradation("linear 0,0 #ffffff 0,100 #cccccc");
    this._addCss("Spin>#spinupbutton", "gradation", obj, ["mouseover", "focused", "selected"]);
    this._addCss("Spin>#spindownbutton", "gradation", obj, ["mouseover", "focused", "selected"]);
    this._addCss("SpinControl>#spinupbutton", "gradation", obj, ["mouseover", "focused", "selected"]);
    this._addCss("SpinControl>#spindownbutton", "gradation", obj, ["mouseover", "focused", "selected"]);
    this._addCss("Combo>#dropbutton", "gradation", obj, ["mouseover", "focused", "selected"]);
    this._addCss("ComboControl>#dropbutton", "gradation", obj, ["mouseover", "focused", "selected"]);
    this._addCss("Calendar>#spinupbutton", "gradation", obj, ["mouseover"]);
    this._addCss("Calendar>#spindownbutton", "gradation", obj, ["mouseover"]);
    this._addCss("CalendarControl>#spinupbutton", "gradation", obj, ["mouseover"]);
    this._addCss("CalendarControl>#spindownbutton", "gradation", obj, ["mouseover"]);
    this._addCss("Grid>#controlcombo>#dropbutton", "gradation", obj, ["mouseover"]);
    this._addCss("Grid>#controlcalendar>#spinupbutton", "gradation", obj, ["mouseover"]);
    this._addCss("Grid>#controlcalendar>#spindownbutton", "gradation", obj, ["mouseover"]);
    this._addCss("GridControl>#controlcombo>#dropbutton", "gradation", obj, ["mouseover"]);
    this._addCss("GridControl>#controlcalendar>#spinupbutton", "gradation", obj, ["mouseover"]);
    this._addCss("GridControl>#controlcalendar>#spindownbutton", "gradation", obj, ["mouseover"]);

    obj = new nexacro.Style_value("theme://images/btn_spinup_P.png");
    this._addCss("Spin>#spinupbutton", "image", obj, ["mouseover", "focused", "selected", "pushed"]);
    this._addCss("SpinControl>#spinupbutton", "image", obj, ["mouseover", "focused", "selected", "pushed"]);
    this._addCss("Calendar>#spinupbutton", "image", obj, ["mouseover", "pushed"]);
    this._addCss("CalendarControl>#spinupbutton", "image", obj, ["mouseover", "pushed"]);
    this._addCss("Grid>#controlcalendar>#spinupbutton", "image", obj, ["mouseover", "pushed"]);
    this._addCss("GridControl>#controlcalendar>#spinupbutton", "image", obj, ["mouseover", "pushed"]);

    obj = new nexacro.Style_gradation("linear 0,0 #e4e4e4 0,100 #ffffff");
    this._addCss("Spin>#spinupbutton", "gradation", obj, ["pushed"]);
    this._addCss("Spin>#spindownbutton", "gradation", obj, ["pushed"]);
    this._addCss("SpinControl>#spinupbutton", "gradation", obj, ["pushed"]);
    this._addCss("SpinControl>#spindownbutton", "gradation", obj, ["pushed"]);
    this._addCss("Combo>#dropbutton", "gradation", obj, ["pushed"]);
    this._addCss("ComboControl>#dropbutton", "gradation", obj, ["pushed"]);
    this._addCss("Calendar>#spinupbutton", "gradation", obj, ["pushed"]);
    this._addCss("Calendar>#spindownbutton", "gradation", obj, ["pushed"]);
    this._addCss("CalendarControl>#spinupbutton", "gradation", obj, ["pushed"]);
    this._addCss("CalendarControl>#spindownbutton", "gradation", obj, ["pushed"]);
    this._addCss("Grid>#controlcombo>#dropbutton", "gradation", obj, ["pushed"]);
    this._addCss("Grid>#controlcalendar>#spinupbutton", "gradation", obj, ["pushed"]);
    this._addCss("Grid>#controlcalendar>#spindownbutton", "gradation", obj, ["pushed"]);
    this._addCss("GridControl>#controlcombo>#dropbutton", "gradation", obj, ["pushed"]);
    this._addCss("GridControl>#controlcalendar>#spinupbutton", "gradation", obj, ["pushed"]);
    this._addCss("GridControl>#controlcalendar>#spindownbutton", "gradation", obj, ["pushed"]);

    obj = new nexacro.Style_gradation("linear 0,0 #ffffff 0,100 #eeeeee");
    this._addCss("Spin>#spinupbutton", "gradation", obj, ["disabled"]);
    this._addCss("Spin>#spindownbutton", "gradation", obj, ["disabled"]);
    this._addCss("SpinControl>#spinupbutton", "gradation", obj, ["disabled"]);
    this._addCss("SpinControl>#spindownbutton", "gradation", obj, ["disabled"]);
    this._addCss("Combo>#dropbutton", "gradation", obj, ["disabled"]);
    this._addCss("ComboControl>#dropbutton", "gradation", obj, ["disabled"]);
    this._addCss("Calendar>#spinupbutton", "gradation", obj, ["disabled"]);
    this._addCss("Calendar>#spindownbutton", "gradation", obj, ["disabled"]);
    this._addCss("CalendarControl>#spinupbutton", "gradation", obj, ["disabled"]);
    this._addCss("CalendarControl>#spindownbutton", "gradation", obj, ["disabled"]);
    this._addCss("Grid>#controlcombo>#dropbutton", "gradation", obj, ["disabled"]);
    this._addCss("Grid>#controlcalendar>#spinupbutton", "gradation", obj, ["disabled"]);
    this._addCss("Grid>#controlcalendar>#spindownbutton", "gradation", obj, ["disabled"]);
    this._addCss("GridControl>#controlcombo>#dropbutton", "gradation", obj, ["disabled"]);
    this._addCss("GridControl>#controlcalendar>#spinupbutton", "gradation", obj, ["disabled"]);
    this._addCss("GridControl>#controlcalendar>#spindownbutton", "gradation", obj, ["disabled"]);

    obj = new nexacro.Style_border("0","none","#808080ff","","0","none","#808080ff","","0","none","#808080ff","","1","solid","#c1c1c1ff","");
    this._addCss("Spin>#spindownbutton", "border", obj, ["normal", "mouseover", "focused", "selected", "pushed"]);
    this._addCss("SpinControl>#spindownbutton", "border", obj, ["normal", "mouseover", "focused", "selected", "pushed"]);

    obj = new nexacro.Style_gradation("linear 0,0 #f1f1f1 0,100 #e4e4e4");
    this._addCss("Spin>#spindownbutton", "gradation", obj, ["normal"]);
    this._addCss("SpinControl>#spindownbutton", "gradation", obj, ["normal"]);
    this._addCss("Calendar>#spindownbutton", "gradation", obj, ["normal"]);
    this._addCss("CalendarControl>#spindownbutton", "gradation", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#spindownbutton", "gradation", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#spindownbutton", "gradation", obj, ["normal"]);

    obj = new nexacro.Style_value("theme://images/btn_spindn_N.png");
    this._addCss("Spin>#spindownbutton", "image", obj, ["normal"]);
    this._addCss("SpinControl>#spindownbutton", "image", obj, ["normal"]);
    this._addCss("Combo>#dropbutton", "image", obj, ["normal", "disabled"]);
    this._addCss("ComboControl>#dropbutton", "image", obj, ["normal", "disabled"]);
    this._addCss("Calendar>#spindownbutton", "image", obj, ["normal", "disabled"]);
    this._addCss("CalendarControl>#spindownbutton", "image", obj, ["normal", "disabled"]);
    this._addCss("Grid>#controlcombo>#dropbutton", "image", obj, ["normal", "disabled"]);
    this._addCss("Grid>#controlcalendar>#spindownbutton", "image", obj, ["normal", "disabled"]);
    this._addCss("GridControl>#controlcombo>#dropbutton", "image", obj, ["normal", "disabled"]);
    this._addCss("GridControl>#controlcalendar>#spindownbutton", "image", obj, ["normal", "disabled"]);

    obj = new nexacro.Style_value("theme://images/btn_spindn_P.png");
    this._addCss("Spin>#spindownbutton", "image", obj, ["mouseover", "focused", "selected", "pushed"]);
    this._addCss("SpinControl>#spindownbutton", "image", obj, ["mouseover", "focused", "selected", "pushed"]);
    this._addCss("Combo>#dropbutton", "image", obj, ["mouseover", "focused", "selected", "pushed"]);
    this._addCss("ComboControl>#dropbutton", "image", obj, ["mouseover", "focused", "selected", "pushed"]);
    this._addCss("Calendar>#spindownbutton", "image", obj, ["mouseover", "pushed"]);
    this._addCss("CalendarControl>#spindownbutton", "image", obj, ["mouseover", "pushed"]);
    this._addCss("Grid>#controlcombo>#dropbutton", "image", obj, ["mouseover", "pushed"]);
    this._addCss("Grid>#controlcalendar>#spindownbutton", "image", obj, ["mouseover", "pushed"]);
    this._addCss("GridControl>#controlcombo>#dropbutton", "image", obj, ["mouseover", "pushed"]);
    this._addCss("GridControl>#controlcalendar>#spindownbutton", "image", obj, ["mouseover", "pushed"]);

    obj = new nexacro.Style_bordertype("round","3","3","true","true","true","true");
    this._addCss("ListBox", "bordertype", obj, ["normal"]);
    this._addCss("ListBoxControl", "bordertype", obj, ["normal"]);
    this._addCss("ProgressBar", "bordertype", obj, ["normal"]);
    this._addCss("ProgressBarControl", "bordertype", obj, ["normal"]);
    this._addCss("FileDownload", "bordertype", obj, ["normal"]);
    this._addCss("FileDownloadControl", "bordertype", obj, ["normal"]);

    obj = new nexacro.Style_color("#000000");
    this._addCss("ListBox", "color", obj, ["normal"]);
    this._addCss("ListBoxControl", "color", obj, ["normal"]);
    this._addCss("PopupMenu", "color", obj, ["mouseover"]);
    this._addCss("PopupMenuControl", "color", obj, ["mouseover"]);
    this._addCss("Tooltip", "color", obj, ["normal"]);

    obj = new nexacro.Style_background("","","","0","0","0","0","true");
    this._addCss("ListBox", "itembackground", obj, ["normal"]);
    this._addCss("ListBoxControl", "itembackground", obj, ["normal"]);

    obj = new nexacro.Style_bordertype("normal","0","0","true","true","true","true");
    this._addCss("ListBox", "itembordertype", obj, ["normal"]);
    this._addCss("ListBoxControl", "itembordertype", obj, ["normal"]);

    obj = new nexacro.Style_value("21");
    this._addCss("ListBox", "itemheight", obj, ["normal"]);
    this._addCss("ListBoxControl", "itemheight", obj, ["normal"]);

    obj = new nexacro.Style_padding("0 7 0 7");
    this._addCss("ListBox", "itempadding", obj, ["normal"]);
    this._addCss("ListBoxControl", "itempadding", obj, ["normal"]);
    this._addCss("PopupMenu", "itempadding", obj, ["normal"]);
    this._addCss("PopupMenuControl", "itempadding", obj, ["normal"]);

    obj = new nexacro.Style_shadow("drop 1,1 2 #00000033");
    this._addCss("ListBox", "shadow", obj, ["normal"]);
    this._addCss("ListBoxControl", "shadow", obj, ["normal"]);

    obj = new nexacro.Style_border("1","solid","#2f313cff","");
    this._addCss("ListBox", "border", obj, ["mouseover", "selected"]);
    this._addCss("ListBoxControl", "border", obj, ["mouseover", "selected"]);
    this._addCss("Calendar>#calendaredit", "border", obj, ["mouseover", "focused"]);
    this._addCss("CalendarControl>#calendaredit", "border", obj, ["mouseover", "focused"]);
    this._addCss("Grid>#controlcalendar>#calendaredit", "border", obj, ["mouseover", "focused"]);
    this._addCss("GridControl>#controlcalendar>#calendaredit", "border", obj, ["mouseover", "focused"]);

    obj = new nexacro.Style_background("#d7d8dcff","","","0","0","0","0","true");
    this._addCss("ListBox", "itembackground", obj, ["mouseover"]);
    this._addCss("ListBoxControl", "itembackground", obj, ["mouseover"]);
    this._addCss("Combo", "itembackground", obj, ["mouseover"]);
    this._addCss("ComboControl", "itembackground", obj, ["mouseover"]);
    this._addCss("Grid>#controlcombo", "itembackground", obj, ["mouseover"]);
    this._addCss("GridControl>#controlcombo", "itembackground", obj, ["mouseover"]);
    this._addCss("PopupMenu", "itembackground", obj, ["mouseover"]);
    this._addCss("PopupMenuControl", "itembackground", obj, ["mouseover"]);

    obj = new nexacro.Style_background("#7e8089ff","","","0","0","0","0","true");
    this._addCss("ListBox", "itembackground", obj, ["selected"]);
    this._addCss("ListBoxControl", "itembackground", obj, ["selected"]);
    this._addCss("Combo", "itembackground", obj, ["focused", "selected"]);
    this._addCss("ComboControl", "itembackground", obj, ["focused", "selected"]);
    this._addCss("Grid>#controlcombo", "itembackground", obj, ["focused", "selected"]);
    this._addCss("GridControl>#controlcombo", "itembackground", obj, ["focused", "selected"]);

    obj = new nexacro.Style_background("","","","0","0","0","0","true");
    this._addCss("Combo", "itembackground", obj, ["normal", "readonly"]);
    this._addCss("ComboControl", "itembackground", obj, ["normal", "readonly"]);
    this._addCss("Radio", "itembackground", obj, ["normal"]);
    this._addCss("RadioControl", "itembackground", obj, ["normal"]);
    this._addCss("Grid>#controlcombo", "itembackground", obj, ["normal", "readonly"]);
    this._addCss("GridControl>#controlcombo", "itembackground", obj, ["normal", "readonly"]);

    obj = new nexacro.Style_border("");
    this._addCss("Combo", "itemborder", obj, ["normal", "readonly"]);
    this._addCss("ComboControl", "itemborder", obj, ["normal", "readonly"]);
    this._addCss("Radio", "itemborder", obj, ["normal"]);
    this._addCss("RadioControl", "itemborder", obj, ["normal"]);
    this._addCss("Grid>#controlcombo", "itemborder", obj, ["normal", "readonly"]);
    this._addCss("GridControl>#controlcombo", "itemborder", obj, ["normal", "readonly"]);
    this._addCss("Menu", "itemborder", obj, ["normal"]);
    this._addCss("MenuControl", "itemborder", obj, ["normal"]);

    obj = new nexacro.Style_color("");
    this._addCss("Combo", "itemcolor", obj, ["normal", "readonly"]);
    this._addCss("ComboControl", "itemcolor", obj, ["normal", "readonly"]);
    this._addCss("Grid>#controlcombo", "itemcolor", obj, ["normal", "readonly"]);
    this._addCss("GridControl>#controlcombo", "itemcolor", obj, ["normal", "readonly"]);

    obj = new nexacro.Style_value("");
    this._addCss("Combo", "itemheight", obj, ["normal", "readonly"]);
    this._addCss("ComboControl", "itemheight", obj, ["normal", "readonly"]);
    this._addCss("Grid>#controlcombo", "itemheight", obj, ["normal", "readonly"]);
    this._addCss("GridControl>#controlcombo", "itemheight", obj, ["normal", "readonly"]);

    obj = new nexacro.Style_padding("");
    this._addCss("Combo", "itempadding", obj, ["normal", "readonly"]);
    this._addCss("ComboControl", "itempadding", obj, ["normal", "readonly"]);
    this._addCss("Radio", "itempadding", obj, ["normal"]);
    this._addCss("RadioControl", "itempadding", obj, ["normal"]);
    this._addCss("Grid>#controlcombo", "itempadding", obj, ["normal", "readonly"]);
    this._addCss("GridControl>#controlcombo", "itempadding", obj, ["normal", "readonly"]);

    obj = new nexacro.Style_color("#ffffff");
    this._addCss("Combo", "itemcolor", obj, ["focused", "selected"]);
    this._addCss("ComboControl", "itemcolor", obj, ["focused", "selected"]);
    this._addCss("Grid>#controlcombo", "itemcolor", obj, ["focused", "selected"]);
    this._addCss("GridControl>#controlcombo", "itemcolor", obj, ["focused", "selected"]);

    obj = new nexacro.Style_value("#316ac5");
    this._addCss("Combo", "selectbackground", obj, ["focused", "selected"]);
    this._addCss("ComboControl", "selectbackground", obj, ["focused", "selected"]);
    this._addCss("Grid>#controlcombo", "selectbackground", obj, ["focused", "selected"]);
    this._addCss("GridControl>#controlcombo", "selectbackground", obj, ["focused", "selected"]);

    obj = new nexacro.Style_border("1","solid","#b2bbc4ff","");
    this._addCss("Combo", "border", obj, ["disabled"]);
    this._addCss("ComboControl", "border", obj, ["disabled"]);
    this._addCss("Grid>#controlcombo", "border", obj, ["disabled"]);
    this._addCss("GridControl>#controlcombo", "border", obj, ["disabled"]);

    obj = new nexacro.Style_background("#888888ff","","","0","0","0","0","true");
    this._addCss("Combo", "itembackground", obj, ["disabled"]);
    this._addCss("ComboControl", "itembackground", obj, ["disabled"]);
    this._addCss("Grid>#controlcombo", "itembackground", obj, ["disabled"]);
    this._addCss("GridControl>#controlcombo", "itembackground", obj, ["disabled"]);

    obj = new nexacro.Style_border("0","none","#000000ff","");
    this._addCss("Combo>#comboedit", "border", obj, ["normal", "mouseover", "focused", "readonly"]);
    this._addCss("ComboControl>#comboedit", "border", obj, ["normal", "mouseover", "focused", "readonly"]);
    this._addCss("Grid>#controlcombo>#comboedit", "border", obj, ["normal", "mouseover", "readonly"]);
    this._addCss("GridControl>#controlcombo>#comboedit", "border", obj, ["normal", "mouseover", "readonly"]);

    obj = new nexacro.Style_bordertype("round","3","3","true","false","true","false");
    this._addCss("Combo>#comboedit", "bordertype", obj, ["normal", "mouseover", "focused", "readonly"]);
    this._addCss("ComboControl>#comboedit", "bordertype", obj, ["normal", "mouseover", "focused", "readonly"]);
    this._addCss("Grid>#controlcombo>#comboedit", "bordertype", obj, ["normal", "mouseover", "readonly"]);
    this._addCss("GridControl>#controlcombo>#comboedit", "bordertype", obj, ["normal", "mouseover", "readonly"]);

    obj = new nexacro.Style_border("0","none","","","0","none","","","0","none","","","1","solid","#abb1b6ff","");
    this._addCss("Combo>#dropbutton", "border", obj, ["normal", "mouseover", "focused", "selected", "pushed", "disabled"]);
    this._addCss("ComboControl>#dropbutton", "border", obj, ["normal", "mouseover", "focused", "selected", "pushed", "disabled"]);
    this._addCss("Grid>#controlcombo>#dropbutton", "border", obj, ["normal"]);
    this._addCss("GridControl>#controlcombo>#dropbutton", "border", obj, ["normal"]);

    obj = new nexacro.Style_gradation("linear 0,0 #ffffff 0,100 #e4e4e4");
    this._addCss("Combo>#dropbutton", "gradation", obj, ["normal"]);
    this._addCss("ComboControl>#dropbutton", "gradation", obj, ["normal"]);
    this._addCss("Grid>#controlcombo>#dropbutton", "gradation", obj, ["normal"]);
    this._addCss("GridControl>#controlcombo>#dropbutton", "gradation", obj, ["normal"]);

    obj = new nexacro.Style_bordertype("round","5","5","true","true","true","true");
    this._addCss("Combo>#combolist", "bordertype", obj, ["normal"]);
    this._addCss("ComboControl>#combolist", "bordertype", obj, ["normal"]);
    this._addCss("GroupBox", "bordertype", obj, ["normal"]);
    this._addCss("GroupBoxControl", "bordertype", obj, ["normal"]);
    this._addCss("Grid>#controlcombo>#combolist", "bordertype", obj, ["normal"]);
    this._addCss("GridControl>#controlcombo>#combolist", "bordertype", obj, ["normal"]);

    obj = new nexacro.Style_value("left middle");
    this._addCss("Radio", "buttonalign", obj, ["normal"]);

    obj = new nexacro.Style_background("","theme://images/radio_bg_N.png","","0","0","0","0","true");
    this._addCss("Radio", "buttonbackground", obj, ["normal"]);
    this._addCss("RadioControl", "buttonbackground", obj, ["normal"]);

    obj = new nexacro.Style_value("0 none #808080");
    this._addCss("Radio", "buttonborder", obj, ["normal"]);

    obj = new nexacro.Style_gradation("");
    this._addCss("Radio", "buttongradation", obj, ["normal"]);
    this._addCss("RadioControl", "buttongradation", obj, ["normal"]);
    this._addCss("CheckBox", "buttongradation", obj, ["normal"]);
    this._addCss("CheckBoxControl", "buttongradation", obj, ["normal"]);
    this._addCss("Step", "buttongradation", obj, ["normal"]);
    this._addCss("StepControl", "buttongradation", obj, ["normal"]);
    this._addCss("Grid>#controlcheckbox", "buttongradation", obj, ["normal"]);
    this._addCss("GridControl>#controlcheckbox", "buttongradation", obj, ["normal"]);

    obj = new nexacro.Style_value("theme://images/radio_ico.png");
    this._addCss("Radio", "buttonimage", obj, ["normal"]);
    this._addCss("RadioControl", "buttonimage", obj, ["normal"]);

    obj = new nexacro.Style_value("20");
    this._addCss("Radio", "buttonsize", obj, ["normal"]);
    this._addCss("RadioControl", "buttonsize", obj, ["normal"]);

    obj = new nexacro.Style_padding("0 0 0 0");
    this._addCss("Radio", "padding", obj, ["normal"]);
    this._addCss("RadioControl", "padding", obj, ["normal"]);
    this._addCss("CheckBox", "padding", obj, ["normal"]);
    this._addCss("CheckBoxControl", "padding", obj, ["normal"]);
    this._addCss("PopupDiv", "padding", obj, ["normal"]);
    this._addCss("PopupDivControl", "padding", obj, ["normal"]);
    this._addCss("DatePicker", "padding", obj, ["normal"]);
    this._addCss("DatePickerControl", "padding", obj, ["normal"]);
    this._addCss("Grid>#controlcheckbox", "padding", obj, ["normal"]);
    this._addCss("GridControl>#controlcheckbox", "padding", obj, ["normal"]);
    this._addCss("PopupMenu>#spindownbutton", "padding", obj, ["normal"]);
    this._addCss("PopupMenu>#spinupbutton", "padding", obj, ["normal"]);
    this._addCss("PopupMenuControl>#spindownbutton", "padding", obj, ["normal"]);
    this._addCss("PopupMenuControl>#spinupbutton", "padding", obj, ["normal"]);
    this._addCss("Menu>#spindownbutton", "padding", obj, ["normal"]);
    this._addCss("Menu>#spinupbutton", "padding", obj, ["normal"]);
    this._addCss("MenuControl>#spindownbutton", "padding", obj, ["normal"]);
    this._addCss("MenuControl>#spinupbutton", "padding", obj, ["normal"]);

    obj = new nexacro.Style_padding("0 0 0 5");
    this._addCss("Radio", "textpadding", obj, ["normal"]);
    this._addCss("RadioControl", "textpadding", obj, ["normal"]);
    this._addCss("CheckBox", "textpadding", obj, ["normal"]);
    this._addCss("CheckBoxControl", "textpadding", obj, ["normal"]);
    this._addCss("Grid>#controlcheckbox", "textpadding", obj, ["normal"]);
    this._addCss("GridControl>#controlcheckbox", "textpadding", obj, ["normal"]);

    obj = new nexacro.Style_background("","theme://images/radio_bg_P.png","","0","0","0","0","true");
    this._addCss("Radio", "buttonbackground", obj, ["mouseover", "selected"]);
    this._addCss("RadioControl", "buttonbackground", obj, ["mouseover", "selected"]);

    obj = new nexacro.Style_background("","theme://images/radio_bg_D.png","","0","0","0","0","true");
    this._addCss("Radio", "buttonbackground", obj, ["disabled"]);
    this._addCss("RadioControl", "buttonbackground", obj, ["disabled"]);

    obj = new nexacro.Style_value("theme://images/radio_ico_D.png");
    this._addCss("Radio", "buttonimage", obj, ["disabled"]);
    this._addCss("RadioControl", "buttonimage", obj, ["disabled"]);

    obj = new nexacro.Style_value("left middle");
    this._addCss("RadioControl", "buttonalign", obj, ["normal"]);

    obj = new nexacro.Style_value("0 none #808080");
    this._addCss("RadioControl", "buttonborder", obj, ["normal"]);
    this._addCss("CheckBoxControl", "buttonborder", obj, ["normal", "disabled"]);
    this._addCss("Grid>#controlcheckbox", "buttonborder", obj, ["normal", "disabled"]);
    this._addCss("GridControl>#controlcheckbox", "buttonborder", obj, ["normal", "disabled"]);

    obj = new nexacro.Style_value("left center");
    this._addCss("CheckBox", "buttonalign", obj, ["normal"]);

    obj = new nexacro.Style_background("","theme://images/chk_bg.png","","0","0","0","0","true");
    this._addCss("CheckBox", "buttonbackground", obj, ["normal"]);
    this._addCss("CheckBoxControl", "buttonbackground", obj, ["normal"]);
    this._addCss("Grid>#controlcheckbox", "buttonbackground", obj, ["normal"]);
    this._addCss("GridControl>#controlcheckbox", "buttonbackground", obj, ["normal"]);

    obj = new nexacro.Style_value("0 none #808080");
    this._addCss("CheckBox", "buttonborder", obj, ["normal", "disabled"]);

    obj = new nexacro.Style_bordertype("","0","0","true","true","true","true");
    this._addCss("CheckBox", "buttonbordertype", obj, ["normal"]);
    this._addCss("CheckBoxControl", "buttonbordertype", obj, ["normal"]);
    this._addCss("Grid>#controlcheckbox", "buttonbordertype", obj, ["normal"]);
    this._addCss("GridControl>#controlcheckbox", "buttonbordertype", obj, ["normal"]);

    obj = new nexacro.Style_value("theme://images/img_check.png");
    this._addCss("CheckBox", "buttonimage", obj, ["normal"]);
    this._addCss("CheckBoxControl", "buttonimage", obj, ["normal"]);
    this._addCss("Grid>#controlcheckbox", "buttonimage", obj, ["normal"]);
    this._addCss("GridControl>#controlcheckbox", "buttonimage", obj, ["normal"]);

    obj = new nexacro.Style_value("19");
    this._addCss("CheckBox", "buttonsize", obj, ["normal"]);
    this._addCss("CheckBoxControl", "buttonsize", obj, ["normal"]);
    this._addCss("Grid>#controlcheckbox", "buttonsize", obj, ["normal"]);
    this._addCss("GridControl>#controlcheckbox", "buttonsize", obj, ["normal"]);

    obj = new nexacro.Style_background("","theme://images/chk_bg_O.png","","0","0","0","0","true");
    this._addCss("CheckBox", "buttonbackground", obj, ["mouseover", "focused", "pushed"]);
    this._addCss("CheckBoxControl", "buttonbackground", obj, ["mouseover", "focused", "pushed"]);
    this._addCss("Grid>#controlcheckbox", "buttonbackground", obj, ["mouseover", "focused", "pushed"]);
    this._addCss("GridControl>#controlcheckbox", "buttonbackground", obj, ["mouseover", "focused", "pushed"]);

    obj = new nexacro.Style_background("","theme://images/chk_bg_D.png","","0","0","0","0","true");
    this._addCss("CheckBox", "buttonbackground", obj, ["disabled"]);
    this._addCss("CheckBoxControl", "buttonbackground", obj, ["disabled"]);
    this._addCss("Grid>#controlcheckbox", "buttonbackground", obj, ["disabled"]);
    this._addCss("GridControl>#controlcheckbox", "buttonbackground", obj, ["disabled"]);

    obj = new nexacro.Style_value("theme://images/img_check_D.png");
    this._addCss("CheckBox", "buttonimage", obj, ["disabled"]);
    this._addCss("CheckBoxControl", "buttonimage", obj, ["disabled"]);
    this._addCss("Grid>#controlcheckbox", "buttonimage", obj, ["disabled"]);
    this._addCss("GridControl>#controlcheckbox", "buttonimage", obj, ["disabled"]);

    obj = new nexacro.Style_value("left center");
    this._addCss("CheckBoxControl", "buttonalign", obj, ["normal"]);
    this._addCss("Grid>#controlcheckbox", "buttonalign", obj, ["normal"]);
    this._addCss("GridControl>#controlcheckbox", "buttonalign", obj, ["normal"]);

    obj = new nexacro.Style_border("2","solid","#3c3c44ff","");
    this._addCss("PopupDiv", "border", obj, ["normal"]);
    this._addCss("PopupDivControl", "border", obj, ["normal"]);

    obj = new nexacro.Style_shadow("drop 0,1 2 #00000066");
    this._addCss("PopupDiv", "shadow", obj, ["normal"]);
    this._addCss("PopupDivControl", "shadow", obj, ["normal"]);

    obj = new nexacro.Style_background("#ffffff3f","","","0","0","0","0","true");
    this._addCss("Step", "buttonbackground", obj, ["normal"]);
    this._addCss("StepControl", "buttonbackground", obj, ["normal"]);

    obj = new nexacro.Style_value("0 none");
    this._addCss("Step", "buttonborder", obj, ["normal"]);

    obj = new nexacro.Style_bordertype("round","7","7","true","true","true","true");
    this._addCss("Step", "buttonbordertype", obj, ["normal"]);
    this._addCss("StepControl", "buttonbordertype", obj, ["normal"]);

    obj = new nexacro.Style_value("theme://images/ico_step.png");
    this._addCss("Step", "buttonimage", obj, ["normal"]);
    this._addCss("StepControl", "buttonimage", obj, ["normal"]);

    obj = new nexacro.Style_padding("0 5 0 5");
    this._addCss("Step", "buttonpadding", obj, ["normal"]);
    this._addCss("StepControl", "buttonpadding", obj, ["normal"]);

    obj = new nexacro.Style_value("13");
    this._addCss("Step", "buttonsize", obj, ["normal"]);
    this._addCss("StepControl", "buttonsize", obj, ["normal"]);

    obj = new nexacro.Style_color("#3f3f3fb3");
    this._addCss("Step", "color", obj, ["normal"]);
    this._addCss("StepControl", "color", obj, ["normal"]);

    obj = new nexacro.Style_padding("5 5 5 5");
    this._addCss("Step", "padding", obj, ["normal"]);
    this._addCss("StepControl", "padding", obj, ["normal"]);

    obj = new nexacro.Style_value("0 none");
    this._addCss("StepControl", "buttonborder", obj, ["normal"]);

    obj = new nexacro.Style_border("1","solid","#f48400ff","");
    this._addCss("ImageViewer", "border", obj, ["normal"]);
    this._addCss("ImageViewerControl", "border", obj, ["normal"]);

    obj = new nexacro.Style_color("#a5a5a5");
    this._addCss("ImageViewer", "color", obj, ["disabled"]);
    this._addCss("ImageViewerControl", "color", obj, ["disabled"]);

    obj = new nexacro.Style_border("1","solid","#e5e5e5ff","");
    this._addCss("ImageViewer", "border", obj, ["disabled"]);
    this._addCss("ImageViewerControl", "border", obj, ["disabled"]);

    obj = new nexacro.Style_border("1","solid","#d5d5d5ff","");
    this._addCss("GroupBox", "border", obj, ["normal"]);
    this._addCss("GroupBoxControl", "border", obj, ["normal"]);

    obj = new nexacro.Style_background("","","","0","0","0","0","true");
    this._addCss("GroupBox", "titlebackground", obj, ["normal"]);
    this._addCss("GroupBoxControl", "titlebackground", obj, ["normal"]);

    obj = new nexacro.Style_gradation("");
    this._addCss("GroupBox", "titlegradation", obj, ["normal"]);
    this._addCss("GroupBoxControl", "titlegradation", obj, ["normal"]);

    obj = new nexacro.Style_value("");
    this._addCss("GroupBox", "titleimage", obj, ["normal"]);
    this._addCss("GroupBoxControl", "titleimage", obj, ["normal"]);

    obj = new nexacro.Style_align("");
    this._addCss("GroupBox", "titleimagealign", obj, ["normal"]);
    this._addCss("GroupBoxControl", "titleimagealign", obj, ["normal"]);

    obj = new nexacro.Style_value("");
    this._addCss("GroupBox", "titleview", obj, ["normal"]);
    this._addCss("GroupBoxControl", "titleview", obj, ["normal"]);

    obj = new nexacro.Style_value("30%");
    this._addCss("GroupBox", "opacity", obj, ["disabled"]);
    this._addCss("GroupBoxControl", "opacity", obj, ["disabled"]);

    obj = new nexacro.Style_border("1","solid","#d5d5d5ff","","1","solid","#a6a6a9ff","","1","solid","#a6a6a9ff","","1","solid","#d5d5d5ff","");
    this._addCss("DatePicker", "border", obj, ["normal"]);
    this._addCss("DatePickerControl", "border", obj, ["normal"]);

    obj = new nexacro.Style_color("#405980");
    this._addCss("DatePicker", "color", obj, ["normal"]);
    this._addCss("DatePickerControl", "color", obj, ["normal"]);

    obj = new nexacro.Style_padding("35 0 0 0");
    this._addCss("DatePicker", "ncpadding", obj, ["normal"]);
    this._addCss("DatePickerControl", "ncpadding", obj, ["normal"]);

    obj = new nexacro.Style_value("yyyy.MM");
    this._addCss("DatePicker", "headerformat", obj, ["normal"]);
    this._addCss("DatePickerControl", "headerformat", obj, ["normal"]);

    obj = new nexacro.Style_value("");
    this._addCss("DatePicker", "weekformat", obj, ["normal"]);
    this._addCss("DatePickerControl", "weekformat", obj, ["normal"]);

    obj = new nexacro.Style_value("22,22");
    this._addCss("DatePicker", "daysize", obj, ["normal"]);
    this._addCss("DatePickerControl", "daysize", obj, ["normal"]);

    obj = new nexacro.Style_color("#333333");
    this._addCss("DatePicker", "daycolor", obj, ["normal"]);
    this._addCss("DatePickerControl", "daycolor", obj, ["normal"]);

    obj = new nexacro.Style_background("","","","0","0","0","0","true");
    this._addCss("DatePicker", "daybackground", obj, ["normal"]);
    this._addCss("DatePickerControl", "daybackground", obj, ["normal"]);

    obj = new nexacro.Style_gradation("");
    this._addCss("DatePicker", "daygradation", obj, ["normal", "mouseover", "selected"]);
    this._addCss("DatePickerControl", "daygradation", obj, ["normal", "mouseover", "selected"]);
    this._addCss("Calendar", "daygradation", obj, ["normal"]);
    this._addCss("CalendarControl", "daygradation", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar", "daygradation", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar", "daygradation", obj, ["normal"]);

    obj = new nexacro.Style_border("1","solid","#ffffffff","");
    this._addCss("DatePicker", "dayborder", obj, ["normal"]);
    this._addCss("DatePickerControl", "dayborder", obj, ["normal"]);

    obj = new nexacro.Style_bordertype("normal","0","0","true","true","true","true");
    this._addCss("DatePicker", "daybordertype", obj, ["normal", "mouseover", "selected"]);
    this._addCss("DatePickerControl", "daybordertype", obj, ["normal", "mouseover", "selected"]);

    obj = new nexacro.Style_font("9 Dotum");
    this._addCss("DatePicker", "dayfont", obj, ["normal", "mouseover", "selected"]);
    this._addCss("DatePickerControl", "dayfont", obj, ["normal", "mouseover", "selected"]);
    this._addCss("Calendar", "dayfont", obj, ["normal"]);
    this._addCss("CalendarControl", "dayfont", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar", "dayfont", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar", "dayfont", obj, ["normal"]);

    obj = new nexacro.Style_value("35");
    this._addCss("DatePicker", "headerheight", obj, ["normal"]);
    this._addCss("DatePickerControl", "headerheight", obj, ["normal"]);

    obj = new nexacro.Style_color("#444444");
    this._addCss("DatePicker", "headercolor", obj, ["normal"]);

    obj = new nexacro.Style_background("","","","0","0","0","0","true");
    this._addCss("DatePicker", "headerbackground", obj, ["normal"]);
    this._addCss("DatePickerControl", "headerbackground", obj, ["normal"]);

    obj = new nexacro.Style_gradation("");
    this._addCss("DatePicker", "headergradation", obj, ["normal"]);
    this._addCss("DatePickerControl", "headergradation", obj, ["normal"]);

    obj = new nexacro.Style_border("0","none","","");
    this._addCss("DatePicker", "headerborder", obj, ["normal"]);
    this._addCss("DatePickerControl", "headerborder", obj, ["normal"]);

    obj = new nexacro.Style_bordertype("normal","0","0","true","true","true","true");
    this._addCss("DatePicker", "headerbordertype", obj, ["normal"]);
    this._addCss("DatePickerControl", "headerbordertype", obj, ["normal"]);

    obj = new nexacro.Style_font("11 Dotum,Dotum");
    this._addCss("DatePicker", "headerfont", obj, ["normal"]);

    obj = new nexacro.Style_background("","theme://images/cal_bg.png","","0","0","0","0","true");
    this._addCss("DatePicker", "bodybackground", obj, ["normal"]);
    this._addCss("DatePickerControl", "bodybackground", obj, ["normal"]);

    obj = new nexacro.Style_gradation("");
    this._addCss("DatePicker", "bodygradation", obj, ["normal"]);
    this._addCss("DatePickerControl", "bodygradation", obj, ["normal"]);

    obj = new nexacro.Style_border("0","none","","");
    this._addCss("DatePicker", "bodyborder", obj, ["normal"]);
    this._addCss("DatePickerControl", "bodyborder", obj, ["normal"]);

    obj = new nexacro.Style_bordertype("normal","0","0","true","true","true","true");
    this._addCss("DatePicker", "bodybordertype", obj, ["normal"]);
    this._addCss("DatePickerControl", "bodybordertype", obj, ["normal"]);

    obj = new nexacro.Style_color("#333333");
    this._addCss("DatePicker", "weekcolor", obj, ["normal"]);

    obj = new nexacro.Style_background("","theme://images/bg_CalWeek.png","repeat-x","0","0","0","0","true");
    this._addCss("DatePicker", "weekbackground", obj, ["normal"]);
    this._addCss("DatePickerControl", "weekbackground", obj, ["normal"]);

    obj = new nexacro.Style_gradation("");
    this._addCss("DatePicker", "weekgradation", obj, ["normal"]);
    this._addCss("DatePickerControl", "weekgradation", obj, ["normal"]);

    obj = new nexacro.Style_font("");
    this._addCss("DatePicker", "weekfont", obj, ["normal"]);
    this._addCss("DatePickerControl", "weekfont", obj, ["normal"]);

    obj = new nexacro.Style_color("#ffffff");
    this._addCss("DatePicker", "todaycolor", obj, ["normal"]);
    this._addCss("DatePickerControl", "todaycolor", obj, ["normal"]);

    obj = new nexacro.Style_background("","theme://images/bg_CalToday.png","stretch","1","1","0","0","true");
    this._addCss("DatePicker", "todaybackground", obj, ["normal"]);
    this._addCss("DatePickerControl", "todaybackground", obj, ["normal"]);

    obj = new nexacro.Style_gradation("");
    this._addCss("DatePicker", "todaygradation", obj, ["normal"]);
    this._addCss("DatePickerControl", "todaygradation", obj, ["normal"]);

    obj = new nexacro.Style_border("1","solid","#995a00ff","");
    this._addCss("DatePicker", "todayborder", obj, ["normal"]);
    this._addCss("DatePickerControl", "todayborder", obj, ["normal"]);

    obj = new nexacro.Style_bordertype("normal","0","0","true","true","true","true");
    this._addCss("DatePicker", "todaybordertype", obj, ["normal"]);
    this._addCss("DatePickerControl", "todaybordertype", obj, ["normal"]);

    obj = new nexacro.Style_font("9 Dotum");
    this._addCss("DatePicker", "todayfont", obj, ["normal"]);
    this._addCss("DatePickerControl", "todayfont", obj, ["normal"]);

    obj = new nexacro.Style_background("","","","0","0","0","0","true");
    this._addCss("DatePicker", "saturdaybackground", obj, ["normal"]);
    this._addCss("DatePickerControl", "saturdaybackground", obj, ["normal"]);

    obj = new nexacro.Style_gradation("");
    this._addCss("DatePicker", "saturdaygradation", obj, ["normal"]);
    this._addCss("DatePickerControl", "saturdaygradation", obj, ["normal"]);

    obj = new nexacro.Style_border("1","solid","#ffffff00","");
    this._addCss("DatePicker", "saturdayborder", obj, ["normal"]);

    obj = new nexacro.Style_bordertype("","0","0","true","true","true","true");
    this._addCss("DatePicker", "saturdaybordertype", obj, ["normal"]);
    this._addCss("DatePickerControl", "saturdaybordertype", obj, ["normal"]);

    obj = new nexacro.Style_font("9 Dotum");
    this._addCss("DatePicker", "saturdayfont", obj, ["normal", "mouseover"]);
    this._addCss("DatePickerControl", "saturdayfont", obj, ["normal", "mouseover"]);

    obj = new nexacro.Style_color("#669933");
    this._addCss("DatePicker", "saturdaycolor", obj, ["normal"]);
    this._addCss("DatePickerControl", "saturdaycolor", obj, ["normal"]);

    obj = new nexacro.Style_background("","","","0","0","0","0","true");
    this._addCss("DatePicker", "sundaybackground", obj, ["normal"]);
    this._addCss("DatePickerControl", "sundaybackground", obj, ["normal"]);

    obj = new nexacro.Style_gradation("");
    this._addCss("DatePicker", "sundaygradation", obj, ["normal"]);
    this._addCss("DatePickerControl", "sundaygradation", obj, ["normal"]);

    obj = new nexacro.Style_border("1","solid","#ffffff00","");
    this._addCss("DatePicker", "sundayborder", obj, ["normal"]);

    obj = new nexacro.Style_bordertype("","0","0","true","true","true","true");
    this._addCss("DatePicker", "sundaybordertype", obj, ["normal"]);
    this._addCss("DatePickerControl", "sundaybordertype", obj, ["normal"]);

    obj = new nexacro.Style_color("#f0770a");
    this._addCss("DatePicker", "sundaycolor", obj, ["normal"]);
    this._addCss("DatePickerControl", "sundaycolor", obj, ["normal"]);

    obj = new nexacro.Style_font("9 Dotum");
    this._addCss("DatePicker", "sundayfont", obj, ["normal", "mouseover", "selected"]);
    this._addCss("DatePickerControl", "sundayfont", obj, ["normal", "mouseover", "selected"]);

    obj = new nexacro.Style_color("#ffffff");
    this._addCss("DatePicker", "daycolor", obj, ["mouseover", "selected"]);
    this._addCss("DatePickerControl", "daycolor", obj, ["mouseover", "selected"]);
    this._addCss("Calendar", "daycolor", obj, ["normal", "mouseover"]);
    this._addCss("CalendarControl", "daycolor", obj, ["normal", "mouseover"]);
    this._addCss("Grid>#controlcalendar", "daycolor", obj, ["normal", "mouseover"]);
    this._addCss("GridControl>#controlcalendar", "daycolor", obj, ["normal", "mouseover"]);

    obj = new nexacro.Style_background("","theme://images/bg_daybackground_O.png","stretch","1","1","0","0","true");
    this._addCss("DatePicker", "daybackground", obj, ["mouseover"]);
    this._addCss("DatePickerControl", "daybackground", obj, ["mouseover"]);

    obj = new nexacro.Style_border("1","solid","#9eb9caff","");
    this._addCss("DatePicker", "dayborder", obj, ["mouseover"]);
    this._addCss("DatePickerControl", "dayborder", obj, ["mouseover"]);

    obj = new nexacro.Style_color("#ffffff");
    this._addCss("DatePicker", "saturdaycolor", obj, ["mouseover", "selected"]);
    this._addCss("DatePickerControl", "saturdaycolor", obj, ["mouseover", "selected"]);

    obj = new nexacro.Style_background("","theme://images/bg_daybackground_O.png","stretch","1","1","0","0","true");
    this._addCss("DatePicker", "saturdaybackground", obj, ["mouseover"]);
    this._addCss("DatePickerControl", "saturdaybackground", obj, ["mouseover"]);

    obj = new nexacro.Style_border("1","solid","#9eb9caff","");
    this._addCss("DatePicker", "saturdayborder", obj, ["mouseover"]);
    this._addCss("DatePickerControl", "saturdayborder", obj, ["mouseover"]);

    obj = new nexacro.Style_bordertype("normal","0","0","true","true","true","true");
    this._addCss("DatePicker", "saturdaybordertype", obj, ["mouseover", "selected"]);
    this._addCss("DatePickerControl", "saturdaybordertype", obj, ["mouseover", "selected"]);

    obj = new nexacro.Style_color("#ffffff");
    this._addCss("DatePicker", "sundaycolor", obj, ["mouseover", "selected"]);
    this._addCss("DatePickerControl", "sundaycolor", obj, ["mouseover", "selected"]);

    obj = new nexacro.Style_background("","theme://images/bg_daybackground_O.png","stretch","1","1","0","0","true");
    this._addCss("DatePicker", "sundaybackground", obj, ["mouseover"]);
    this._addCss("DatePickerControl", "sundaybackground", obj, ["mouseover"]);

    obj = new nexacro.Style_border("1","solid","#9eb9caff","");
    this._addCss("DatePicker", "sundayborder", obj, ["mouseover"]);
    this._addCss("DatePickerControl", "sundayborder", obj, ["mouseover"]);

    obj = new nexacro.Style_bordertype("normal","0","0","true","true","true","true");
    this._addCss("DatePicker", "sundaybordertype", obj, ["mouseover", "selected"]);
    this._addCss("DatePickerControl", "sundaybordertype", obj, ["mouseover", "selected"]);

    obj = new nexacro.Style_background("","theme://images/bg_daybackground_S.png","stretch","1","1","0","0","true");
    this._addCss("DatePicker", "daybackground", obj, ["selected"]);
    this._addCss("DatePickerControl", "daybackground", obj, ["selected"]);

    obj = new nexacro.Style_border("1","solid","#045c92ff","");
    this._addCss("DatePicker", "dayborder", obj, ["selected"]);
    this._addCss("DatePickerControl", "dayborder", obj, ["selected"]);

    obj = new nexacro.Style_background("","theme://images/bg_daybackground_S.png","stretch","1","1","0","0","true");
    this._addCss("DatePicker", "saturdaybackground", obj, ["selected"]);
    this._addCss("DatePickerControl", "saturdaybackground", obj, ["selected"]);

    obj = new nexacro.Style_border("1","solid","#045c92ff","");
    this._addCss("DatePicker", "saturdayborder", obj, ["selected"]);
    this._addCss("DatePickerControl", "saturdayborder", obj, ["selected"]);

    obj = new nexacro.Style_font("antialias bold 9 Arial");
    this._addCss("DatePicker", "saturdayfont", obj, ["selected"]);
    this._addCss("DatePickerControl", "saturdayfont", obj, ["selected"]);

    obj = new nexacro.Style_background("","theme://images/bg_daybackground_S.png","stretch","1","1","0","0","true");
    this._addCss("DatePicker", "sundaybackground", obj, ["selected"]);
    this._addCss("DatePickerControl", "sundaybackground", obj, ["selected"]);

    obj = new nexacro.Style_border("1","solid","#045c92ff","");
    this._addCss("DatePicker", "sundayborder", obj, ["selected"]);
    this._addCss("DatePickerControl", "sundayborder", obj, ["selected"]);

    obj = new nexacro.Style_color("#ffffff");
    this._addCss("DatePickerControl", "headercolor", obj, ["normal"]);

    obj = new nexacro.Style_font("15 bold antialias Dotum");
    this._addCss("DatePickerControl", "headerfont", obj, ["normal"]);

    obj = new nexacro.Style_color("#ffffff");
    this._addCss("DatePickerControl", "weekcolor", obj, ["normal"]);

    obj = new nexacro.Style_border("0","none","#808080ff","","1","solid","#000000ff","","1","solid","#000000ff","","0","none","#808080ff","");
    this._addCss("DatePickerControl", "saturdayborder", obj, ["normal"]);

    obj = new nexacro.Style_border("0","none","#808080ff","","1","solid","#000000ff","","1","solid","#000000ff","","1","solid","#000000ff","");
    this._addCss("DatePickerControl", "sundayborder", obj, ["normal"]);

    obj = new nexacro.Style_background("","","","0","0","0","0","true");
    this._addCss("Calendar", "daybackground", obj, ["normal"]);
    this._addCss("CalendarControl", "daybackground", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar", "daybackground", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar", "daybackground", obj, ["normal"]);

    obj = new nexacro.Style_border("0","none","#808080ff","","1","solid","#000000ff","","1","solid","#000000ff","","0","solid","","");
    this._addCss("Calendar", "dayborder", obj, ["normal"]);
    this._addCss("CalendarControl", "dayborder", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar", "dayborder", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar", "dayborder", obj, ["normal"]);

    obj = new nexacro.Style_bordertype("","0","0","true","true","true","true");
    this._addCss("Calendar", "daybordertype", obj, ["normal"]);
    this._addCss("CalendarControl", "daybordertype", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar", "daybordertype", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar", "daybordertype", obj, ["normal"]);

    obj = new nexacro.Style_value("32 30");
    this._addCss("Calendar", "daysize", obj, ["normal"]);
    this._addCss("CalendarControl", "daysize", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar", "daysize", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar", "daysize", obj, ["normal"]);

    obj = new nexacro.Style_background("#35363bff","","","0","0","0","0","true");
    this._addCss("Calendar", "popupbackground", obj, ["normal"]);
    this._addCss("CalendarControl", "popupbackground", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar", "popupbackground", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar", "popupbackground", obj, ["normal"]);
    this._addCss("Menu", "popupbackground", obj, ["normal"]);
    this._addCss("MenuControl", "popupbackground", obj, ["normal"]);

    obj = new nexacro.Style_border("1","solid","#000000ff","");
    this._addCss("Calendar", "popupborder", obj, ["normal"]);
    this._addCss("CalendarControl", "popupborder", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar", "popupborder", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar", "popupborder", obj, ["normal"]);

    obj = new nexacro.Style_bordertype("round","5","5","true","true","true","true");
    this._addCss("Calendar", "popupbordertype", obj, ["normal"]);
    this._addCss("CalendarControl", "popupbordertype", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar", "popupbordertype", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar", "popupbordertype", obj, ["normal"]);
    this._addCss("Menu", "popupbordertype", obj, ["normal"]);
    this._addCss("MenuControl", "popupbordertype", obj, ["normal"]);

    obj = new nexacro.Style_gradation("");
    this._addCss("Calendar", "popupgradation", obj, ["normal"]);
    this._addCss("CalendarControl", "popupgradation", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar", "popupgradation", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar", "popupgradation", obj, ["normal"]);
    this._addCss("Menu", "popupgradation", obj, ["normal"]);
    this._addCss("MenuControl", "popupgradation", obj, ["normal"]);

    obj = new nexacro.Style_value("223 245");
    this._addCss("Calendar", "popupsize", obj, ["normal"]);
    this._addCss("CalendarControl", "popupsize", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar", "popupsize", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar", "popupsize", obj, ["normal"]);

    obj = new nexacro.Style_background("@gradation","","","0","0","0","0","true");
    this._addCss("Calendar", "daybackground", obj, ["mouseover"]);
    this._addCss("CalendarControl", "daybackground", obj, ["mouseover"]);
    this._addCss("Grid>#controlcalendar", "daybackground", obj, ["mouseover"]);
    this._addCss("GridControl>#controlcalendar", "daybackground", obj, ["mouseover"]);

    obj = new nexacro.Style_gradation("linear 0,0 #58595dff 0,100 #35363bff [3% #45464aff]");
    this._addCss("Calendar", "daygradation", obj, ["mouseover"]);
    this._addCss("CalendarControl", "daygradation", obj, ["mouseover"]);
    this._addCss("Grid>#controlcalendar", "daygradation", obj, ["mouseover"]);
    this._addCss("GridControl>#controlcalendar", "daygradation", obj, ["mouseover"]);

    obj = new nexacro.Style_border("0","none","#808080ff","","1","solid","#000000ff","","1","solid","#000000ff","","0","none","#808080ff","");
    this._addCss("Calendar", "dayborder", obj, ["mouseover", "selected"]);
    this._addCss("CalendarControl", "dayborder", obj, ["mouseover", "selected"]);
    this._addCss("Grid>#controlcalendar", "dayborder", obj, ["mouseover", "selected"]);
    this._addCss("GridControl>#controlcalendar", "dayborder", obj, ["mouseover", "selected"]);

    obj = new nexacro.Style_color("#000000");
    this._addCss("Calendar", "daycolor", obj, ["selected"]);
    this._addCss("CalendarControl", "daycolor", obj, ["selected"]);
    this._addCss("Grid>#controlcalendar", "daycolor", obj, ["selected"]);
    this._addCss("GridControl>#controlcalendar", "daycolor", obj, ["selected"]);

    obj = new nexacro.Style_background("#ffebbcff","","","0","0","0","0","true");
    this._addCss("Calendar", "daybackground", obj, ["selected"]);
    this._addCss("CalendarControl", "daybackground", obj, ["selected"]);
    this._addCss("Grid>#controlcalendar", "daybackground", obj, ["selected"]);
    this._addCss("GridControl>#controlcalendar", "daybackground", obj, ["selected"]);

    obj = new nexacro.Style_padding("0 5 0 5");
    this._addCss("Calendar>#calendaredit", "padding", obj, ["normal"]);
    this._addCss("CalendarControl>#calendaredit", "padding", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#calendaredit", "padding", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#calendaredit", "padding", obj, ["normal"]);

    obj = new nexacro.Style_bordertype("normal","0","0","true","true","true","true");
    this._addCss("Calendar>#dropbutton", "bordertype", obj, ["normal", "mouseover", "pushed", "focused"]);
    this._addCss("CalendarControl>#dropbutton", "bordertype", obj, ["normal", "mouseover", "pushed", "focused"]);
    this._addCss("Grid>#controlcalendar>#dropbutton", "bordertype", obj, ["normal", "mouseover", "pushed", "focused"]);
    this._addCss("GridControl>#controlcalendar>#dropbutton", "bordertype", obj, ["normal", "mouseover", "pushed", "focused"]);

    obj = new nexacro.Style_value("theme://images/btn_calendar_N.png");
    this._addCss("Calendar>#dropbutton", "image", obj, ["normal", "mouseover", "pushed", "focused"]);
    this._addCss("CalendarControl>#dropbutton", "image", obj, ["normal", "mouseover", "pushed", "focused"]);
    this._addCss("Grid>#controlcalendar>#dropbutton", "image", obj, ["normal", "mouseover", "pushed", "focused"]);
    this._addCss("GridControl>#controlcalendar>#dropbutton", "image", obj, ["normal", "mouseover", "pushed", "focused"]);

    obj = new nexacro.Style_align("right middle");
    this._addCss("Calendar>#dropbutton", "imagealign", obj, ["normal", "mouseover", "pushed", "focused"]);
    this._addCss("CalendarControl>#dropbutton", "imagealign", obj, ["normal", "mouseover", "pushed", "focused"]);
    this._addCss("Grid>#controlcalendar>#dropbutton", "imagealign", obj, ["normal", "mouseover", "pushed", "focused"]);
    this._addCss("GridControl>#controlcalendar>#dropbutton", "imagealign", obj, ["normal", "mouseover", "pushed", "focused"]);

    obj = new nexacro.Style_border("1","solid","transparent","");
    this._addCss("Calendar>#spinupbutton", "border", obj, ["normal"]);
    this._addCss("Calendar>#spindownbutton", "border", obj, ["normal"]);
    this._addCss("CalendarControl>#spinupbutton", "border", obj, ["normal"]);
    this._addCss("CalendarControl>#spindownbutton", "border", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#spinupbutton", "border", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#spindownbutton", "border", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#spinupbutton", "border", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#spindownbutton", "border", obj, ["normal"]);

    obj = new nexacro.Style_background("","theme://images/btn_CalPrev_N.png","","0","0","50","50","true");
    this._addCss("Calendar>#popupcalendar>#prevbutton", "background", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#prevbutton", "background", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#prevbutton", "background", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#prevbutton", "background", obj, ["normal"]);

    obj = new nexacro.Style_background("","theme://images/btn_CalPrev_O.png","","0","0","50","50","true");
    this._addCss("Calendar>#popupcalendar>#prevbutton", "background", obj, ["mouseover", "selected", "focused"]);
    this._addCss("CalendarControl>#popupcalendar>#prevbutton", "background", obj, ["mouseover", "selected", "focused"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#prevbutton", "background", obj, ["mouseover", "selected", "focused"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#prevbutton", "background", obj, ["mouseover", "selected", "focused"]);

    obj = new nexacro.Style_background("","theme://images/btn_CalPrev_P.png","","0","0","50","50","true");
    this._addCss("Calendar>#popupcalendar>#prevbutton", "background", obj, ["pushed"]);
    this._addCss("CalendarControl>#popupcalendar>#prevbutton", "background", obj, ["pushed"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#prevbutton", "background", obj, ["pushed"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#prevbutton", "background", obj, ["pushed"]);

    obj = new nexacro.Style_background("","theme://images/btn_CalNext_N.png","","0","0","50","50","true");
    this._addCss("Calendar>#popupcalendar>#nextbutton", "background", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#nextbutton", "background", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#nextbutton", "background", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#nextbutton", "background", obj, ["normal"]);

    obj = new nexacro.Style_background("","theme://images/btn_CalNext_O.png","","0","0","50","50","true");
    this._addCss("Calendar>#popupcalendar>#nextbutton", "background", obj, ["mouseover", "selected", "focused"]);
    this._addCss("CalendarControl>#popupcalendar>#nextbutton", "background", obj, ["mouseover", "selected", "focused"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#nextbutton", "background", obj, ["mouseover", "selected", "focused"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#nextbutton", "background", obj, ["mouseover", "selected", "focused"]);

    obj = new nexacro.Style_background("","theme://images/btn_CalNext_P.png","","0","0","50","50","true");
    this._addCss("Calendar>#popupcalendar>#nextbutton", "background", obj, ["pushed"]);
    this._addCss("CalendarControl>#popupcalendar>#nextbutton", "background", obj, ["pushed"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#nextbutton", "background", obj, ["pushed"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#nextbutton", "background", obj, ["pushed"]);

    obj = new nexacro.Style_value("15");
    this._addCss("Calendar>#popupcalendar>#yearspin", "buttonsize", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#monthspin", "buttonsize", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#yearspin", "buttonsize", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#monthspin", "buttonsize", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#yearspin", "buttonsize", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#monthspin", "buttonsize", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#yearspin", "buttonsize", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#monthspin", "buttonsize", obj, ["normal"]);

    obj = new nexacro.Style_color("#555555");
    this._addCss("Calendar>#popupcalendar>#yearspin", "color", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#yearspin>#spinedit", "color", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#monthspin", "color", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#monthspin>#spinedit", "color", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#yearspin", "color", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#yearspin>#spinedit", "color", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#monthspin", "color", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#monthspin>#spinedit", "color", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#yearspin", "color", obj, ["normal"]);
    this._addCss("Grid>#controlcalendar>#popupcalendar>#monthspin", "color", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#yearspin", "color", obj, ["normal"]);
    this._addCss("GridControl>#controlcalendar>#popupcalendar>#monthspin", "color", obj, ["normal"]);

    obj = new nexacro.Style_padding("0 0 0 3");
    this._addCss("Calendar>#popupcalendar>#yearspin>#spinedit", "padding", obj, ["normal"]);
    this._addCss("Calendar>#popupcalendar>#monthspin>#spinedit", "padding", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#yearspin>#spinedit", "padding", obj, ["normal"]);
    this._addCss("CalendarControl>#popupcalendar>#monthspin>#spinedit", "padding", obj, ["normal"]);

    obj = new nexacro.Style_background("","theme://images/btn_CalSpinup_N.png","","0","0","50","50","true");
    this._addCss("Calendar>#popupcalendar>#yearspin>#spinupbutton", "background", obj, ["normal", "mouseover", "pushed"]);
    this._addCss("Calendar>#popupcalendar>#monthspin>#spinupbutton", "background", obj, ["normal", "mouseover", "pushed"]);
    this._addCss("CalendarControl>#popupcalendar>#yearspin>#spinupbutton", "background", obj, ["normal", "mouseover", "pushed"]);
    this._addCss("CalendarControl>#popupcalendar>#monthspin>#spinupbutton", "background", obj, ["normal", "mouseover", "pushed"]);

    obj = new nexacro.Style_background("","theme://images/btn_CalSpindn_N.png","","0","0","50","0","true");
    this._addCss("Calendar>#popupcalendar>#yearspin>#spindownbutton", "background", obj, ["normal", "mouseover", "pushed"]);
    this._addCss("Calendar>#popupcalendar>#monthspin>#spindownbutton", "background", obj, ["normal", "mouseover", "pushed"]);
    this._addCss("CalendarControl>#popupcalendar>#yearspin>#spindownbutton", "background", obj, ["normal", "mouseover", "pushed"]);
    this._addCss("CalendarControl>#popupcalendar>#monthspin>#spindownbutton", "background", obj, ["normal", "mouseover", "pushed"]);

    obj = new nexacro.Style_border("1","solid","#202020ff","");
    this._addCss("Grid", "border", obj, ["normal"]);
    this._addCss("GridControl", "border", obj, ["normal"]);
    this._addCss("Tab", "border", obj, ["normal"]);
    this._addCss("TabControl", "border", obj, ["normal"]);

    obj = new nexacro.Style_font("13 bold Dotum,Dotum");
    this._addCss("Grid", "font", obj, ["normal"]);
    this._addCss("GridControl", "font", obj, ["normal"]);

    obj = new nexacro.Style_border("");
    this._addCss("Grid", "line", obj, ["normal"]);
    this._addCss("GridControl", "line", obj, ["normal"]);

    obj = new nexacro.Style_value("100%");
    this._addCss("Grid", "opacity", obj, ["normal"]);
    this._addCss("GridControl", "opacity", obj, ["normal"]);

    obj = new nexacro.Style_border("");
    this._addCss("Grid", "selectline", obj, ["normal"]);
    this._addCss("Grid>#head", "selectline", obj, ["normal"]);
    this._addCss("Grid>#summ", "selectline", obj, ["normal"]);
    this._addCss("GridControl", "selectline", obj, ["normal"]);
    this._addCss("GridControl>#head", "selectline", obj, ["normal"]);
    this._addCss("GridControl>#summ", "selectline", obj, ["normal"]);

    obj = new nexacro.Style_value("");
    this._addCss("Grid", "selectlinetype", obj, ["normal"]);
    this._addCss("Grid>#head", "selectlinetype", obj, ["normal"]);
    this._addCss("Grid>#body", "selectlinetype", obj, ["normal"]);
    this._addCss("Grid>#summ", "selectlinetype", obj, ["normal"]);
    this._addCss("GridControl", "selectlinetype", obj, ["normal"]);
    this._addCss("GridControl>#head", "selectlinetype", obj, ["normal"]);
    this._addCss("GridControl>#body", "selectlinetype", obj, ["normal"]);
    this._addCss("GridControl>#summ", "selectlinetype", obj, ["normal"]);

    obj = new nexacro.Style_value("");
    this._addCss("Grid", "treeclosebuttonimage", obj, ["normal"]);
    this._addCss("GridControl", "treeclosebuttonimage", obj, ["normal"]);

    obj = new nexacro.Style_value("");
    this._addCss("Grid", "treecollapseimage", obj, ["normal"]);
    this._addCss("GridControl", "treecollapseimage", obj, ["normal"]);

    obj = new nexacro.Style_value("");
    this._addCss("Grid", "treeexpandimage", obj, ["normal"]);
    this._addCss("GridControl", "treeexpandimage", obj, ["normal"]);

    obj = new nexacro.Style_value("");
    this._addCss("Grid", "treeitemimage", obj, ["normal"]);
    this._addCss("GridControl", "treeitemimage", obj, ["normal"]);

    obj = new nexacro.Style_value("");
    this._addCss("Grid", "treeitemmargin", obj, ["normal"]);
    this._addCss("GridControl", "treeitemmargin", obj, ["normal"]);

    obj = new nexacro.Style_line("0","","","");
    this._addCss("Grid", "treelinetype", obj, ["normal"]);
    this._addCss("GridControl", "treelinetype", obj, ["normal"]);

    obj = new nexacro.Style_value("");
    this._addCss("Grid", "treeopenbuttonimage", obj, ["normal"]);
    this._addCss("GridControl", "treeopenbuttonimage", obj, ["normal"]);

    obj = new nexacro.Style_border("0","none","#ffffff00","");
    this._addCss("Grid>#head", "border", obj, ["normal"]);
    this._addCss("GridControl>#head", "border", obj, ["normal"]);

    obj = new nexacro.Style_align("center middle");
    this._addCss("Grid>#head", "cellalign", obj, ["normal"]);
    this._addCss("Grid>#body", "cellalign", obj, ["normal"]);
    this._addCss("GridControl>#head", "cellalign", obj, ["normal"]);
    this._addCss("GridControl>#body", "cellalign", obj, ["normal"]);

    obj = new nexacro.Style_background("","","","0","0","0","0","true");
    this._addCss("Grid>#head", "cellbackground", obj, ["normal"]);
    this._addCss("Grid>#summ", "cellbackground", obj, ["normal"]);
    this._addCss("GridControl>#head", "cellbackground", obj, ["normal"]);
    this._addCss("GridControl>#summ", "cellbackground", obj, ["normal"]);

    obj = new nexacro.Style_background("","","","0","0","0","0","true");
    this._addCss("Grid>#head", "cellbackground2", obj, ["normal"]);
    this._addCss("GridControl>#head", "cellbackground2", obj, ["normal"]);

    obj = new nexacro.Style_color("#ffffff");
    this._addCss("Grid>#head", "cellcolor", obj, ["normal"]);
    this._addCss("GridControl>#head", "cellcolor", obj, ["normal"]);

    obj = new nexacro.Style_color("");
    this._addCss("Grid>#head", "cellcolor2", obj, ["normal"]);
    this._addCss("Grid>#summ", "cellcolor2", obj, ["normal"]);
    this._addCss("GridControl>#head", "cellcolor2", obj, ["normal"]);
    this._addCss("GridControl>#summ", "cellcolor2", obj, ["normal"]);

    obj = new nexacro.Style_font("15 bold Dotum,Dotum");
    this._addCss("Grid>#head", "cellfont", obj, ["normal"]);
    this._addCss("GridControl>#head", "cellfont", obj, ["normal"]);

    obj = new nexacro.Style_gradation("");
    this._addCss("Grid>#head", "cellgradation", obj, ["normal"]);
    this._addCss("Grid>#body", "cellgradation", obj, ["normal"]);
    this._addCss("Grid>#summ", "cellgradation", obj, ["normal"]);
    this._addCss("GridControl>#head", "cellgradation", obj, ["normal"]);
    this._addCss("GridControl>#body", "cellgradation", obj, ["normal"]);
    this._addCss("GridControl>#summ", "cellgradation", obj, ["normal"]);

    obj = new nexacro.Style_gradation("");
    this._addCss("Grid>#head", "cellgradation2", obj, ["normal"]);
    this._addCss("Grid>#body", "cellgradation2", obj, ["normal"]);
    this._addCss("Grid>#summ", "cellgradation2", obj, ["normal"]);
    this._addCss("GridControl>#head", "cellgradation2", obj, ["normal"]);
    this._addCss("GridControl>#body", "cellgradation2", obj, ["normal"]);
    this._addCss("GridControl>#summ", "cellgradation2", obj, ["normal"]);

    obj = new nexacro.Style_border("1","solid","#212225ff","");
    this._addCss("Grid>#head", "cellline", obj, ["normal"]);
    this._addCss("Grid>#body", "cellline", obj, ["normal"]);
    this._addCss("GridControl>#head", "cellline", obj, ["normal"]);
    this._addCss("GridControl>#body", "cellline", obj, ["normal"]);

    obj = new nexacro.Style_value("");
    this._addCss("Grid>#head", "celllinespace", obj, ["normal"]);
    this._addCss("Grid>#body", "celllinespace", obj, ["normal"]);
    this._addCss("Grid>#summ", "celllinespace", obj, ["normal"]);
    this._addCss("GridControl>#head", "celllinespace", obj, ["normal"]);
    this._addCss("GridControl>#body", "celllinespace", obj, ["normal"]);
    this._addCss("GridControl>#summ", "celllinespace", obj, ["normal"]);

    obj = new nexacro.Style_value("");
    this._addCss("Grid>#head", "celllinetype", obj, ["normal"]);
    this._addCss("Grid>#body", "celllinetype", obj, ["normal"]);
    this._addCss("Grid>#summ", "celllinetype", obj, ["normal"]);
    this._addCss("GridControl>#head", "celllinetype", obj, ["normal"]);
    this._addCss("GridControl>#body", "celllinetype", obj, ["normal"]);
    this._addCss("GridControl>#summ", "celllinetype", obj, ["normal"]);

    obj = new nexacro.Style_padding("");
    this._addCss("Grid>#head", "cellpadding", obj, ["normal"]);
    this._addCss("Grid>#summ", "cellpadding", obj, ["normal"]);
    this._addCss("GridControl>#head", "cellpadding", obj, ["normal"]);
    this._addCss("GridControl>#summ", "cellpadding", obj, ["normal"]);

    obj = new nexacro.Style_gradation("linear 0,0 #58595d 0,100 #27282bff [2% #45464a]");
    this._addCss("Grid>#head", "gradation", obj, ["normal"]);
    this._addCss("GridControl>#head", "gradation", obj, ["normal"]);

    obj = new nexacro.Style_value("");
    this._addCss("Grid>#head", "selectbackground", obj, ["normal"]);
    this._addCss("Grid>#summ", "selectbackground", obj, ["normal"]);
    this._addCss("GridControl>#head", "selectbackground", obj, ["normal"]);
    this._addCss("GridControl>#summ", "selectbackground", obj, ["normal"]);

    obj = new nexacro.Style_border("");
    this._addCss("Grid>#head", "selectborder", obj, ["normal"]);
    this._addCss("Grid>#summ", "selectborder", obj, ["normal"]);
    this._addCss("GridControl>#head", "selectborder", obj, ["normal"]);
    this._addCss("GridControl>#summ", "selectborder", obj, ["normal"]);

    obj = new nexacro.Style_color("");
    this._addCss("Grid>#head", "selectcolor", obj, ["normal"]);
    this._addCss("Grid>#summ", "selectcolor", obj, ["normal"]);
    this._addCss("GridControl>#head", "selectcolor", obj, ["normal"]);
    this._addCss("GridControl>#summ", "selectcolor", obj, ["normal"]);

    obj = new nexacro.Style_font("");
    this._addCss("Grid>#head", "selectfont", obj, ["normal"]);
    this._addCss("Grid>#summ", "selectfont", obj, ["normal"]);
    this._addCss("GridControl>#head", "selectfont", obj, ["normal"]);
    this._addCss("GridControl>#summ", "selectfont", obj, ["normal"]);

    obj = new nexacro.Style_gradation("");
    this._addCss("Grid>#head", "selectgradation", obj, ["normal"]);
    this._addCss("Grid>#body", "selectgradation", obj, ["normal"]);
    this._addCss("Grid>#summ", "selectgradation", obj, ["normal"]);
    this._addCss("GridControl>#head", "selectgradation", obj, ["normal"]);
    this._addCss("GridControl>#body", "selectgradation", obj, ["normal"]);
    this._addCss("GridControl>#summ", "selectgradation", obj, ["normal"]);

    obj = new nexacro.Style_background("","theme://images/grid_bg.png","stretch","0","0","0","0","true");
    this._addCss("Grid>#body", "background", obj, ["normal"]);
    this._addCss("GridControl>#body", "background", obj, ["normal"]);

    obj = new nexacro.Style_border("0","none","#222222ff","");
    this._addCss("Grid>#body", "border", obj, ["normal"]);
    this._addCss("GridControl>#body", "border", obj, ["normal"]);

    obj = new nexacro.Style_background("","","","0","0","50","50","true");
    this._addCss("Grid>#body", "cellbackground", obj, ["normal"]);
    this._addCss("GridControl>#body", "cellbackground", obj, ["normal"]);

    obj = new nexacro.Style_background("#00000033","","","0","0","50","50","true");
    this._addCss("Grid>#body", "cellbackground2", obj, ["normal"]);
    this._addCss("GridControl>#body", "cellbackground2", obj, ["normal"]);

    obj = new nexacro.Style_color("#c4c9d2");
    this._addCss("Grid>#body", "cellcolor", obj, ["normal", "mouseover"]);
    this._addCss("GridControl>#body", "cellcolor", obj, ["normal", "mouseover"]);

    obj = new nexacro.Style_color("#c4c9d2");
    this._addCss("Grid>#body", "cellcolor2", obj, ["normal", "mouseover"]);
    this._addCss("GridControl>#body", "cellcolor2", obj, ["normal", "mouseover"]);

    obj = new nexacro.Style_font("15 Dotum,Dotum");
    this._addCss("Grid>#body", "cellfont", obj, ["normal", "mouseover"]);
    this._addCss("GridControl>#body", "cellfont", obj, ["normal", "mouseover"]);

    obj = new nexacro.Style_padding("4 4 4 4");
    this._addCss("Grid>#body", "cellpadding", obj, ["normal"]);
    this._addCss("GridControl>#body", "cellpadding", obj, ["normal"]);

    obj = new nexacro.Style_value("#ffffff66 center middle");
    this._addCss("Grid>#body", "selectbackground", obj, ["normal"]);
    this._addCss("GridControl>#body", "selectbackground", obj, ["normal"]);

    obj = new nexacro.Style_border("0","none","#222222ff","");
    this._addCss("Grid>#body", "selectborder", obj, ["normal"]);
    this._addCss("GridControl>#body", "selectborder", obj, ["normal"]);

    obj = new nexacro.Style_font("15 Dotum,Dotum");
    this._addCss("Grid>#body", "selectfont", obj, ["normal"]);
    this._addCss("GridControl>#body", "selectfont", obj, ["normal"]);

    obj = new nexacro.Style_border("0","none","#ffffff00","","1","solid","#212225ff","","0","none","#ffffff00","","0","none","#ffffff00","");
    this._addCss("Grid>#body", "selectline", obj, ["normal"]);
    this._addCss("GridControl>#body", "selectline", obj, ["normal"]);

    obj = new nexacro.Style_background("#ffffff3c","","","0","0","0","0","true");
    this._addCss("Grid>#body", "cellbackground", obj, ["mouseover"]);
    this._addCss("GridControl>#body", "cellbackground", obj, ["mouseover"]);

    obj = new nexacro.Style_background("#ffffff3c","","","0","0","0","0","true");
    this._addCss("Grid>#body", "cellbackground2", obj, ["mouseover"]);
    this._addCss("GridControl>#body", "cellbackground2", obj, ["mouseover"]);

    obj = new nexacro.Style_value("15 Dotum,Dotum");
    this._addCss("Grid>#body", "cellfont2", obj, ["mouseover"]);
    this._addCss("GridControl>#body", "cellfont2", obj, ["mouseover"]);

    obj = new nexacro.Style_align("right middle");
    this._addCss("Grid>#summ", "cellalign", obj, ["normal"]);
    this._addCss("GridControl>#summ", "cellalign", obj, ["normal"]);

    obj = new nexacro.Style_background("","","","0","0","0","0","true");
    this._addCss("Grid>#summ", "cellbackground2", obj, ["normal"]);
    this._addCss("GridControl>#summ", "cellbackground2", obj, ["normal"]);

    obj = new nexacro.Style_color("#333333");
    this._addCss("Grid>#summ", "cellcolor", obj, ["normal"]);
    this._addCss("GridControl>#summ", "cellcolor", obj, ["normal"]);

    obj = new nexacro.Style_font("9 Dotum");
    this._addCss("Grid>#summ", "cellfont", obj, ["normal"]);
    this._addCss("GridControl>#summ", "cellfont", obj, ["normal"]);

    obj = new nexacro.Style_border("1","solid","#b1b1b1ff","");
    this._addCss("Grid>#summ", "cellline", obj, ["normal"]);
    this._addCss("GridControl>#summ", "cellline", obj, ["normal"]);

    obj = new nexacro.Style_gradation("linear 0,0 #fefefe 0,100 #e7e7e7");
    this._addCss("Grid>#summ", "gradation", obj, ["normal"]);
    this._addCss("GridControl>#summ", "gradation", obj, ["normal"]);

    obj = new nexacro.Style_bordertype("round","4","4","false","true","true","true");
    this._addCss("Tab", "bordertype", obj, ["normal"]);
    this._addCss("TabControl", "bordertype", obj, ["normal"]);

    obj = new nexacro.Style_background("@gradation","","","0","0","0","0","true");
    this._addCss("Tab", "buttonbackground", obj, ["normal"]);
    this._addCss("TabControl", "buttonbackground", obj, ["normal"]);
    this._addCss("FileUpload", "buttonbackground", obj, ["normal"]);
    this._addCss("FileUploadControl", "buttonbackground", obj, ["normal"]);

    obj = new nexacro.Style_value("1 solid #202020");
    this._addCss("Tab", "buttonborder", obj, ["normal"]);

    obj = new nexacro.Style_bordertype("round","4","4","true","true","false","false");
    this._addCss("Tab", "buttonbordertype", obj, ["normal"]);
    this._addCss("TabControl", "buttonbordertype", obj, ["normal"]);

    obj = new nexacro.Style_gradation("linear 0,0 #58595d 0,100 #46474b [3% #45464a]");
    this._addCss("Tab", "buttongradation", obj, ["normal"]);
    this._addCss("TabControl", "buttongradation", obj, ["normal"]);

    obj = new nexacro.Style_padding("4 14 5 14");
    this._addCss("Tab", "buttonpadding", obj, ["normal"]);
    this._addCss("TabControl", "buttonpadding", obj, ["normal"]);

    obj = new nexacro.Style_line("0","","","");
    this._addCss("Tab", "focusborder", obj, ["normal"]);
    this._addCss("TabControl", "focusborder", obj, ["normal"]);

    obj = new nexacro.Style_value("");
    this._addCss("Tab", "showextrabutton", obj, ["normal"]);
    this._addCss("TabControl", "showextrabutton", obj, ["normal"]);

    obj = new nexacro.Style_gradation("linear 0,0 #58595d 0,100 #35363b [3% #58595d][35% #45464a]");
    this._addCss("Tab", "buttongradation", obj, ["mouseover"]);
    this._addCss("TabControl", "buttongradation", obj, ["mouseover"]);

    obj = new nexacro.Style_color("#ff6600");
    this._addCss("Tab", "color", obj, ["mouseover", "selected", "focused"]);
    this._addCss("TabControl", "color", obj, ["mouseover", "selected", "focused"]);

    obj = new nexacro.Style_gradation("linear 0,0 #58595d 0,100 #35363b [3% #45464a]");
    this._addCss("Tab", "buttongradation", obj, ["selected", "focused"]);
    this._addCss("TabControl", "buttongradation", obj, ["selected", "focused"]);

    obj = new nexacro.Style_value("theme://images/tab_spinup.png");
    this._addCss("Tab>#spinupbutton", "image", obj, ["normal"]);
    this._addCss("TabControl>#spinupbutton", "image", obj, ["normal"]);

    obj = new nexacro.Style_value("theme://images/tab_spinup_D.png");
    this._addCss("Tab>#spinupbutton", "image", obj, ["disabled"]);
    this._addCss("TabControl>#spinupbutton", "image", obj, ["disabled"]);

    obj = new nexacro.Style_value("theme://images/tab_spindn.png");
    this._addCss("Tab>#spindownbutton", "image", obj, ["normal"]);
    this._addCss("TabControl>#spindownbutton", "image", obj, ["normal"]);

    obj = new nexacro.Style_value("theme://images/tab_spindn_D.png");
    this._addCss("Tab>#spindownbutton", "image", obj, ["disabled"]);
    this._addCss("TabControl>#spindownbutton", "image", obj, ["disabled"]);

    obj = new nexacro.Style_value("theme://images/tab_extra.png");
    this._addCss("Tab>#extrabutton", "image", obj, ["normal", "mouseover", "focused", "selected", "pushed", "disabled"]);
    this._addCss("TabControl>#extrabutton", "image", obj, ["normal", "mouseover", "focused", "selected", "pushed", "disabled"]);

    obj = new nexacro.Style_value("1 solid #202020");
    this._addCss("TabControl", "buttonborder", obj, ["normal"]);

    obj = new nexacro.Style_border("1","solid","#101113ff","");
    this._addCss("PopupMenu", "border", obj, ["normal"]);
    this._addCss("PopupMenuControl", "border", obj, ["normal"]);

    obj = new nexacro.Style_value("theme://images/img_check.png");
    this._addCss("PopupMenu", "checkboximage", obj, ["normal"]);
    this._addCss("PopupMenuControl", "checkboximage", obj, ["normal"]);

    obj = new nexacro.Style_value("theme://images/ico_PopupMenu.png");
    this._addCss("PopupMenu", "expandimage", obj, ["normal"]);
    this._addCss("PopupMenuControl", "expandimage", obj, ["normal"]);

    obj = new nexacro.Style_background("#35363bff","","","0","0","0","0","true");
    this._addCss("PopupMenu", "itembackground", obj, ["normal"]);
    this._addCss("PopupMenuControl", "itembackground", obj, ["normal"]);

    obj = new nexacro.Style_border("0","none","#808080ff","","1","solid","#101113ff","","1","solid","#101113ff","","0","none","#808080ff","");
    this._addCss("PopupMenu", "itemborder", obj, ["normal"]);
    this._addCss("PopupMenuControl", "itemborder", obj, ["normal"]);

    obj = new nexacro.Style_value("24");
    this._addCss("PopupMenu", "itemheight", obj, ["normal"]);
    this._addCss("PopupMenuControl", "itemheight", obj, ["normal"]);

    obj = new nexacro.Style_value("");
    this._addCss("PopupMenu", "popuptype", obj, ["normal"]);
    this._addCss("PopupMenuControl", "popuptype", obj, ["normal"]);
    this._addCss("Menu", "popuptype", obj, ["normal"]);
    this._addCss("MenuControl", "popuptype", obj, ["normal"]);

    obj = new nexacro.Style_background("","theme://images/cmb_list_BGS.png","repeat-x","0","0","0","0","true");
    this._addCss("PopupMenu", "itembackground", obj, ["selected", "focused", "pushed"]);
    this._addCss("PopupMenuControl", "itembackground", obj, ["selected", "focused", "pushed"]);

    obj = new nexacro.Style_gradation("linear 0,0 #fbfbfa 100,0 #deded7");
    this._addCss("PopupMenu>#spindownbutton", "gradation", obj, ["normal"]);
    this._addCss("PopupMenu>#spinupbutton", "gradation", obj, ["normal"]);
    this._addCss("PopupMenuControl>#spindownbutton", "gradation", obj, ["normal"]);
    this._addCss("PopupMenuControl>#spinupbutton", "gradation", obj, ["normal"]);
    this._addCss("Menu>#spindownbutton", "gradation", obj, ["normal"]);
    this._addCss("Menu>#spinupbutton", "gradation", obj, ["normal"]);
    this._addCss("MenuControl>#spindownbutton", "gradation", obj, ["normal"]);
    this._addCss("MenuControl>#spinupbutton", "gradation", obj, ["normal"]);

    obj = new nexacro.Style_value("theme://images/btn_vinc_N.png");
    this._addCss("PopupMenu>#spindownbutton", "image", obj, ["normal"]);
    this._addCss("PopupMenuControl>#spindownbutton", "image", obj, ["normal"]);

    obj = new nexacro.Style_align("cetner middle");
    this._addCss("PopupMenu>#spindownbutton", "imagealign", obj, ["normal"]);
    this._addCss("PopupMenu>#spinupbutton", "imagealign", obj, ["normal"]);
    this._addCss("PopupMenuControl>#spindownbutton", "imagealign", obj, ["normal"]);
    this._addCss("PopupMenuControl>#spinupbutton", "imagealign", obj, ["normal"]);
    this._addCss("Menu>#spindownbutton", "imagealign", obj, ["normal"]);
    this._addCss("Menu>#spinupbutton", "imagealign", obj, ["normal"]);
    this._addCss("MenuControl>#spindownbutton", "imagealign", obj, ["normal"]);
    this._addCss("MenuControl>#spinupbutton", "imagealign", obj, ["normal"]);

    obj = new nexacro.Style_value("theme://images/btn_vdec_N.png");
    this._addCss("PopupMenu>#spinupbutton", "image", obj, ["normal"]);
    this._addCss("PopupMenuControl>#spinupbutton", "image", obj, ["normal"]);

    obj = new nexacro.Style_value("");
    this._addCss("Menu", "autohotkey", obj, ["normal"]);
    this._addCss("MenuControl", "autohotkey", obj, ["normal"]);

    obj = new nexacro.Style_value("");
    this._addCss("Menu", "checkboximage", obj, ["normal"]);
    this._addCss("MenuControl", "checkboximage", obj, ["normal"]);

    obj = new nexacro.Style_value("theme://images/ico_menu_expand.png");
    this._addCss("Menu", "expandimage", obj, ["normal"]);
    this._addCss("MenuControl", "expandimage", obj, ["normal"]);

    obj = new nexacro.Style_gradation("linear 0,0 #56575bff 0,100 #212326ff [3% #45464aff]");
    this._addCss("Menu", "gradation", obj, ["normal"]);
    this._addCss("MenuControl", "gradation", obj, ["normal"]);

    obj = new nexacro.Style_align("");
    this._addCss("Menu", "itemalign", obj, ["normal"]);
    this._addCss("MenuControl", "itemalign", obj, ["normal"]);

    obj = new nexacro.Style_background("","theme://images/menu_bg.png","","0","0","100","50","true");
    this._addCss("Menu", "itembackground", obj, ["normal"]);
    this._addCss("MenuControl", "itembackground", obj, ["normal"]);

    obj = new nexacro.Style_padding("0 28 0 28");
    this._addCss("Menu", "itempadding", obj, ["normal"]);
    this._addCss("MenuControl", "itempadding", obj, ["normal"]);

    obj = new nexacro.Style_border("2","solid","#3c3c44ff","");
    this._addCss("Menu", "popupborder", obj, ["normal"]);
    this._addCss("MenuControl", "popupborder", obj, ["normal"]);

    obj = new nexacro.Style_color("#ffffff");
    this._addCss("Menu", "popupcolor", obj, ["normal", "mouseover", "selected", "focused", "pushed"]);
    this._addCss("MenuControl", "popupcolor", obj, ["normal", "mouseover", "selected", "focused", "pushed"]);

    obj = new nexacro.Style_font("9 Dotum");
    this._addCss("Menu", "popupfont", obj, ["normal", "mouseover", "selected", "focused", "pushed"]);
    this._addCss("MenuControl", "popupfont", obj, ["normal", "mouseover", "selected", "focused", "pushed"]);

    obj = new nexacro.Style_background("","","","0","0","0","0","true");
    this._addCss("Menu", "popupitembackground", obj, ["normal"]);
    this._addCss("MenuControl", "popupitembackground", obj, ["normal"]);

    obj = new nexacro.Style_border("0","none","","");
    this._addCss("Menu", "popupitemborder", obj, ["normal"]);
    this._addCss("MenuControl", "popupitemborder", obj, ["normal"]);

    obj = new nexacro.Style_bordertype("","0","0","true","true","true","true");
    this._addCss("Menu", "popupitembordertype", obj, ["normal"]);
    this._addCss("MenuControl", "popupitembordertype", obj, ["normal"]);

    obj = new nexacro.Style_gradation("");
    this._addCss("Menu", "popupitemgradation", obj, ["normal"]);
    this._addCss("MenuControl", "popupitemgradation", obj, ["normal"]);

    obj = new nexacro.Style_value("21");
    this._addCss("Menu", "popupitemheight", obj, ["normal"]);
    this._addCss("MenuControl", "popupitemheight", obj, ["normal"]);

    obj = new nexacro.Style_padding("0 7 0 10");
    this._addCss("Menu", "popupitempadding", obj, ["normal"]);
    this._addCss("MenuControl", "popupitempadding", obj, ["normal"]);

    obj = new nexacro.Style_padding("0 0 0 0");
    this._addCss("Menu", "popuppadding", obj, ["normal"]);
    this._addCss("MenuControl", "popuppadding", obj, ["normal"]);

    obj = new nexacro.Style_background("#7e8089ff","","","0","0","0","0","true");
    this._addCss("Menu", "popupitembackground", obj, ["mouseover", "selected", "focused", "pushed"]);
    this._addCss("MenuControl", "popupitembackground", obj, ["mouseover", "selected", "focused", "pushed"]);

    obj = new nexacro.Style_background("@gradation","","","0","0","0","0","true");
    this._addCss("Menu", "itembackground", obj, ["mouseover", "selected", "focused", "pushed"]);
    this._addCss("MenuControl", "itembackground", obj, ["mouseover", "selected", "focused", "pushed"]);

    obj = new nexacro.Style_gradation("linear 0,0 #36373c 0,100 #4a4b50 [45% #19191b] [50% #101011] [55% #19191b] [95% #34353a]");
    this._addCss("Menu", "itemgradation", obj, ["mouseover", "selected", "focused", "pushed"]);
    this._addCss("MenuControl", "itemgradation", obj, ["mouseover", "selected", "focused", "pushed"]);

    obj = new nexacro.Style_value("theme://images/ico_menu_expand_O.png");
    this._addCss("Menu", "expandimage", obj, ["mouseover", "selected", "focused", "pushed"]);
    this._addCss("MenuControl", "expandimage", obj, ["mouseover", "selected", "focused", "pushed"]);

    obj = new nexacro.Style_value("theme://images/btn_hinc_N.png");
    this._addCss("Menu>#spindownbutton", "image", obj, ["normal"]);
    this._addCss("MenuControl>#spindownbutton", "image", obj, ["normal"]);

    obj = new nexacro.Style_value("theme://images/btn_hdec_N.png");
    this._addCss("Menu>#spinupbutton", "image", obj, ["normal"]);
    this._addCss("MenuControl>#spinupbutton", "image", obj, ["normal"]);

    obj = new nexacro.Style_color("@gradation");
    this._addCss("ProgressBar", "barcolor", obj, ["normal"]);
    this._addCss("ProgressBarControl", "barcolor", obj, ["normal"]);

    obj = new nexacro.Style_border("1","solid","#8c98a0ff","");
    this._addCss("ProgressBar", "border", obj, ["normal"]);
    this._addCss("ProgressBarControl", "border", obj, ["normal"]);

    obj = new nexacro.Style_color("transparent");
    this._addCss("ProgressBar", "color", obj, ["normal"]);
    this._addCss("ProgressBarControl", "color", obj, ["normal"]);

    obj = new nexacro.Style_gradation("linear 0,0 #ffffff 0,100 #ffffff [3% #f2f2f2][25% #ffffff]");
    this._addCss("ProgressBar", "gradation", obj, ["normal"]);
    this._addCss("ProgressBarControl", "gradation", obj, ["normal"]);

    obj = new nexacro.Style_padding("2 3 3 3");
    this._addCss("ProgressBar", "padding", obj, ["normal"]);
    this._addCss("ProgressBarControl", "padding", obj, ["normal"]);

    obj = new nexacro.Style_background("#eaeaeaff","","","0","0","0","0","true");
    this._addCss("ProgressBar", "background", obj, ["disabled"]);
    this._addCss("ProgressBarControl", "background", obj, ["disabled"]);

    obj = new nexacro.Style_gradation("linear 0,0 #c7c7c7 0,100 #c0c0c0");
    this._addCss("ProgressBar", "bargradation", obj, ["disabled"]);
    this._addCss("ProgressBarControl", "bargradation", obj, ["disabled"]);

    obj = new nexacro.Style_value("1 solid #101113");
    this._addCss("FileUpload", "buttonborder", obj, ["normal", "pushed"]);

    obj = new nexacro.Style_bordertype("round","3","3","true","true","true","true");
    this._addCss("FileUpload", "buttonbordertype", obj, ["normal"]);
    this._addCss("FileUploadControl", "buttonbordertype", obj, ["normal"]);

    obj = new nexacro.Style_color("#ffffff");
    this._addCss("FileUpload", "buttoncolor", obj, ["normal"]);
    this._addCss("FileUploadControl", "buttoncolor", obj, ["normal"]);

    obj = new nexacro.Style_font("9 Dotum");
    this._addCss("FileUpload", "buttonfont", obj, ["normal"]);
    this._addCss("FileUploadControl", "buttonfont", obj, ["normal"]);

    obj = new nexacro.Style_gradation("linear 0,0 #56575b 0,100 #212326 [3% #45464a]");
    this._addCss("FileUpload", "buttongradation", obj, ["normal"]);
    this._addCss("FileUploadControl", "buttongradation", obj, ["normal"]);

    obj = new nexacro.Style_padding("0 0 0 0");
    this._addCss("FileUpload", "buttonpadding", obj, ["normal"]);
    this._addCss("FileUploadControl", "buttonpadding", obj, ["normal"]);

    obj = new nexacro.Style_value("100");
    this._addCss("FileUpload", "buttonsize", obj, ["normal"]);
    this._addCss("FileUploadControl", "buttonsize", obj, ["normal"]);

    obj = new nexacro.Style_value("File Add");
    this._addCss("FileUpload", "buttontext", obj, ["normal"]);
    this._addCss("FileUploadControl", "buttontext", obj, ["normal"]);

    obj = new nexacro.Style_color("#8a8c9dff");
    this._addCss("FileUpload", "color", obj, ["normal"]);
    this._addCss("FileUploadControl", "color", obj, ["normal"]);

    obj = new nexacro.Style_background("@gradation","","","0","0","0","0","true");
    this._addCss("FileUpload", "editbackground", obj, ["normal"]);
    this._addCss("FileUploadControl", "editbackground", obj, ["normal"]);

    obj = new nexacro.Style_border("1","solid","#ffffffff","");
    this._addCss("FileUpload", "editborder", obj, ["normal"]);
    this._addCss("FileUploadControl", "editborder", obj, ["normal"]);

    obj = new nexacro.Style_bordertype("round","4","4","true","true","true","true");
    this._addCss("FileUpload", "editbordertype", obj, ["normal"]);
    this._addCss("FileUploadControl", "editbordertype", obj, ["normal"]);

    obj = new nexacro.Style_color("#333333");
    this._addCss("FileUpload", "editcolor", obj, ["normal"]);
    this._addCss("FileUploadControl", "editcolor", obj, ["normal"]);

    obj = new nexacro.Style_font("");
    this._addCss("FileUpload", "editfont", obj, ["normal"]);
    this._addCss("FileUploadControl", "editfont", obj, ["normal"]);

    obj = new nexacro.Style_gradation("linear 0,0 #c2c7d0 0,100 #ffffff");
    this._addCss("FileUpload", "editgradation", obj, ["normal"]);
    this._addCss("FileUploadControl", "editgradation", obj, ["normal"]);

    obj = new nexacro.Style_padding("0 7 0 7");
    this._addCss("FileUpload", "editpadding", obj, ["normal"]);
    this._addCss("FileUploadControl", "editpadding", obj, ["normal"]);

    obj = new nexacro.Style_color("");
    this._addCss("FileUpload", "editselectbackground", obj, ["normal"]);
    this._addCss("FileUploadControl", "editselectbackground", obj, ["normal"]);

    obj = new nexacro.Style_color("");
    this._addCss("FileUpload", "editselectcolor", obj, ["normal"]);
    this._addCss("FileUploadControl", "editselectcolor", obj, ["normal"]);

    obj = new nexacro.Style_value("32");
    this._addCss("FileUpload", "itemheight", obj, ["normal"]);
    this._addCss("FileUploadControl", "itemheight", obj, ["normal"]);

    obj = new nexacro.Style_gradation("linear 0,0 #212326 0,100 #212326");
    this._addCss("FileUpload", "buttongradation", obj, ["pushed"]);

    obj = new nexacro.Style_value("1 solid #101113");
    this._addCss("FileUploadControl", "buttonborder", obj, ["normal"]);

    obj = new nexacro.Style_gradation("linear 0,0 #ecece5 0,100 #fefefe");
    this._addCss("FileUploadControl", "buttongradation", obj, ["pushed"]);

    obj = new nexacro.Style_border("1","solid","#999999ff","");
    this._addCss("FileDownload", "border", obj, ["normal"]);
    this._addCss("FileDownloadControl", "border", obj, ["normal"]);

    obj = new nexacro.Style_gradation("linear 0,0 #fefefe 0,100 #ecece5");
    this._addCss("FileDownload", "gradation", obj, ["normal"]);
    this._addCss("FileDownloadControl", "gradation", obj, ["normal"]);

    obj = new nexacro.Style_gradation("linear 0,0 #ecece5 0,100 #fefefe");
    this._addCss("FileDownload", "gradation", obj, ["pushed"]);
    this._addCss("FileDownloadControl", "gradation", obj, ["pushed"]);

    obj = new nexacro.Style_background("#ffffe1ff","","","0","0","0","0","true");
    this._addCss("Tooltip", "background", obj, ["normal"]);

    obj = new nexacro.Style_padding("3 7 3 7");
    this._addCss("Tooltip", "padding", obj, ["normal"]);

    obj = null;
    
//[add theme images]
  };
})();
