...
if (!nexacro.TextArea) {
		...
		if (api && elem) {
			...
			if (nexacro.OS == "iOS") {
				...
				setTimeout(function () {
					...
					pThis._on_input_blur(elem);
					api._is_on_killfocus = false;

					if (focus_text != cur_text || (application._is_input_paste && pThis == application._input_paste_comp)) {
						application._is_input_paste = false;
						application._input_paste_comp = null;

						var win = pThis._getWindow();
						if (win && win._is_active_window != true && !obj) {
							elem.setElementValue(api._value, false, true);
							return;
						}

						var ret = pThis._on_fire_changeEventSet(focus_text, focus_val, cur_text, cur_val);
						if (!ret) {
							return;
						}
					}
	...
	_pTextArea._cancelSelect = function () {
		...
		if (nexacro.Browser == "Edge" || nexacro.Browser == "IE") {
			...
			if (api._is_selected() && this.autoselect == true) {
				this.setCaretPos(api._begin_pos);
			}
		}
	};
