...
if (nexacro.Browser == "Runtime")
{
	...
	_pFileTransaction.getResponseDataset = function (scope) {
		...
		if(!ds)
		{
			var uid = Base.getSequenceId(scope, uidPrefix);
			ds = new Dataset(uid);
	...
	if (!nexacro.ExtFileUpload)
	{
		...
		nexacro.ExtFileUpload = function (id, parent)
		{
			...
			this.support = {};
			Base.object.copyProperties(this.support, nexacro._ExtFileUpDownloadSupport);
		...
		_pFileUpload.setFileInfo = function(index, fileList) {
			...
			for(var i=0; i<count; i++)
			{
				...
				//var fileId = Base.getUniqueId("uuid_");
		...
		_pFileUpload.upload = function (v, inDatasetsParam, outDatasetsParam, datatype)
		{
			...
			trace("_pFileUpload.upload======================");
			trace("v : " + v);
			trace("inDatasetsParam : " + inDatasetsParam);
			trace("outDatasetsParam : " + outDatasetsParam);
		...
		{
			//변경되지 않으면 stop
			if(Base.isEmpty(newvalue)) return;
		    ...
		    { 
				...
				try
				{
					...
					{   
						...
						//trace("step 3 result[1]=\n" + result[1] + ", isArray=" +(Base.isArray(result[1])));
		...
		_pSSV._getColLinesFromSSVLines = function (ssvLines, curIdx)
		{
			...
			trace("typeCol > " + typeCol + ", isArray= " + Base.isArray(typeCol));	
			return typeCol;
		};		
    ...
    */
	...
	//                }
		...
		_pExtFileUpload.addResponseZone = function ()
		{
			...
			if(Base.isEmpty(guideComp) || Base.isEmpty(helpMessageComp)) {
		...
		{
			var index = Base.array.lastIndexOfProp(this._addedfiles, "id", fileid);
			if (index > -1)
			{
				this._addedfiles[index] = null;
				Base.array.removeAt(this._addedfiles, index);
			}
			
			if (!this.support.XHR2 || !this.support.FileAPI)
			{
				//index = Base.array.lastIndexOfProp(this._inputfiles, "name", fileid);
				if (index > -1)
				{
					...
					Base.array.removeAt(this._inputfiles, index);
				}
			} else {
		...
		_pExtFileUpload.removeAll = function ()
		{
			...
			for(var i=0; i<count; i++)
			{
				...
				index = Base.array.lastIndexOfProp(this._addedfiles, "id", fileId);
				
				//trace("addFiles fileId="+fileId +", index="+index);
				if (index > -1)
				{
					addFiles[index] = null;
					Base.array.removeAt(addFiles, index);
	  ...
	  _pExtFileUpload.on_fire_onloadstart = function (obj, type, evt, fileId)
	  {
		...
		_pExtFileUpload._dragLeaveGuide = function (evt) 
		{
			...
			var convertXY = Base.XComp.PositionSize.convertXY(application.mainframe,[0,0], guideComp);
			...
			var size = Base.XComp.PositionSize.getContentSize(guideComp);
