...
if (!nexacro.Grid) {
	...
	_pGridEdit.on_fire_onchar = function (event_info, textEvt) {
		...
		return this._grid.on_fire_onchar(this, event_info.chartext, event_info.pretext, event_info.posttext, textEvt);
	};
	...
	_pGridMaskEdit.on_fire_onchar = function (event_info) {
		...
		return this._grid.on_fire_onchar(this, event_info.chartext, event_info.pretext, event_info.posttext, obj.evt);
	};
	...
	_pGridBand.on_update_style_selectborder = function () {
		...
		this._grid._setSelectBorder(this.on_find_CurrentStyle_selectborder(this._pseudo));
	};
	...
	_pGridBand.on_change_containerRect = function (width, height) {
		...
		this._on_refresh_rows();
		grid._setSelectBorder(this.on_find_CurrentStyle_selectborder(this._pseudo));
	};
	...
	_pGridBand._recreate_contents = function (recreate_colarea, init_scroll, scrolling, no_hide_edit, no_update_supp) {
		...
		if (this._isBody) {
			...
			var rowSizes = grid._rowSizeList, datarow, band_scroll_sizes = [], band_sizes_cnt = 1, band_scroll_max = grid._div_max_height;
	...
	_pGridScrollBarCtrl._set_pixelpos = function (v, evt_kind) {
		...
		if (evt_kind == "mousewheel" || evt_kind == "slide" || evt_kind == "fling") {
			str = this._getScrollInfo(v, evt_kind);
		}
	...
	_pGrid._adjustGridScrollRows_callback = function (no_ani) {
		...
		this._setSelectBorder(body.on_find_CurrentStyle_selectborder(body._pseudo));
	};
	...
	_pGrid._adjustGridScrollCols_callback = function (no_ani) {
		...
		this._adjustOverlayElements(false, this._is_use_fakemerge);
		this._setSelectBorder(this._bodyBand.on_find_CurrentStyle_selectborder(this._bodyBand._pseudo));
	};
	...
	_pGrid._moveCellAfterFocus = function () {
		...
		if (this.autoenter == "select") {
			...
			if (!this._showEditing && !this._showEditor(false, true)) {
				this._hideEditor();
				retn = false;
			}
			this._onceTime_focus = false;
	...
	_pGrid._setSelectBorder = function (selectborder, is_recreate) {
		if (!this._is_created || !selectborder) {
			return;
		}
		...
		if (elements && elements.length > 0) {
			...
			for (var i = 0, len = elements.length; i < len; i++) {
				...
				if (sub_elements && sub_elements.length > 0) {
					var start_cell = sub_elements[0].linkedcontrol;
					var end_cell = sub_elements[sub_elements.length - 1].linkedcontrol;
	...
	_pGrid._destroySelectBorderElements = function (start_index) {
		...
		if (selectborder_elements.length) {
			var index = start_index > 0 ? start_index : 0;
			for (index, len = selectborder_elements.length; index < len; index++) {
				selectborder_elements[index].destroy();
				selectborder_elements[index] = null;
			}
		}
		selectborder_elements.splice(start_index);
		this._selectborder_elements = selectborder_elements;
	};
