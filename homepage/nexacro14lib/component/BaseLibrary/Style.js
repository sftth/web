...
if ( !JsNamespace.exist("Base.XComp.Style") )
{
	/**
	 * @namespace
	 * @name Base.XComp.Style
	 * @memberof! <global>
	 */
	JsNamespace.declare("Base.XComp.Style", {
		 /**
		 ...
		 * @example
		 * trace(Base.XComp.Style.getBorderWidth("border style이 none이거나 width가 0일 경우")); //output: [0,0,0,0] 
		 * trace(Base.XComp.Style.getBorderWidth("border가 1인 component")); //output: [1,1,1,1]
		 *
		 * @memberOf Base.XComp.Style
		 */
		getBorderWidth: function(xComp)	
		{
			...
			if (currentBorder)
			{
				...
				leftWidth = Base.isEmpty(leftWidth) ? "0" : leftWidth;
				topWidth = Base.isEmpty(topWidth) ? "0" : topWidth;
				rightWidth = Base.isEmpty(rightWidth) ? "0" : rightWidth;
				bottomWidth = Base.isEmpty(bottomWidth) ? "0" : bottomWidth;
		 ...
		 * @example
		 * trace(Base.XComp.Style.getPadding("전체 padding = 0")); //output: [0,0,0,0] 
		 * trace(Base.XComp.Style.getPadding("left padding = 20")); //output: [20,0,0,0] 
		 *
		 * @memberOf Base.XComp.Style
		 */
		 ...
		 * @return {array.<number>} [ leftSize, topSize, rightSize, bottomSize ]
		 * @example
		 * trace(Base.XComp.Style.getMargin("전체 padding = 0")); //output: [0,0,0,0] 
		 * trace(Base.XComp.Style.getMargin("left padding = 20")); //output: [20,0,0,0] 
		 *
		 * @memberOf Base.XComp.Style
		 */
		 ...
		 * @return {object} 
		 * @example
		 * trace(Base.XComp.Style.getStyle(xComp));
         * // output:
		 ...
		 * @return {object} 
		 * @example
		 * trace(Base.XComp.Style.getCurrentStyle(xComp));
		 * // output:
		 * //  {background: "left middle"
		 * //   ,border: "1 solid #808080ff "
		 * //   ,color: "#333333ff"
		 ...
		 * Base.XComp.Style.setStyle(st_sample10, styleValue, false);
		 *
		 * //현재 설정된 style은 유지하고 지정된 style값만 설정
		 * var styleValue = {border: "2 solid yellow", color: "red"};
         * Base.XComp.Style.setStyle(st_sample9, styleValue);
         *
         * @memberOf Base.XComp.Style
		 */
		setStyle: function(xComp, styleValue, clearFlag)	
		{
			...
			var prevStyle = Base.XComp.Style.getStyle(xComp);
			var newValue = {}, value;
			var isFunction = Base.isFunction;

			if(Base.isEmpty(clearFlag)) clearFlag = true;
		...
		* Base.XComp.Style.setStyleIf(btn_result_01, styleValue, "red", this, false);
        *
        * //btn_result_02의 enable속성이 true일 경우에만 boder,color 적용
        * var styleValue = {border:"3 double red", color: "red"};
        * Base.XComp.Style.setStyleIf(btn_result_02, styleValue, function(xComp, name, value) {
        ...
        * Base.XComp.Style.setStyleIf(btn_result_03, styleValue, "" , this, false);
        *
		* @memberOf Base.XComp.Style
		*/
			...
			if (xComp && styleValue) 
			{
				...
				for (var name in styleValue)
				{
					...
					if (styleValue.hasOwnProperty(name))
					{
						...
						else if (condition == "" && Base.isEmpty(xCompStyle[name]))
						{
							xComp["style"]["set_" + name](value);
						}						
