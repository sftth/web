 ...
 * LG Electronics Util Library
 ...
 ● LGE.Util.addDataset					: Form내에 데이타셋 동적생성 후 추가
...
if (!JsNamespace.exist("LGE.Util")) {
	...
	JsNamespace.declare("LGE.Util", {
		...
		/*--------------------------------------------------------------------------------------------
		 * ⓟ param {object}	대상 form
		 * ⓟ param {string}	생성될 Dataset 명
		 * ⓡ return {object}	Dataset
		 --------------------------------------------------------------------------------------------*/			
		addDataset : function(objForm, sDatasetName)
		{
			var objDataset = objForm.objects[sDatasetName];

			if(Base.isEmpty(objDataset))
			{
				objDataset = new Dataset;
				objDataset.set_name(sDatasetName);
				objForm.addChild(sDatasetName, objDataset);
			}
			return objDataset;
		},
