...
if ((!nexacro.Device || nexacro.OS == "Android") && !nexacro.XPush) {
	if (nexacro.Browser != "Runtime" && !nexacro._isHybrid()) {
		...
		_pSockJSWrapper._nexacro_command = function (strCommand, strMessageType, strMessageKey, strType, userid) {
			var cmd;
		...
		_pWebPush.connect = function (userid, sessionid) {
			...
			var sockjs_url = "http://" + ip + ":" + this.xpush.iplist[this.ip_idx].port + "/XPUSH";
		...
		_pWebPush.command = function (strCommand, strMessageType, strMessageKey, strType, msg_data, userid) {
			...
			if (strCommand == "keepalive") {
				...
				this.sockjs._nexacro_command(strCommand, strMessageType, strMessageKey, strType, userid);
			}
		};
	...
	_pXPush.command = function (strCommand, strMessageType, strMessageKey, objForm, objDataset, strType, strCallBack, nRow, strCheck, bUseActiveFormCallBack, strUserID) {
		...
		if (this.strCommand === undefined || this.strCommand === null || this.strCommand == "") {
			...
			if (strCommand == "ADDF" || strCommand == "DELF") {
				...
				this.commandlist.push(cc);
				if (this._isWebPush()) {
					this.webpush.command(strCommand, strMessageType, strMessageKey, undefined, undefined, strUserID ? strUserID : "");
				}
				else {
					if (this._handle) {
						nexacro.__commandXPushHandle(this._handle, strCommand, strMessageType, strMessageKey, "", strUserID ? strUserID : "");
					}
				}
			}
			else if (strCommand == "MSGC") {
	...
	_pXPush.connect = function (userid, sessionid) {
		...
		if (this._isWebPush()) {
			...
			this.webpush.connect(userid, sessionid);
		}
		else {
			var bSuccessed;
			if (this._handle) {
