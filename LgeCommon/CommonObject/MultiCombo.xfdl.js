    ...
    return function()
    {
        ...
        ***********************************************************************************************/
        ...
        this.fn_initComp = function(oForm,sPopId,oCallback,oParam)
        {
        	...
        	this.fn_addColumn(this.fv_innerdataset, this.fv_checkcolumn, "string");	// add_column
        	...
        	if (this.fv_innerdataset.rowcount < this.fv_displayrowcount) 
        	{
        		this.fv_displayrowcount = this.fv_innerdataset.rowcount;	
        	}
        	...
        	//그리드 생성
        	var aEtcCol = null;
        	if (Base.isEmpty(this.fv_etccolumn)) 
        	{
        		aEtcCol = [];
        	}
        	else 
        	{
        		aEtcCol = this.fv_etccolumn.split(",");
        	}
        	...
        	if (this.fv_usecheck) 
        	{
        		nColWidth = parseInt((nWidth - nChkColWidth) / (nEtcCnt + 1));
        	}
        	else 
        	{
        		nColWidth = parseInt((nWidth) / (nEtcCnt + 1));
        	}
        	...
        	if (this.fv_usecheck) 
        	{
        		sFormats += '	<Column size="'+nChkColWidth+'" band="left" />	\n';
        	}
        ...
        // [add_column] 데이타셋에 컬럼 추가
        this.fn_addColumn = function(objDs,sColumnNm,sColumnType)
        {
        	if (Base.isEmpty(objDs)) 
        	{
        		trace("gfn_addColumn dataset object error");
        	}
        	if (Base.isEmpty(sColumnNm)) 
        	{
        		trace("gfn_addColumn sColumnNm error");
        	}
        	if (Base.isEmpty(sColumnType)) 
        	{
        		sColumnType = "string";
        	}
        ...
        {
        	if (Base.isEmpty(sCol)) 
        	{
        		sCol = this.fv_codecolumn;
        	}
        ...
        {	
        	if (Base.isEmpty(this.fv_innerdataset) || this.fv_innerdataset.rowcount == 0) 
        	{
        		return;
        	}
