...
if ( !JsNamespace )
{
	...
	var JsNamespace = {
		...
		*/
		 ...
		 * @example
		 * trace(JsNamespace.exist('Base.array')); // exist => true
		 * @memberOf JsNamespace		 
		*/
		exist: function(name)
		{
		...
		*/
		 ...
		 * @param {object} value
		 * @example
		 * JsNamespace.setDebugInfo('Base.array', arrayUtil);
		 * @memberOf JsNamespace	 
		 * @private
		*/
		...
		_classHelper: {
			...
			*/
			getPropertiesValues: function()
			{
				...
				var props = this._propertiesMap,
					res = {};
				Base.object.Each(props, function(propNm, val)
				{
			...
			*/
			setProperties: function()
			{
				...
				if ( arguments.length == 1 )
				{
					...
					if ( arg0 )
					{
						if ( !arg0.length && Base.isObject(arg0) )
						{
						...
						else if ( arg0.length || Base.isArray(arg0) )
						{
			...
			*/
			callParent: function(args)
			{
				...
				if ( !curCls )
				{
					//trace.error("class 선언이 JsNamespaces.declareClass 함수를 이용하여 처리되지 않았습니다.");
				}
				var superCls = curCls._superClass;
				if ( !superCls )
				{
					//trace.error(curCls._className + "는 상속 되지 않은 Class입니다.");
				}
				...
				if ( !superMethod )
				{
					//trace("this.callParent() was called but there's no such method (" + method._thisName +
		...
		*/
		 ...
		 * JsNamespace.addMethods(membernames, Base.Point, false, 
		 *      function(name) {
		 *          var script = "return createObject('point', Math." + name + "(this.x), Math." + name + "(this.y));";
		 *          this[name] = new Function(script); //this는 Base.Point.prototype
		...
		*/
		 ...
		 * @example
		 * var membernames = {'x' : {}, 'y': {}};
		 * JsNamespace.addProperties(membernames, Base.Point, 
		 *      function(prop, val) {
		 *          return val; 
		 *      }
		 * );
