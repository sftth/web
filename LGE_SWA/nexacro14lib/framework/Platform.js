...
if (!nexacro._bInitPlatform) {
	...
	_pLoadManager.loadDataModule = function (url, svcid, indatasets, outdatasets, parameters, callback, async, datatype, compress, service) {
		...
		if (!load_item) {
			...
			application._onHttpSystemError(this.context, true, this.context, "comm_fail_duplication_svcid", svcid, 0, url, null);
		}
	};
	...
	_pTouchManager.ontouchmove = function (win, elem, touchid, clientX, clientY, screenX, screenY, curtime, orgtime, is_last_changedtouch) {
		...
		var c1 = win.findComponent(this._start_elem);
		var c2 = win.findComponent(elem);
		if (c1 && c2) {
			if (c1 != c2) {
				if (c1._is_subcontrol) {
					c1 = c1.parent;
				}

				if (c1.enable) {
					c1._stat_change("enable", "normal");
				}
			}
			else {
				c1._stat_change(this._start_elem_status, this._start_elem_pseudo);
			}
		}
	...
	if (!this.application) {
		...
		_pApplication.set_networksecurelevel = function (v) {
			...
			this.networksecurelevel = nexacro._parseInt(v);
		};
