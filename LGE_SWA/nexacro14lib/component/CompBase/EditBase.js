...
if (!nexacro.EditBase) {
	...
	if (nexacro.Browser == "IE" && nexacro.BrowserVersion <= 10) {
		...
		_pEditBase._on_default_input_keyinput = function (elem) {
			...
			if (!this._is_composition() && elem_value == editbase_text && elem_value == editbase_value && (elem_value && (comp.maxlength == 0 || comp.maxlength >= this.getLength(elem, editbase_text)))) {
				this._setCaret(elem);
				this.onUpdateStyle(comp);
				return false;
			}
			...
			if (this._is_hangul(insertText) && diff > insertText.length) {
				...
				this._input_begin_pos = this._input_begin_pos - (diff - insertTextlen);
	...
	_pEditNormal._on_default_input_keypress = function (elem, keyCode, charCode, altKey, ctrlKey, shiftKey) {
		...
		var ioschartext = text.substr(iospos, 1);
		var _is_enter_hangul = false;
		if (k == nexacro.Event.KEY_ENTER && this._is_hangul(ioschartext) && this._begin_pos > 0) {
