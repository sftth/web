    ...
    return function()
    {
        ...
        ***********************************************************************************************/
        ...
        this.fn_retrieveAttachFiles = function(sFileGroupID) {
        	...
        	var sUrl			= "sample/board/fileListDo.do";
        ...
        //alert('OK search:'+strSvcID+":"+nErrorCode+":"+ strErrorMag);
        }
        //-------------------------------------------------------------------------------------------------
        ...
        this.uploadFiles = function(sFileGroupID)
        {
        	...
        	var sURL 	= service.url + "sample/board/fileUpload.do?fileGroupId=" + sFileGroupID;
        ...
        {
        trace('업로드 완료부분:'+e.errormsg+':'+e.errorcode);
        if(e.errorcode < 0){
        	alert(e.errormsg);
        	return;
        }
        	...
        	while(bFind)
        	{
        		if(oParent["fn_ExtFileUp_onsuccess"]){
        			...
        			if(oParent.name == "bizFrame") bFind = false;
        		}
        	}
        }
        ...
        this.downloadFile = function(row)
        {
        	...
        	var sURL =  service.url + "sample/board/FileDownload.do?FILE_GROUP_ID=" + fileGroupId + "&FILE_ID=" + fileId+"&pageCount=1";
        ...
        this.sample_download_onsize = function(obj,e)
        {
        	
        }
        //상위에서 추가시 해당 파일리스트 리셋처리
        this.fn_attfile_reset = function(){
        	trace('clear list');
        	this.ds_attachFile.clearData();
        }
        ...
        this.on_initEvent = function()
        {
            ...
            this.addEventHandler("onsize", this.sample_download_onsize, this);
