...
if ( !JsNamespace.exist("Base.XComp.Event") )
{
	/**
	 * 
	 * @description
	 ...
	 * 
	 * @namespace
	 * @name Base.XComp.Event
	 * @memberof! <global>
	 */	 
	JsNamespace.declare("Base.XComp.Event", {
		 ...
		 *
		 * Base.XComp.Event.add(this.st_sample01, events, this);
		 *
		 ...
		 * var events = {"onlbuttondown": this.onLbuttonDownHandler, "onlbuttonup": this.onLbuttonUpHandler};
		 *
		 * Base.XComp.Event.remove(this.st_sample01, events, this);
		 *
		 * @param {XComp} XComp events 설정할 대상 XComp.
		 * @param {object} events 이벤트명=eventHandler함수로 정의된 object collection.
		 * @param {*} scope scope로 설정할 대상으로 eventHandler 내부 루틴에 this에 해당하는 값
		...
		**/		
		 ...
		 * Base.XComp.Event.makeDraggable(this.st_sample02, this.onDragging, this, [this.st_sample02]);
		 *
		 ...
		 * };
		 * Base.XComp.Event.makeDraggable(this.st_sample02, draggingFunc, this, [this.st_sample02]);		 
		 *
		...
		**/
		"makeDraggable": function(XComp, draggingFunc, scope, addArgs, isOffsetFromStart, delayTask)
		{
			...
			Base.XComp.Event.add(XComp,
					{
						"onlbuttondown": Base.XComp.Event._dragDownHandler,
						"onlbuttonup": Base.XComp.Event._dragUpHandler
					}, scope);
		 ...
		 * @example
		 *
		 * Base.XComp.Event.clearDraggable(this.st_sample02);
		 *
		...
		**/
		"clearDraggable": function(XComp)
		{
			if ( XComp._dragFuncs )
			{
				...
				XComp._delayDragProc = null;
				Base.XComp.Event.remove(XComp,
						{
							"onlbuttondown": Base.XComp.Event._dragDownHandler,
							"onlbuttonup": Base.XComp.Event._dragUpHandler
						});
						
				var topForm = Base.XComp.getTopLevelForm(XComp.parent);
				...
				if ( !hasDragComp && topForm.findEventHandler("onmousemove", Base.XComp.Event._dragMoveHandler) > -1 )
				{
					...
					topForm.removeEventHandler("onmousemove", Base.XComp.Event._dragMoveHandler);
				}
			}
		},
		...
		**/
		...
		{
			var topForm = Base.XComp.getTopLevelForm(obj.parent);
			if ( topForm.findEventHandler("onmousemove", Base.XComp.Event._dragMoveHandler) < 0 )
			{
				topForm.addEventHandler("onmousemove", Base.XComp.Event._dragMoveHandler);
			}
			...
			Base.XComp.Event._lockMouseEvent(topForm);			
		},
		...
		**/
		"_dragUpHandler": function(obj, e)
		{
			...
			if ( drag )
			{
				Base.XComp.Event.cancelAnimationFrame(obj._reqDragId);//func
				obj._reqDragId = null;
				obj._drag = null;
				var func = obj._dragFuncs,
					addArgs,
		...
		"_dragMoveHandler": function(form, e)
		{
			...
			if ( obj )
			{
				...
				if ( drag )
				{
					...
					obj._reqDragId = Base.XComp.Event.requestAnimationFrame(Base.XComp.Event._dragProcess, obj.__scope, obj, e); //func, scope(default: topForm), func's arguments
				}
			}
		},
		...
		*/
		"_lockMouseEvent": function(comp, onlyLbtnUp)
		{
			...
			win._on_sys_lbuttonup = Base.XComp.Event._on_default_sys_lbuttonup;
			if ( !onlyLbtnUp ) win._on_sys_mousemove = Base.XComp.Event._on_default_sys_mousemove;
		},
		...
		*/
		 ...
		 * Base.XComp.Event.makeRepeatable(this.btn_repeat, this.onRepeating, this, [ds, this.grd_sample]);
		 *
		 ...
		 * };
		 * Base.XComp.Event.makeRepeatable(this.btn_repeat, repeatFunc, this, [ds, this.grd_sample]);	 
		 *
		...
		**/
		"makeRepeatable": function(XComp, repeatFunc, scope, args)
		{
			...
			Base.XComp.Event.add(XComp,
					{
						"onlbuttondown": Base.XComp.Event._repeatDownHandler,
					//	"onmouseenter": Base.XComp.Event._repeatEnterHandler,
					//	"onmouseleave": Base.XComp.Event._repeatLeaveHandler,
						"onlbuttonup": Base.XComp.Event._repeatUpHandler
					}, scope);
		},
		 ...
		 * @example
		 *
		 * Base.XComp.Event.clearRepeatable(this.btn_repeat);
		 *		 
		 * @memberOf Base.XComp.Event
		**/
		"clearRepeatable": function(XComp)
		{
			...
			{
				Base.XComp.Event._cancelRepeatable(XComp);
				Base.XComp.Event.remove(XComp, {"onmousemove": Base.XComp.Event._repeatMoveHandler}, this);
		...
		**/
		"_cancelRepeatable": function(XComp)
		{
			...
			{
				Base.XComp.Event.cancelAnimationFrame(XComp._reqId);
				XComp._reqId = null;
			}
		...
		**/
		...
		{
			//Base.XComp.Event._lockMouseEvent(obj, true);
			Base.XComp.Event._lockMouseEvent(obj);

			Base.XComp.Event.add(obj, {"onmousemove": Base.XComp.Event._repeatMoveHandler}, this);
			...
			obj._timeId = nexacro._setSystemTimer(repeatConf._win_handle, 
				function() {
					Base.XComp.Event._repeatProcess.call(pThis, obj); //func, scope(default: topForm), func's arguments
				}, repeatConf.firstInterval);
			//Base.XComp.Event._repeatProcess.call(this, obj);
		},
		...
		**/
		"_repeatUpHandler": function(obj, e)
		{
			...
			Base.XComp.Event.remove(obj, {"onmousemove": Base.XComp.Event._repeatMoveHandler}, this);
			var repeatConfig = obj._repeatFuncs,
				  args, addArgs;

			Base.XComp.Event._cancelRepeatable(obj);
		...
		**/
		"_repeatProcess": function(obj)
		{
			...
			if ( obj._reqId != null )
			{
				Base.XComp.Event.cancelAnimationFrame(obj._reqId);
				obj._reqId = null;
			}
			if ( status == "stop" )
			{
			...
			//obj._reqRepeatId = Base.XComp.Event.requestAnimationFrame(Base.XComp.Event._repeatProcess, this, obj); //func, scope(default: topForm), func's arguments
			...
			if ( func )
			{
				...
				obj._reqId = Base.XComp.Event.requestAnimationFrame(function() { 
					func.apply(this, args);
				}, this);
			}
			if ( status == "firstRepeat" )
		 ...
		 * 	
		 *     var reqId = Base.XComp.Event.requestAnimationFrame(this.renderLoop, this);
		 * }
		 *
		 * this.renderLoop();
		 *
		...
		**/
		"requestAnimationFrame": function(callback, scope)
		{
			...
			{
				args = Base.array.toArray(arguments, 2);
			}
			else
			{
				args = [];
			...
			var useSetTimeout = Base.XComp.Event._requestAnimationFrameUseSetTimer;
			if( useSetTimeout === undefined ) useSetTimeout = false;
			
			var rAF = Base.XComp.Event._requestAnimationFrame;
			if ( !rAF )
			{
				 // Runtime
				if ( isRuntime )
				...
				else	// HTML
				{
					...
					if ( !rAF )
					{
						...
						if ( !rAF )
						{
							...
							Base.XComp.Event._requestAnimationFrameUseSetTimer = true;
							useSetTimeout = true;
						}
					}
				}
				
				Base.XComp.Event._requestAnimationFrame = rAF;
			}
			...
			*/
			if ( !Base.XComp.Event._requestAnimationFrameLastTimeInfo )
			{
				Base.XComp.Event._requestAnimationFrameLastTimeInfo = {};
			}
						
			var id;
			if ( isRuntime )
			{
				var form = Base.XComp.Event._getRequestAnimationFrameForm(scope);
				var callbackString = callback.toString();
				var lastTimeRef = Base.XComp.Event._requestAnimationFrameLastTimeInfo[callbackString];
				if ( lastTimeRef === undefined )
				{	
					Base.XComp.Event._requestAnimationFrameLastTimeInfo[callback.toString()] = {'lastTime': 0, 'timers':[]};
					lastTimeRef = Base.XComp.Event._requestAnimationFrameLastTimeInfo[callbackString];
				}
			...
			{
				var callbackString = callback.toString();
				var lastTimeRef = Base.XComp.Event._requestAnimationFrameLastTimeInfo[callback.toString()];
				if ( lastTimeRef === undefined )
				{	
					Base.XComp.Event._requestAnimationFrameLastTimeInfo[callbackString] = {'lastTime': 0};
					lastTimeRef = Base.XComp.Event._requestAnimationFrameLastTimeInfo[callbackString];
				}
		...
		**/		
		 ...
		 * @example
		 * 
		 * Base.XComp.Event.cancelAnimationFrame(reqId);
		 *
		...
		**/
		...
		{			
			var cAF = Base.XComp.Event._cancelAnimationFrame;
			...
			if ( !cAF )
			{
				...
				{
					cAF = function(id) {
						var lastTimeInfo = Base.XComp.Event._requestAnimationFrameLastTimeInfo;
						if ( lastTimeInfo )
						{
				...
				Base.XComp.Event._cancelAnimationFrame = cAF;
			}
