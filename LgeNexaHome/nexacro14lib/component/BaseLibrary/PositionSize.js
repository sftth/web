...
if ( !JsNamespace.exist("Base.XComp.PositionSize") )
{
	/**
	 * @namespace
	 * @name Base.XComp.PositionSize
	 * @memberof! <global>
	 */
	JsNamespace.declare("Base.XComp.PositionSize", {
		 /**
		 ...
		 * @example
		 * trace(Base.XComp.PositionSize.getTopLevelFormBox(Div00.Button00)); //output: [829,135,63,43]
		 * trace(Base.XComp.PositionSize.getTopLevelFormBox(Div01.Div02.Button01)); //output: [841,346,63,43]
		 * trace(Base.XComp.PositionSize.getTopLevelFormBox(Tab00.tabpage1.Button02)); //output: [813,521,63,43]	
		 *
		 * @memberOf Base.XComp.PositionSize
		 */
		getTopLevelFormBox: function(XComp)	
		{
			var topForm = Base.XComp.getTopLevelForm(XComp);
			var xy = Base.XComp.PositionSize.convertXY(topForm, [0, 0], XComp);
		 ...
		 * <--------------- 900 ------------------>	
		 *		 
		 * trace(Base.XComp.PositionSize.convertXY(this,[0,0], Button00)); //output: [900,300]
		...
		convertXY: function(XCompA, xy, XCompB)
		{
			...
			if ( XCompB )
			{
				...
				var pThis = Base.XComp.PositionSize;
		 ...
		 * @example
		 * trace(Base.XComp.PositionSize.getScrollTop(Div01)); //output: 20
		 * trace(Base.XComp.PositionSize.getScrollTop(Div01)); //output: 0
		 *
		 * @memberOf Base.XComp.PositionSize
		 */
		 ...
		 * @example
		 * trace(Base.XComp.PositionSize.getScrollLeft(Div01)); //output: 10
		 * trace(Base.XComp.PositionSize.getScrollLeft(Div01)); //output: 0
		 *
		 * @memberOf Base.XComp.PositionSize
		 */
		 ...
		 * @example
		 * trace(Base.XComp.PositionSize.getScrollBarSize(Div03, "vert")); //output:  11
		 *
		 * @memberOf Base.XComp.PositionSize
		 */
		 ...
		 * @example
		 * trace(Base.XComp.PositionSize.getCurrentScrollBarSize(Div03, "vert")); //output: 0
		 *
		 * @memberOf Base.XComp.PositionSize
		 */
		 ...
		 * <------------------------ 100 ----------------------->
		 *
		 * var clientwidth = Base.XComp.PositionSize.getClientWidth(Div01);
		...
		getClientWidth: function(XComp, wholeWidth)
		{
			...
			borderWidths = Base.XComp.Style.getBorderWidth(XComp);
			marginWidths = Base.XComp.Style.getMargin(XComp);
			clientwidth -= borderWidths[0] + borderWidths[2];
			clientwidth -= marginWidths[0] + marginWidths[2];
			scrollbarSize = Base.XComp.PositionSize.getCurrentScrollBarSize(XComp, "horz");
		 ...
		 * |------------------------------------------------|   v
		 *
		 * var clientheight = Base.XComp.PositionSize.getClientHeight(Div01);
		...
		getClientHeight: function(XComp, wholeHeight)
		{
			...
			scrollbarSize = Base.XComp.PositionSize.getCurrentScrollBarSize(XComp);
		 ...
		 * <------------------------ 200 ----------------------->
		 *
		 * var scrollwidth = Base.XComp.PositionSize.getScrollWidth(Div00);
		...
		getScrollWidth: function(XComp)
		{
			...
			scrollwidth += Base.XComp.PositionSize.getClientWidth(XComp);
		 ...
		 * |----------------------------------------------------| v
		 *
		 * var scrollheight = Base.XComp.PositionSize.getScrollHeight(Div00);
		...
		getScrollHeight: function(XComp)
		{
			...
			scrollheight += Base.XComp.PositionSize.getClientHeight(XComp);
		 ...
		 * trace(Base.XComp.PositionSize.getTextSize(btn_sample1)); // output : [69,12]
		 *
		 * // btn_sample2.style.font ==> Dotum,9,bold
		 * trace(Base.XComp.PositionSize.getTextSize(btn_sample2)); // output : [80,12]
		 *
		 * // btn_sample3.style.font ==> Verdana,10
		 * trace(Base.XComp.PositionSize.getTextSize(btn_sample3)); // output : [83,16]
		 *
		 * @memberOf Base.XComp.PositionSize
		 */	
		getTextSize: function(XComp, text, multiline, content_width, fitText)
		{
			...
			{
				font = XComp;
				multiline = Base.isEmpty(multiline) ? false : multiline;
			...
			else
			{
				...
				if ( Base.isEmpty(font) )
				{
					font = nexacro.Component._default_font;
				}				
				
				text = Base.isEmpty(text) ? XComp.text : text;
				if ( Base.isEmpty(multiline) )
				{
		 ...
		 * Base.XComp.PositionSize.getImageSize(url, this.imageLoadCallback, this);
		 ...
		 * trace(Base.XComp.PositionSize.getContentSize(btn_sample4)); // output : [73,16]
		 *
		 * // chk_sample1 ==> width : 38, height : 20, text : "CheckBox", border : 1px solid red
		 * trace(Base.XComp.PositionSize.getContentSize(chk_sample1)); // output : [80,14]
		 *
		 * // cmb_sample1 ==> width : 150, height : 20, text : ""
		 * trace(Base.XComp.PositionSize.getContentSize(cmb_sample1)); // output : [38,12]
		 *
		 * // cal_sample1 ==> width : 120, height : 29, value : "20130505"
		 * trace(Base.XComp.PositionSize.getContentSize(cal_sample1)); // output : [108,16]
		 *
		 * // txt_sample1 ==> width : 114, height : 44
		 * trace(Base.XComp.PositionSize.getContentSize(txt_sample1)); // output : [114,44]
		 *
		 * @memberOf Base.XComp.PositionSize
		 */
		getContentSize: function(XComp, width, multiLine)
		{
			...
			if ( Base.isXComponent(XComp) )
			{		
				var w = 0, h = 0;		
				
				// scroll을 가지는 컴포넌트는 현재 자신의 size 를 리턴
				if ( !Base.isEmpty(XComp.hscrollbar) && !Base.isEmpty(XComp.vscrollbar) )
				{
					if ( Base.XComp.typeOf(XComp) == "Tabpage" ) 
					{
				...
				else
				{
					...
					{
						sz = Base.XComp.PositionSize.getTextSize(XComp, "W", false);
					}
					else
					{
						if ( Base.isNumber(width) )
						{
							var spaceSize = this._getStyleSpaceSize(XComp);
							width -= spaceSize[0];
							
							sz = Base.XComp.PositionSize.getTextSize(XComp, text, true, width);
						}
						else
						{
							sz = Base.XComp.PositionSize.getTextSize(XComp, text, multiLine);
						}
					}
					...
					var type = Base.XComp.typeOf(XComp),
					...
					if ( type == "Calendar" )
					{
						...
						{
							// calendaredit
							sz = Base.XComp.PositionSize._getStyleSpaceSize(XComp.calendaredit);
							w += sz[0];
							h += sz[1];
						}
					}
					...
					{
						sz = Base.XComp.PositionSize._getStyleSpaceSize(XComp.comboedit);
		 ...
		 * @example
		 *   var position = Base.XComp.PositionSize.getPopupPosition(Button00, 44, 23, "vert", 2); //return: [bottom, 1394, 317]
		 *   PopupDiv00.trackPopup(position[1], position[2]);
		 *
		 *   //해상도(1920*1080)를 초과한 높이 2000의 위치를 요구했을경우.
		 *   var position = Base.XComp.PositionSize.getPopupPosition(Button00, 300, 2000, "vert", 2); //return: [right,854,0,300,1080]
		 *   PopupDiv00.trackPopup(position[1], position[2]);
		 * @memberOf Base.XComp.PositionSize
		 */			
		 ...
		 * @example
		 *   var position = Base.XComp.PositionSize.getPositionByForm(Button00, 44, 23, "vert", 2, this); //return: [bottom, 1394, 317]
		 *   Button00.move(position[1], position[2]);
		 *
		 *   //해상도(1920*1080)를 초과한 높이 2000의 위치를 요구했을경우.
		 *   var position = Base.XComp.PositionSize.getPositionByForm(Button00, 300, 2000, "vert", 2, this); //return: [right,854,0,300,1080]
		 *   Button00.move(position[1], position[2]);
		 * @memberOf Base.XComp.PositionSize
		 */			
		getPositionByForm: function(XComp, width, height, direction, offset, scopeXComp)
		{
			...
			var xy = Base.XComp.PositionSize.convertXY(relativeXComp, [0, 0], XComp);
			var x = xy[0];
			var y = xy[1];
			...
			var position = Base.XComp.PositionSize._getDisplayPosition(width, height, x, y, rect, direction, offset);
		 ...
		 */	
		 _getHorizontalAlign: function(width, height, screenX, screenY, screenRect, direction, offset)
		 {
			...
			{
				screenX = undefined;
				//trace({message:"unexpected situation!!", stack:true});
			}
